...
static void script_vm_4ce14(Core *c) {
  ...
  if (G0 == 0) {
    ...
    if (G0 == 6 && c->mem_r8(0x800BF871u) == 19) {
      c->r[2] = 19;
      return;
    }
    // else run
  ...
  for (;;) {
    ...
    if (!s2set) { // bit7 clear -> predicate 0x8004D7EC
      if (eng(c).bit.test7EC(c->mem_r16s(s4 + 10), 0) != 0) {
        skip = true;
      }
    }
    if (!skip && (c->mem_r32(s5 + 20) & mask)) {
      skip = true; // slot already done
    }
    if (!skip && s2set) { // bit7 set -> predicate 0x8004D868
      if (eng(c).bit.test868(c->mem_r16s(s4 + 10)) != 0) {
        skip = true;
      }
    }
    if (!skip && (c->mem_r32(s5 + 20) & mask)) {
      skip = true; // re-check (predicate may have set it)
    }
    if (!skip) {
      uint32_t ret;
      ...
      if (ret != 0) {
        c->mem_w32(s5 + 16, c->mem_r32(s5 + 16) | mask);
      }
    }
    s3++;
    s4 += 16;
