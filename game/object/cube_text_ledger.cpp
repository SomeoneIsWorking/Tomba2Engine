...
static constexpr GuestFrameSpill kSpills_80040C00[3] = {
    {17, 20},
    {31 /*ra*/, 24},
    {16, 16},
};
static constexpr GuestFrameSpill kSpills_80040AA4[4] = {
    {17, 20},
    {18, 24},
    {31 /*ra*/, 28},
    {16, 16},
...
// Fixed cells (addresses already folded from the recorded binary evidence's `<hi>16<<16 + <lo>` constants —
...
// NOT reimplemented here (out of this agent's RE'd scope) — invoked via typed runtime address dispatch so a future
...
void CubeTextLedger::deactivateSlot(Core *c) {
  GuestFrame<32, 3> frame(c, kSpills_80040C00);
  const uint32_t slot = c->r[4];
  if (c->mem_r16s(G_LEDGER_GATE) == 0) {
    c->r[2] = 0xFFFFFFFFu;
    return;
  }
...
void CubeTextLedger::spawnPopup(Core *c) {
  GuestFrame<32, 4> frame(c, kSpills_80040AA4);
  ...
  c->r[5] = 3;
  c->r[6] = 1;
  psx::cpu::dispatchGuestToReturn0(*c, FN_FREELIST_ALLOC, psx::cpu::ExecutionBudget::currentTurn(*c), __func__);
  ...
  c->r[5] = value;
  c->r[6] = variant;
  psx::cpu::dispatchGuestToReturn0(*c, FN_NODE_INIT, psx::cpu::ExecutionBudget::currentTurn(*c), __func__);
  c->r[2] = node;
}
...
// tomba::native::declareOverride() per address, with a tomba::native::declareOverride setter so the shared thunk lands
// in the recorded binary evidence's image-qualified runtime dispatcher table (what the substrate's direct `a direct
// guest-address call` call sites consult — confirmed via authenticated executable/overlay evidence, shard_1.c,
// shard_2.c, shard_4.c, shard_5.c for FN_40B48/FN_40C00, and authenticated executable/overlay evidence for FN_40AA4),
// while any native caller reaching these via typed runtime address dispatch(c, addr) (ActorReward::smEventDispatch
// does, for FN_40B48/FN_40C00) hits the same registry entry. The test-only substrate gate keeps running the exact guest
// body) lives once inside the registry, not per-trampoline.

void CubeTextLedger::registerOverrides(Game * /*game*/) {
  tomba::native::declareOverride(0x80040C00u, "CubeTextLedger::deactivateSlot", CubeTextLedger::deactivateSlot);
  tomba::native::declareOverride(0x80040AA4u, "CubeTextLedger::spawnPopup", CubeTextLedger::spawnPopup);
}
