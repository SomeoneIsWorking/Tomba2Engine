...
//
// RE SOURCE: Ghidra headless decompile (the Ghidra evidence workflow, project scratch/ghidra/main_ram) cross-
...
#include "object/actor.h"            // Actor — named-field lens (pos/vel are 16.16, see findings)
#include <cstdint>

// Guest-stack spill tables — generated by `tools/binary ABI evidence <addr> --scaffold --guestabi`,
// never hand-derived. Frame sizes/offsets come straight from the oracle body.
static constexpr GuestFrameSpill kSpills_8004B208[3] = {
    {16, 16},
    {31 /*ra*/, 24},
    {17, 20},
};
static constexpr GuestFrameSpill kSpills_80049A60[3] = {
    {16, 16},
    {31 /*ra*/, 24},
    {17, 20},
};
static constexpr GuestFrameSpill kSpills_80049E54[1] = {
    {31 /*ra*/, 16},
};
static constexpr GuestFrameSpill kSpills_8004A3D4[3] = {
    {17, 20},
    {31 /*ra*/, 24},
    {16, 16},
};
...
// reached uniformly through typed runtime address dispatch like every other callee here).
...
// pointers, read via a base-register load at 0x800E7E80+0xD0/0xDC in the raw guest code).
...
// ----------------------------------------------------------------------------------------------
// ORACLE: guest 0x8004B208
void ActorReward::smBlinkB(Core *c) {
  GuestFrame<32, 3> frame(c, kSpills_8004B208);
  const uint32_t obj = c->r[R_A0];
  const uint32_t side = c->r[R_A1];
  if (c->mem_r8(obj + 5) == 0) {
    c->mem_w8(obj + 5, 1);
    ...
    c->r[R_A0] = obj;
    c->r[R_A1] = (uint32_t)delta;
    c->r[R_A2] = 0;
    c->r[R_A3] = 0;
    psx::cpu::dispatchGuestToReturn0(*c, FN_41194, psx::cpu::ExecutionBudget::currentTurn(*c), __func__);
    if (side == 0) {
      c->mem_w8(obj + 0x18, 0);
    } else {
      c->mem_w8(obj + 0x18, 0xff);
...
// ----------------------------------------------------------------------------------------------
// ORACLE: guest 0x80049A60
void ActorReward::smWindowScroll(Core *c) {
  GuestFrame<32, 3> frame(c, kSpills_80049A60);
  const uint32_t obj = c->r[R_A0];
  Actor a(c, obj);
  const uint32_t side = c->r[R_A1];
  ...
  case 1: {
    int16_t prevScroll = a.velY();
    uint16_t newScroll = (uint16_t)(c->mem_r16(obj + 0x4a) + c->mem_r16(obj + 0x50));
    a.setVelY(newScroll);
    int32_t accum = (int32_t)a.posYFixed() + (int32_t)prevScroll * 0x100;
    a.setPosYFixed((uint32_t)accum);
    ...
    if (iv < 0) {
      ...
      c->r[R_A0] = obj;
      psx::cpu::dispatchGuestToReturn0(*c, FN_766C, psx::cpu::ExecutionBudget::currentTurn(*c), __func__);
      c->r[R_A0] = obj;
      c->r[R_A1] = 0;
      c->r[R_A2] = (uint32_t)(int32_t)(-c->mem_r16s(obj + 0x84));
      psx::cpu::dispatchGuestToReturn0(*c, FN_493E8, psx::cpu::ExecutionBudget::currentTurn(*c), __func__);
      if (c->r[R_V0] != 0) {
        c->mem_w8(obj + 0, 1);
        a.setVelY(0);
        c->mem_w8(obj + 5, (uint8_t)(c->mem_r8(obj + 5) + 1));
  ...
  case 2: {
    int16_t scroll = a.velY();
    int32_t accum = (int32_t)a.posYFixed() + (int32_t)scroll * 0x100;
    a.setPosYFixed((uint32_t)accum);
    if (scroll < 0x3000) {
      int16_t speed = a.accelY();
      a.setVelY((uint16_t)(int16_t)(scroll + speed));
    }
    if (c->mem_r8(obj + 0x29) == 0) {
      ...
      if ((c->mem_r8(obj + 0x28) & 0x80) == 0) {
        ...
        c->r[R_A0] = obj;
        c->r[R_A1] = 0;
        c->r[R_A2] = (uint32_t)(int32_t)delta;
        psx::cpu::dispatchGuestToReturn0(*c, FN_49250, psx::cpu::ExecutionBudget::currentTurn(*c), __func__);
        ...
        if (c->mem_r8(G_E7FEB) == 8) {
          bool cond = (c->mem_r8(G_BF816) != 1) || ((uint32_t)c->mem_r8(G_BF817) != c->mem_r16(obj + 0x6a)) ||
                      ((int32_t)c->mem_r16s(obj + 0x32) +
                           ((int32_t)c->mem_r16s(obj + 0x86) - (int32_t)c->mem_r16s(obj + 0x84)) <
                       (int32_t)c->mem_r16s(G_BF812));
          if (cond) {
            break; // no state-5, no scroll reset
          }
          c->mem_w16(obj + 0x32,
                     (uint16_t)(c->mem_r16s(G_BF812) - (c->mem_r16s(obj + 0x86) - c->mem_r16s(obj + 0x84))));
          setState5 = true;
        } else {
          c->r[R_A0] = obj;
          psx::cpu::dispatchGuestToReturn0(*c, FN_766C, psx::cpu::ExecutionBudget::currentTurn(*c), __func__);
  ...
  case 6: {
    ...
    psx::cpu::dispatchGuestToReturn0(*c, FN_77ACC, psx::cpu::ExecutionBudget::currentTurn(*c), __func__);
    if (c->r[R_V0] == 0) {
      c->mem_w8(obj + 4, 3);
    }
    break;
...
// ----------------------------------------------------------------------------------------------
// ORACLE: guest 0x80049E54
void ActorReward::smTallyTick(Core *c) {
  GuestFrame<24, 1> frame(c, kSpills_80049E54);
  const uint32_t obj = c->r[R_A0];
  ...
  if (c->mem_r8(obj + 7) == 0) {
    if ((int16_t)before < 1) {
      ...
      if (changed) {
        c->mem_w16(G_TALLY_PREV, (uint16_t)(cur - 1));
      }
      uint32_t savedA1 = step;
      c->r[R_A0] = obj;
      c->r[R_A1] = savedA1;
      psx::cpu::dispatchGuestToReturn0(*c, FN_72114, psx::cpu::ExecutionBudget::currentTurn(*c), __func__);
      ret = 0;
    }
  } else {
    ret = 0;
...
// ----------------------------------------------------------------------------------------------
// ORACLE: guest 0x8004A3D4
void ActorReward::smEventDispatch(Core *c) {
  GuestFrame<32, 3> frame(c, kSpills_8004A3D4);
  const uint32_t obj = c->r[R_A0];

  if (c->mem_r16(G_TALLY_CUR) == 0) {
    c->r[R_V0] = 0;
    return;
  }
  ...
  case 0: {
    c->r[R_A0] = evt;
    c->r[R_A1] = 1;
    psx::cpu::dispatchGuestToReturn0(*c, FN_D4C4, psx::cpu::ExecutionBudget::currentTurn(*c), __func__);
    c->mem_w8(G_BF88C, (uint8_t)evt);
    c->mem_w8(G_E7EEC, c->mem_r8(G_BF88C));
    c->r[R_A0] = 0x1e;
    psx::cpu::dispatchGuestToReturn0(*c, FN_40C00, psx::cpu::ExecutionBudget::currentTurn(*c), __func__);
    break;
  }
  case 5: {
    c->r[R_A0] = evt;
  ...
  case 3:
    ...
    // guest frame, so every reacher must carry the RE'd constant (guest 0x8004A3D4 evidence).
    c->r[R_A0] = 9;
    c->r[31] = 0x8004A458u;
    psx::cpu::dispatchGuestToReturn0(*c, FN_40B48, psx::cpu::ExecutionBudget::currentTurn(*c), __func__);
    [[fallthrough]];
  case 1:
  case 2:
  case 4:
  ...
  case 10: {
    ...
    c->r[R_A0] = 0;
    psx::cpu::dispatchGuestToReturn0(*c, FN_67DA8, psx::cpu::ExecutionBudget::currentTurn(*c), __func__);
  ...
  case 0x56: {
    c->r[R_A0] = 0x4d;
    c->r[R_A1] = 2;
    psx::cpu::dispatchGuestToReturn0(*c, FN_D4F4, psx::cpu::ExecutionBudget::currentTurn(*c), __func__);
    c->r[R_A0] = 0x57;
    c->r[R_A1] = 2;
    psx::cpu::dispatchGuestToReturn0(*c, FN_ED0C, psx::cpu::ExecutionBudget::currentTurn(*c), __func__);
    c->r[R_A0] = 0x3e;
    c->r[31] = 0x8004A548u;
    psx::cpu::dispatchGuestToReturn0(*c, FN_40B48, psx::cpu::ExecutionBudget::currentTurn(*c), __func__);
    break;
  ...
  case 0x73: {
    ...
    if (key == 0x6f) {
      if (c->mem_r8(G_BFB24) == 0) {
        ...
        c->r[R_A0] = 0x70;
        c->r[R_A1] = 1;
        psx::cpu::dispatchGuestToReturn0(*c, FN_D650, psx::cpu::ExecutionBudget::currentTurn(*c), __func__);
        cue = 0x11;
      }
    } else if (key == 0x71) {
      if (c->mem_r8(G_BFB26) == 0) {
        c->r[R_A0] = 0x72;
        c->r[R_A1] = 1;
        psx::cpu::dispatchGuestToReturn0(*c, FN_D4F4, psx::cpu::ExecutionBudget::currentTurn(*c), __func__);
        cue = 0x12;
    ...
    c->r[R_A0] = cue;
    c->r[R_A1] = 0x41;
    psx::cpu::dispatchGuestToReturn0(*c, FN_ED94, psx::cpu::ExecutionBudget::currentTurn(*c), __func__);
    break;
  }
  case 0x7b: {
    c->r[R_A0] = evt;
  ...
  case 0x89: {
    ...
    psx::cpu::dispatchGuestToReturn0(
        *c, FN_40B48, psx::cpu::ExecutionBudget::currentTurn(*c), __func__); // shared tail L_8004A6CC
    break;
  }
  case 0x90: {
    if (c->mem_r8(G_TALLY_CAP) == 8) {
      c->mem_w8(G_TALLY_CAP, c->mem_r8(G_TALLY_CAP2));
    }
    ...
    c->r[R_A0] = 0x711;
    c->r[R_A1] = 0;
    psx::cpu::dispatchGuestToReturn0(*c, FN_310F4, psx::cpu::ExecutionBudget::currentTurn(*c), __func__);
    {
  ...
  case 0x99: {
    ...
    c->r[R_A1] = 100000;
    c->r[R_A2] = 0;
    psx::cpu::dispatchGuestToReturn0(*c, FN_71B44, psx::cpu::ExecutionBudget::currentTurn(*c), __func__);
    [[fallthrough]];
  }
  default: {
    c->r[R_A0] = evt;
  ...
  case 0xa2: {
    c->r[R_A0] = obj;
    psx::cpu::dispatchGuestToReturn0(*c, FN_115AEC, psx::cpu::ExecutionBudget::currentTurn(*c), __func__);
    break;
  }
  case 0xa5: {
    c->r[R_A0] = evt;
    ...
    psx::cpu::dispatchGuestToReturn0(*c, FN_D4C4, psx::cpu::ExecutionBudget::currentTurn(*c), __func__);
    c->r[R_A0] = obj;
    psx::cpu::dispatchGuestToReturn0(*c, FN_114F24, psx::cpu::ExecutionBudget::currentTurn(*c), __func__);
    break;
  }
  }

  c->r[R_A0] = 0x1f;
  c->r[R_A1] = (uint32_t)-140;
  psx::cpu::dispatchGuestToReturn0(*c, FN_310F4, psx::cpu::ExecutionBudget::currentTurn(*c), __func__);
  uint32_t o2 = c->r[R_V0];
  if (o2 != 0) {
    c->mem_w8(o2 + 0x28, (uint8_t)(c->mem_r8(o2 + 0x28) | 0x80));
  ...
  c->r[R_A1] = 0;
  c->r[R_A2] = 0;
  psx::cpu::dispatchGuestToReturn0(*c, FN_74590, psx::cpu::ExecutionBudget::currentTurn(*c), __func__);
  c->r[R_V0] = 1;
}
...
// Ghidra headless decompile (scratch/ghidra/main_ram, the Ghidra evidence workflow) cross-checked against the
// raw guest ground truth (authenticated executable/overlay evidence guest 0x80070018, shard_1.c guest 0x800702C0,
// shard_2.c guest 0x80070650) for exact field widths, arithmetic order, and (for update/
...
// guest 0x80070018 exactly -- see ObjectTable::dispatchFaithful for the same mirror shape).
// ----------------------------------------------------------------------------------------------
namespace {
...
void ActorReward::update(Core *c) {
  ...
  if (state == 1) {
    ...
    if (c->mem_r8(obj + 5) == 0) {
      // ra mirror: guest 0x80070018 sets c->r[31]=0x800700C8u before guest 0x800702C0(c) --
      // resolvePosition's own frame spills this ra at its +32 slot; must byte-match.
      c->r[R_A0] = obj;
      c->r[31] = 0x800700C8u;
      ActorReward::resolvePosition(c);
    } else if (c->mem_r8(obj + 5) == 1) {
      // ra mirror: ground truth sets c->r[31]=0x800700D8u before guest 0x80070650(c).
      c->r[R_A0] = obj;
      c->r[31] = 0x800700D8u;
      ActorReward::approachTargetX(c);
    }
    ...
    if (state != 2) {
      ...
      // ra mirror: ground truth sets c->r[31]=0x800702ACu before guest 0x8007A624(c); FN_A624
      // spills it at its own +20 frame slot -- must byte-match.
      c->r[R_A0] = obj;
      c->r[31] = 0x800702ACu;
      psx::cpu::dispatchGuestToReturn0(*c, FN_A624, psx::cpu::ExecutionBudget::currentTurn(*c), __func__);
      updateEpilogue(c);
      return;
    }
      ...
      if ((c->mem_r8(obj + 0x5f) & 1) != 0) {
        if ((c->mem_r8(obj + 0x5f) & 2) != 0) {
          ...
          case 4:
            ...
            c->r[R_A1] = 100;
            c->r[31] = 0x800701F0u;
            psx::cpu::dispatchGuestToReturn0(*c, FN_B3F4, psx::cpu::ExecutionBudget::currentTurn(*c), __func__);
            result = c->r[R_V0];
            break;
          case 5:
            c->r[R_A0] = obj;
          ...
          case 0xf:
            c->r[R_A0] = obj;
            c->r[31] = 0x80070254u;
            psx::cpu::dispatchGuestToReturn0(*c, FN_A118, psx::cpu::ExecutionBudget::currentTurn(*c), __func__);
            result = c->r[R_V0];
            break;
          case 0x10:
            c->r[R_A0] = obj;
            c->r[31] = 0x80070264u;
            psx::cpu::dispatchGuestToReturn0(*c, FN_A2A0, psx::cpu::ExecutionBudget::currentTurn(*c), __func__);
            result = c->r[R_V0];
            break;
          case 0x11:
            c->r[R_A0] = obj;
            c->r[31] = 0x80070274u;
            psx::cpu::dispatchGuestToReturn0(*c, FN_B428, psx::cpu::ExecutionBudget::currentTurn(*c), __func__);
            result = c->r[R_V0];
            break;
          default:
            break;
        ...
        // ra mirror: ground truth sets c->r[31]=0x80070290u before guest 0x8004A3D4(c).
        c->r[R_A0] = obj;
        c->r[31] = 0x80070290u;
        ActorReward::smEventDispatch(c); // same class, direct call
        if (c->r[R_V0] == 0) {
          updateEpilogue(c);
          return;
        }
  ...
  // ra mirror: ground truth sets c->r[31]=0x80070168u before guest 0x800517F8(c) (renderUpdateBody
  // IS framed -- see GraphicsBind::renderUpdateBody -- and its nested propagateRotmat() tail call
  // spills whatever's in c->r[31] into its own frame), or 0x80070158u before guest 0x80077B5C(c).
  if ((flags5f & 0x80) == 0) {
    c->r[R_A0] = obj;
    c->r[31] = 0x80070168u;
    psx::cpu::dispatchGuestToReturn0(
        *c, FN_517F8, psx::cpu::ExecutionBudget::currentTurn(*c), __func__); // GraphicsBind::renderUpdateBody
  } else {
    c->r[R_A0] = obj;
    c->r[31] = 0x80070158u;
    psx::cpu::dispatchGuestToReturn0(
        *c, FN_77B5C, psx::cpu::ExecutionBudget::currentTurn(*c), __func__); // Animation::advanceLinkChain
  }
  updateEpilogue(c);
}
...
// count)=sp+28, s1(angle)=sp+20, s0(owner)=sp+16 (matches guest 0x800702C0 exactly).
// ----------------------------------------------------------------------------------------------
namespace {
...
void ActorReward::resolvePosition(Core *c) {
  ...
  if (sel < 8) {
    ...
    case 3: {
      ...
      auto roundShiftR13 = [](int32_t v) -> int32_t {
        return (v - (v >> 31)) >> 13;
      };
...
// stack frame (matches guest 0x80070650 exactly -- no sp descent in the raw guest body).
// ----------------------------------------------------------------------------------------------
...
// (tomba::native::declareOverride), each with a tomba::native::declareOverride setter (reaches the substrate's DIRECT
// a direct guest-address call calls from FUN_8004AAC4) that shares the same entry any NATIVE caller reaches
// going through typed runtime address dispatch, traced by the `dispatch` debug channel. See actor_sm_reward.h
...
// update (0x80070018) has NO direct same-shard caller (grepped authenticated executable/overlay evidence) -- it's
// reached only via the DYNAMIC per-object dispatch in ObjectTable::dispatchFaithful (typed runtime address dispatch(c,
// fn) with fn read from the object's own type table at runtime), so the typed runtime address dispatch-side registry
// entry alone is the reach path (the tomba::native::declareOverride setter below is installed anyway, defensively).
// resolvePosition (0x800702C0) / approachTargetX (0x80070650) DO have a direct same-shard caller
// (guest 0x80070018's own guest 0x800702C0(c)/guest 0x80070650(c) calls) -- but that caller is FULLY
// SUPERSEDED once update() is registered (ObjectTable::dispatchFaithful never falls through to
// guest 0x80070018 anymore), so those direct calls become dead in practice for core A. Dual-wired
// anyway for defensive completeness / consistency with the other 5 ActorReward leaves, and because
// the test-only substrate leg must still reach the real guest body via the image-qualified runtime dispatcher if
// anything ever calls guest 0x800702C0/80070650 directly outside update()'s own reach.

void ActorReward::registerOverrides(Game * /*game*/) {
  tomba::native::declareOverride(0x80049A60u, "ActorReward::smWindowScroll", ActorReward::smWindowScroll);
  tomba::native::declareOverride(0x80049E54u, "ActorReward::smTallyTick", ActorReward::smTallyTick);
  tomba::native::declareOverride(0x8004A3D4u, "ActorReward::smEventDispatch", ActorReward::smEventDispatch);
  tomba::native::declareOverride(0x80070018u, "ActorReward::update", ActorReward::update);
  tomba::native::declareOverride(0x800702C0u, "ActorReward::resolvePosition", ActorReward::resolvePosition);
  tomba::native::declareOverride(0x80070650u, "ActorReward::approachTargetX", ActorReward::approachTargetX);
  tomba::native::declareOverride(0x8004B150u, "ActorReward::smBlinkA", ActorReward::smBlinkA);
  tomba::native::declareOverride(0x8004B208u, "ActorReward::smBlinkB", ActorReward::smBlinkB);
}
