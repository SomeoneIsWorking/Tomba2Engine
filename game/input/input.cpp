...
L_800937A4:;
  ...
  c->r[31] = 0x800938D4u;
  c->mem_w32((c->r[29] + (uint32_t)16), c->r[3]);
  func_80099970(c);
  c->r[1] = (uint32_t)32784u << 16;
...
L_80093900:;
  ...
  c->r[1] = (uint32_t)32784u << 16;
  c->r[31] = 0x8009397Cu;
  c->mem_w16((c->r[1] + (uint32_t)23770), (uint16_t)c->r[2]);
  func_800931C0(c);
...
static void eov_voiceTableInit(Core *c) {
  Input::voiceTableInit(c);
}
...
void Input::registerOverrides(Game * /*game*/) {
  overrides::install(
      0x80092E3Cu, "Input::setVoiceVolume", &Input::setVoiceVolume, gen_func_80092E3C, shard_set_override);
  overrides::install(0x80093650u, "Input::voiceTableInit", eov_voiceTableInit, gen_func_80093650, shard_set_override);
}
