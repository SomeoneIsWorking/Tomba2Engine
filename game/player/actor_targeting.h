...
struct TargetingBody {
  ...
  uint32_t typeByte()  const { return mCore->mem_r8(mAt + targeting::kTypeByte); }
  uint32_t guardByte() const { return mCore->mem_r8(mAt + targeting::kGuardByte); }
  uint32_t walkState() const { return mCore->mem_r8(mAt + targeting::kWalkState); }
  uint32_t posX()      const { return mCore->mem_r16(mAt + targeting::kPosX); }
  uint32_t posY()      const { return mCore->mem_r16(mAt + targeting::kPosY); }
  uint32_t posZ()      const { return mCore->mem_r16(mAt + targeting::kPosZ); }
  uint32_t heading()   const { return mCore->mem_r16(mAt + targeting::kHeading); }
  uint32_t heightOff() const { return mCore->mem_r16(mAt + targeting::kHeightOff); }
  int32_t  reach()     const { return mCore->mem_r16s(mAt + targeting::kReach); }
  int32_t  bandY()     const { return mCore->mem_r16s(mAt + targeting::kBandY); }

  void setContact(uint8_t v) const { mCore->mem_w8(mAt + targeting::kContact, v); }
};
