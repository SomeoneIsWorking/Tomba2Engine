...
constexpr uint32_t LEAF_NILADIC_CUE =
    0x8001F830u; // FUN_8001F830 — niladic substrate cue (type8Interact's item[0]==5 branch)
...
void ActorTomba::mode0ActionGate() {
  ...
  if (pathA) {
    guest_fn(c, 0x8005A970u, 0x8005A950u); // normal handler (direct same-shard in gen)
  } else {
    guest_fn(c, 0x80112B50u, 0x8005A960u); // swim/water-interaction handler
  }
}
...
L_80060300:;
  c->r[3] = c->r[3] | c->r[2];
  c->mem_w8((c->r[4] + (uint32_t)361), (uint8_t)c->r[3]);
  return;
L_8006030C:;
...
void ActorTomba::gov_proximityAngleWalk(Core *c) {
  eng(c).actorTomba.proximityAngleWalk(c);
}
void ActorTomba::gov_limbFrameLoad(Core *c) {
  eng(c).actorTomba.limbFrameLoad(c);
}
void ActorTomba::gov_invincibilityFlashStep(Core *c) {
  eng(c).actorTomba.invincibilityFlashStep(c);
}
void ActorTomba::gov_rampOffsetStep(Core *c) {
  eng(c).actorTomba.rampOffsetStep(c);
}
...
void ActorTomba::gov_matrixComposeAttached(Core *c) {
  eng(c).actorTomba.matrixComposeAttached();
}
void ActorTomba::gov_enterOuterState0(Core *c) {
  eng(c).actorTomba.enterOuterState0((int32_t)c->r[5]);
}
void ActorTomba::gov_mode0ActionGate(Core *c) {
  eng(c).actorTomba.mode0ActionGate();
}
void ActorTomba::gov_mode0WalkHandler(Core *c) {
  eng(c).actorTomba.mode0WalkHandler();
}
void ActorTomba::gov_actionHandler8005ACC8(Core *c) {
  eng(c).actorTomba.actionHandler8005ACC8();
}
void ActorTomba::gov_actionHandler8005AEE4(Core *c) {
  eng(c).actorTomba.actionHandler8005AEE4();
}
void ActorTomba::gov_actionHandler8005F1B0(Core *c) {
  eng(c).actorTomba.actionHandler8005F1B0();
}
void ActorTomba::gov_actionHandler800588BC(Core *c) {
  eng(c).actorTomba.actionHandler800588BC();
}
void ActorTomba::gov_actionHandler800531DC(Core *c) {
  eng(c).actorTomba.actionHandler800531DC();
}
void ActorTomba::gov_actionHandler800660AC(Core *c) {
  eng(c).actorTomba.actionHandler800660AC();
}
void ActorTomba::gov_actionHandler8005EF48(Core *c) {
  eng(c).actorTomba.actionHandler8005EF48();
}
...
void ActorTomba::registerOverrides(Game * /*game*/) {
  ...
  extern void gen_func_8005AEE4(Core *);
  engine_set_override_main(0x8005AEE4u, gov_actionHandler8005AEE4, gen_func_8005AEE4);
  extern void gen_func_8005F1B0(Core *);
  engine_set_override_main(0x8005F1B0u, gov_actionHandler8005F1B0, gen_func_8005F1B0);
  extern void gen_func_800588BC(Core *);
  engine_set_override_main(0x800588BCu, gov_actionHandler800588BC, gen_func_800588BC);
  extern void gen_func_800531DC(Core *);
  engine_set_override_main(0x800531DCu, gov_actionHandler800531DC, gen_func_800531DC);
  extern void gen_func_800660AC(Core *);
  engine_set_override_main(0x800660ACu, gov_actionHandler800660AC, gen_func_800660AC);
  extern void gen_func_8005EF48(Core *);
  engine_set_override_main(0x8005EF48u, gov_actionHandler8005EF48, gen_func_8005EF48);
...
void ActorTomba::matrixComposeAttached() {
  ...
  if (c->r[2] != 0) {
    ...
    while (true) {
      ...
      if (!((int32_t)c->r[19] < (int32_t)c->r[2])) {
        break;
      }
...
void ActorTomba::enterOuterState0(int32_t mode) {
  ...
  c->r[7] = c->r[7] + 16296u;
  rec_dispatch(
      c, 0x800519E0u); // GraphicsBind::recordArrayInit(G,17,*(0x800ED014),0x800A3FA8) — a2 = [r17+188], r17=0x800ECF58
  {
  ...
  c->r[5] = c->r[5] & 1u;
  rec_dispatch(c,
               0x80057DC0u); // ActorTomba::growthStep(G, sign-bit(G+0x17E)) — ALREADY NATIVE, dispatched here for now
  c->r[4] = c->r[16] + c->r[0];
  c->r[2] = (uint32_t)8064u << 16;
  c->r[31] = 0x80058768u;
...
L_800587D8:
  ...
  c->r[4] = c->r[16] + c->r[0];
  rec_dispatch(c,
               c->r[2]); // table-dispatched substrate leaf (table @ 0x800A45B8 or 0x800C45B8, indexed by DAT_800BF870)
L_800587F8:
  c->r[2] = (uint32_t)8064u << 16;
...
void ActorTomba::caseAreaEntryHook_80065374() {
  ...
  if (c->r[3] == c->r[2]) {
    goto L_800653CC; // area==5
  }
  c->r[2] = (uint32_t)((int32_t)c->r[3] < 6);
  if (c->r[2] == c->r[0]) {
    goto L_800653A8; // area>=6
  }
  if (c->r[3] == c->r[0]) {
    goto L_800653BC; // area==0
  }
  goto L_800653E4; // area in 1..4: no-op
L_800653A8:
  c->r[2] = c->r[0] + 6u;
  if (c->r[3] == c->r[2]) {
    goto L_800653DC; // area==6
  }
...
void ActorTomba::caseModeFsm_800620D0(uint32_t G) {
  ...
  c->r[31] = 0x800620E8u;
  c->mem_w32(c->r[29] + 20, c->r[17]);
  rec_dispatch(c, 0x80076D68u); // still-substrate leaf FUN_80076D68(G) — un-triaged (ActorTomba::frameTick uses the
                                // same addr as a leaf; here reached as a plain substrate call)
  c->r[3] = (uint32_t)c->mem_r8(c->r[16] + 6u); // G+0x6 (sub-state)
  c->r[17] = c->r[0] + 1u;
  if (c->r[3] == c->r[17]) {
    goto L_80062178; // state 1
  }
  {
    const bool lt2 = ((int32_t)c->r[3] < 2);
    if (!lt2) {
      goto L_80062110; // state >=2
    }
  }
  c->r[4] = c->r[16] + c->r[0];
  if (c->r[3] == c->r[0]) {
    goto L_8006212C; // state 0
  }
  goto L_80062278; // unreachable in practice (state<2, !=0, !=1 impossible for uint8) — kept for fidelity
L_80062110: {
  const uint32_t two = c->r[0] + 2u;
  if (c->r[3] == two) {
    goto L_800621E4; // state 2
  }
}
  {
    const uint32_t three = c->r[0] + 3u;
    if (c->r[3] == three) {
      goto L_80062238; // state 3
    }
  }
...
L_80062178:
  ...
  rec_dispatch(c, 0x80056C00u); // still-substrate leaf(G,1) — un-triaged
  {
    ...
    if (c->r[2] != c->r[0]) {
      c->r[2] = c->r[3] - 1u;
      goto L_800621DC;
    } // timer != 0: decrement, store, done
  }
  c->r[2] = (uint32_t)c->mem_r8(c->r[16] + 41u); // G+0x29
  c->r[4] = c->r[16] + c->r[0];
  if (c->r[2] == c->r[0]) {
    goto L_800621CC; // G+0x29 == 0
  }
...
void ActorTomba::caseModeFsm_80061A7C(uint32_t G) {
  ...
  c->r[5] = c->r[0] + 1u;
  if (c->r[3] == c->r[5]) {
    goto L_80061B04; // state 1
  }
  {
    const bool lt2 = ((int32_t)c->r[3] < 2);
    if (!lt2) {
      goto L_80061AB8; // state >=2
    }
  }
  c->r[4] = c->r[16] + c->r[0];
  if (c->r[3] == c->r[0]) {
    goto L_80061AD4; // state 0
  }
  goto L_80061C54; // unreachable (see caseModeFsm_800620D0)
L_80061AB8: {
  const uint32_t two = c->r[0] + 2u;
  if (c->r[3] == two) {
    goto L_80061B6C; // state 2
  }
}
  {
    const uint32_t three = c->r[0] + 3u;
    if (c->r[3] == three) {
      goto L_80061BAC; // state 3
    }
  }
...
L_80061BE4:
  ...
  if ((int32_t)(int16_t)c->mem_r16(c->r[16] + 66u) == 0) {
    goto L_80061BF8; // v66==0: skip write-back
  }
...
L_80061BF8:
  ...
  c->r[2] = c->r[3] - 1u; // delay-slot value, computed always
  if ((int32_t)(int16_t)c->mem_r16(c->r[16] + 64u) != 0) {
    goto L_80061C50; // v64!=0: store decrement, done
  }
  c->r[2] = (uint32_t)(int32_t)(int16_t)c->mem_r16(c->r[16] + 66u);
  if ((int32_t)(int16_t)c->mem_r16(c->r[16] + 66u) != 0) {
    goto L_80061C54; // v66(current) != 0: no-op
  }
L_80061C18:
  c->r[4] = c->r[16] + c->r[0];
  c->r[5] = c->r[0] + 2u;
...
void ActorTomba::caseModeFsm_80060064(uint32_t G) {
  ...
  c->r[2] = c->r[0] + 1u;
  if (c->r[3] == c->r[0]) {
    goto L_80060098; // G+0x6 == 0
  }
  c->r[4] = c->r[16] + c->r[0];
  if (c->r[3] == c->r[2]) {
    goto L_800601D8; // G+0x6 == 1
  }
...
L_80060098:
  ...
  c->r[4] = c->r[16] + c->r[0];
  if (c->r[17] == c->r[0]) {
    goto L_800600B8; // G+0x7 == 0
  }
  if (c->r[17] == c->r[2]) {
    goto L_80060100; // G+0x7 == 1
  }
...
L_80060100:
  ...
  c->r[4] = c->r[0] + 15u;
  if (c->r[3] != c->r[17]) {
    goto L_8006024C; // result != G+0x7's pre-fetch value: bail to tail
  }
  c->r[5] = c->r[0] + c->r[0];
  c->r[2] = (uint32_t)c->mem_r8(c->r[16] + 6u);
  c->r[6] = c->r[5] + c->r[0];
  ...
  c->r[31] = 0x80060148u;
  c->mem_w8(c->r[16] + 6u, (uint8_t)c->r[2]); // G+0x6++
  rec_dispatch(c, 0x80074590u); // still-substrate leaf: r4 is LEFTOVER=G (not reloaded since the func_80076D68 call
                                // above), r5=0, r6=0 — matches gen exactly; not necessarily the same (cueId,0,0)
                                // signature as the other func_80074590 call sites in this file
  c->r[4] = c->r[0] + 37u;
  c->r[5] = c->r[0] + c->r[0];
  c->r[31] = 0x80060158u;
  c->r[6] = c->r[5] + c->r[0];
...
L_800601D8:
  ...
  rec_dispatch(
      c,
      0x80056B48u); // still-substrate leaf(G,1) — un-triaged (same addr as ActorTomba::velocityIntegrate's guest FUN)
  c->r[31] = 0x80060200u;
  c->r[4] = c->r[16] + c->r[0];
  rec_dispatch(c, 0x80055D5Cu); // still-substrate leaf(G) — un-triaged
  c->r[31] = 0x80060208u;
...
void ActorTomba::caseModeFsm_8006228C(uint32_t G) {
  ...
  c->r[17] = c->r[0] + 1u;                      // s1 = 1 (constant, dead scratch — never restored, matches gen)
  {
    ...
    c->r[2] = (uint32_t)lt2;
    if (c->r[3] == c->r[17]) {
      goto L_80062338; // state == 1
    }
  }
  if (c->r[2] == c->r[0]) {
    goto L_800622C8; // state >= 2
  }
  c->r[4] = c->r[16] + c->r[0];
  if (c->r[3] == c->r[0]) {
    goto L_800622E4; // state == 0
  }
...
void ActorTomba::caseModeFsm_8006506C(uint32_t G) {
  ...
  c->r[18] = c->r[0] + 1u;                       // s2 = 1 (constant)
  {
    ...
    c->r[2] = (uint32_t)lt2;
    if (c->r[17] == c->r[18]) {
      goto L_80065188; // state == 1
    }
  }
  if (c->r[2] == c->r[0]) {
    goto L_800650AC; // state >= 2
  }
  if (c->r[17] == c->r[0]) {
    goto L_800650C8; // state == 0
  }
...
L_800650C8:        // state-0 init (gated)
  ...
  rec_dispatch(c, 0x80054198u); // func_80054198(G) (resetSwap)
  {
    ...
    if (c->r[3] != c->r[2]) {
      goto L_80065138; // (G+0x17C & mask) != 0x200
    }
  }
  c->r[4] = c->r[16] + c->r[0];
  c->r[5] = c->r[0] + 64u;
...
L_80065138:                                       // (G+0x17C & mask) != 0x200 branch
  ...
  c->r[31] = 0x80065158u;
  c->r[6] = c->r[0] + 3u;
  rec_dispatch(c, 0x80054D14u); // func_80054D14(G,24,3) — arg5 set below fallthrough label, but this path recomputes
                                // G+6/G+7 directly
  c->r[2] = c->r[0] + 2u;
