...
struct Tracer {
  ...
  // Key: the host return address captured by backtrace. Simple linear-probed table; cap size for
  // cheap hot-path accounting.
  struct Row {
    void *caller;
    uint64_t a;
    uint64_t b;
  ...
  void bump(Core *c) {
    ...
    void *caller = (n >= 2) ? bt[1] : nullptr;
    for (int i = 0; i < nRows; i++) {
      if (rows[i].caller == caller) {
        if (side) {
          rows[i].b++;
          totalB++;
        } else {
  ...
  static void dumpStatic() {
    instance().dump(stderr);
  }
  void dump(FILE *out) {
    std::fprintf(out,
                 "[rng-calltrace] totalA=%llu totalB=%llu deltaA-B=%lld  (unique ra=%d)\n",
                 (unsigned long long)totalA,
                 (unsigned long long)totalB,
                 (long long)((long long)totalA - (long long)totalB),
                 nRows);
    ...
    void *addrs[32];
    int nres = 0;
    for (int i = 0; i < nRows && nres < 32; i++, nres++) {
      addrs[nres] = rows[idx[i]].caller;
    }
