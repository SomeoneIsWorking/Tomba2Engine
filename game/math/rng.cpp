...
// MULT product. Both operands are treated as 32-bit signed (guest instruction path: MULT after mov s0,a1/s1,a0 —
...
int32_t Rng::inRange(int32_t lo, int32_t hi) {
  ...
  int32_t span = hi - lo; // caller expected: hi >= lo (guest instruction path doesn't check)
