...
// object ptr is *0x1f800138 (== task-0 obj 0x801fe000); its state fields:
//   sm[0x48] top state, sm[0x4a] running sub-mode, sm[0x4c] area machine,
//   sm[0x5c] intro timer.
...
#include "cd/libcd_native.h" // class LibcdNative — native_sync=false faithful libcd (SBS byte-exact gate)
#include "cfg.h"
#include "core.h"
#include "game_ctx.h"
...
// 0x801082B8 menu draw) + the L_801084CC/L_801084D0 common epilogue tail
// (mem_w8 0x1F800232=0; mem_w8 0x800BF81E &= 2) that every branch falls through
// to and the native_sync shortcut was missing entirely. The fade leaf
// dispatches to substrate 0x8007E9C8 (byte-exact packet-pool/ scratchpad/OT
// writes -- same pattern as Sop::fieldModeFaithful) instead of the
// host-state-only ScreenFade::set() the native_sync=true path uses. Other pages
// delegate whole to substrate (own frame
// + dispatch table).
void Engine::submitPage810cFaithful() {
  Core *c = core;
  ...
  if (task && c->mem_r8(task + 0x6Bu) == 1) { // page==1 -> table[1]==0x8010829C (docs/engine_re.md)
    ...
    c->mem_w32(sp + 16,
               c->r[16]); // s0 @ sp+16 (unused this branch, spilled anyway)
    c->r[4] = 0x00808080u;
    c->r[5] = 0;
    c->r[6] = 4;
    c->r[31] = 0x801082B0u;
...
// (Engine::fieldTransition + workers are defined in this TU; forward declared
// via engine.h.)
...
// (0x80024e00, 0x80040aa4, 0x80074bf8, 0x80024f18, 0x800251f0, 0x80077b5c — all
...
void Engine::sceneEventFifoFaithful() {
  Core *c = core;
  c->r[29] -= 32;
  const uint32_t sp = c->r[29];
  ...
  // converge on the same guest leaves this method also calls, e.g.
  // Animation::advanceLinkChain/0x80077b5c below) — this is the reference
  // technique for telling "genuine control-flow bug" apart from "expected
...
void Engine::fieldTransitionFaithful() {
  Core *c = core;
  c->r[29] -= 24;
  const uint32_t sp = c->r[29];
  ...
  if (s4c < 10) {
    ...
    case 4:
      c->r[31] = 0x80108ACCu;
      eng(c).transitionMainFaithful();
      break;
    case 5:
    case 6:
      c->r[31] = 0x80108AECu;
      eng(c).transitionD3cFaithful();
      break;
    case 7:
      c->r[31] = 0x80108ADCu;
      eng(c).transitionE20Faithful();
      break;
    case 8:
      c->r[31] = 0x80108AFCu;
      eng(c).transitionF3cFaithful();
      break;
    default:
      break;
    }
...
void Engine::transitionMainFaithful() {
  ...
  if (st < 5) {
    ...
    case 0:
      ...
      c->mem_w16(sm + 0x4e, (uint16_t)(c->mem_r16(sm + 0x4e) + 1));
      fade(c).applyLeafCall(0xffffffu,
                            0); // = guest FUN_8007e9c8(0xffffff,0,4); jal-site was 0x80107B98u
      sm = c->mem_r32(0x1f800138u);
      c->r[4] = 0x800452C0u;
    ...
    case 1: {
      ...
      fade(c).applyLeafCall((u << 19) | (u << 11) | (u << 3),
                            0); // jal-site was 0x80107BECu
      sm = c->mem_r32(0x1f800138u);
      uint8_t v = (uint8_t)(c->mem_r8(sm + 0x6b) - 1);
      c->mem_w8(sm + 0x6b, v);
      if (v == 0) {
...
// 0x80044BD4 spawn-and-wait (override registry -> PcScheduler::spawnAndWait) of
// the area-DATA loader 0x800452C0 (Asset::areaDataLoadAsTask, task-1 fiber) —
// the stage fiber parks inside the wait loop and falls through to case 1 on
// completion, organically reproducing the substrate's multi-tick cadence
// (retires the Slip #3 two-tick defer + Slip #5 RNG compensation from the
// faithful path).
void Engine::submode1Faithful() {
  Core *c = core;
  uint32_t sm = c->mem_r32(0x1f800138u);
  c->r[29] -= 24;
  const uint32_t sp = c->r[29];
...
// ===== Stage-0/START.BIN task-switch + preload state machine (moved from
// native_boot.cpp, 2026-07 restructure). Same stage-machine-orchestration
// domain this file already owns for the GAME stage; this cluster owns task-0's
// stage SWITCH (native_start_stage/native_load_overlay), the START.BIN
// file-table builder (ov_start_bin_stage), and the stage-0 preload SM
// (native_stage0_sm), which hands off to native_start_stage(c,1) to enter DEMO.
...
// native_stage0_sm (the boot-time inline preload), and by stage0Advance's final
// step (native scheduler tick that enters DEMO). Was `native_start_stage`
...
// Read a NUL-terminated guest string into `out` (bounded). Used to pull the
// START.BIN filename tables (which live in the loaded overlay) for native
// resolution.
static void read_guest_str(Core *c, uint32_t addr, char *out, int cap) {
  int k = 0;
  for (; k < cap - 1; k++) {
    char ch = (char)c->mem_r8(addr + k);
    out[k] = ch;
    if (!ch) {
      break;
    }
...
// preload_texgroup / preload_cel / preload_build_vram / preload_stage1 (the
// stage-0/stage-1 area/asset PRELOAD chain) live in game/core/asset.cpp
// (2026-07 restructure) — same asset-loading domain as
...
// coro cadence.
//
// This static wrapper stays for the sole remaining inline path
// (native_task0_bootstrap in the early boot init — before the scheduler is
// spinning); the scheduler path now uses stage0Advance.
...
// Stage-0 START.BIN entry (0x8010649C): own the file-table BUILDER PC-native.
// Original substrate is a sequence of CdSearchFile loops that resolve ~36 disc
// filenames and record each {LBA,size}; we bypass libcd entirely and walk the
// native ISO9660 directory with disc_find_file, then seed the preload SM at
// state 0 so stage0Advance can pace preloadTexgroup / preloadStage1 /
// startStage(1) across subsequent ticks. Reference: overlay disas of
// 0x8010649c..0x80106728 (later-211).
//
// The three filename tables and one XA-singleton table below are baked into the
// START.BIN overlay (0x8010649C body); their addresses and shapes are fixed by
// the game.
struct StartBinLoop {
  uint32_t name_table, dest_table, count;
};
...
static bool resolve_via_iso9660(Core *c, uint32_t name_ptr, uint32_t *lba, uint32_t *size) {
  char name[80];
  read_guest_str(c, name_ptr, name, sizeof name);
...
// libcd directory cache — native port of FUN_8008BBE8 (CdNewMedia) +
// FUN_8008BF50 (CdCacheFile), the two libcd helpers that populate the game's
// in-memory directory cache. The native_sync path uses native ISO9660
// (resolve_via_iso9660 above) for file lookup — fast, doesn't touch libcd — but
// the substrate on B populates the libcd cache as a side effect of
// CdSearchFile. To keep A byte- identical, reproduce the writes here from the
// same ISO9660 sectors, using native disc_read_sector.
//
// Guest state populated:
//   0x800AC2D4  u32           currently cached dir index (0 = root loaded, else
//   path-table 1-based idx) 0x80102D68  128 × 44 B    path-table cache (index,
//   parent, extent, name[32]) 0x80102768   64 × 24 B    file-entry cache for
//   the currently-cached dir (MSF, size, name)
//
// RE: Ghidra headless decomp of FUN_8008BBE8 / FUN_8008BF50 (SCEI library range
// 0x80080000..0x8009E000). ISO9660 field offsets are the standard PVD /
// path-table-L / dir-record layout.
...
// Read one 2048 B disc sector into a local buffer AND into the guest-RAM
// scratch at 0x80104368, mirroring the substrate's per-sector read into
// DAT_80104368. Returns false on read failure.
static bool cdlibcd_read_into_scratch(Core *c, uint32_t lba, uint8_t *out) {
  if (!disc_read_sector(&c->game->disc, lba, out)) {
    return false;
  }
  for (uint32_t i = 0; i < 2048; i++) {
    c->mem_w8(kGuestSectorScratch + i, out[i]);
  }
  return true;
}

// FUN_8008BBE8 CdNewMedia: read PVD at LBA 16, follow to the L-path-table, walk
// entries into the path-table cache. Sets 0x800AC2D4 = 0.
static void cdlibcd_new_media(Core *c) {
  uint8_t sec[2048];
  if (!cdlibcd_read_into_scratch(c, 16, sec)) {
    return;
  }
  ...
  const uint32_t pt_lba =
      (uint32_t)sec[0x8C] | ((uint32_t)sec[0x8D] << 8) | ((uint32_t)sec[0x8E] << 16) | ((uint32_t)sec[0x8F] << 24);
  if (!cdlibcd_read_into_scratch(c, pt_lba, sec)) {
    return;
  }

  // Walk path-table entries: byte name_len + byte xa_len + u32 extent LE + u16
  // parent LE + name[name_len] + pad-to-even.
  uint32_t off = 0;
  int i = 0;
  while (off + 8 <= 2048 && sec[off] != 0 && i < 128) {
    const uint8_t name_len = sec[off + 0];
    const uint32_t extent = (uint32_t)sec[off + 2] | ((uint32_t)sec[off + 3] << 8) | ((uint32_t)sec[off + 4] << 16) |
                            ((uint32_t)sec[off + 5] << 24);
    ...
    c->mem_w32(slot + 4,
               (uint32_t)parent); // parent idx as u32 (only low byte meaningful)
    c->mem_w32(slot + 8, extent); // dir extent LBA
    for (uint32_t j = 0; j < name_len; j++) {
      c->mem_w8(slot + 12 + j, sec[off + 8 + j]);
    }
  ...
  // Sentinel: next slot's parent field cleared (loop exit marker for the
  // original FUN_8008BBE8).
  if (i < 128) {
    c->mem_w32(kGuestPathTableCache + (uint32_t)i * 44u + 4, 0);
  }

  c->mem_w32(kGuestCurCachedDir, 0);
}

// FUN_8008BF50 CdCacheFile(dir_idx): read the directory sector at
// path_cache[dir_idx-1].extent, walk directory records, populate the file-entry
// cache. Sets 0x800AC2D4 = dir_idx. Idempotent when dir_idx already matches the
// cached one.
static void cdlibcd_cache_file(Core *c, uint32_t dir_idx) {
  if (dir_idx == c->mem_r32(kGuestCurCachedDir)) {
    return;
  }
  ...
  while (off < 2048 && sec[off] != 0 && i < 64) {
    ...
    const uint32_t extent = (uint32_t)sec[off + 2] | ((uint32_t)sec[off + 3] << 8) | ((uint32_t)sec[off + 4] << 16) |
                            ((uint32_t)sec[off + 5] << 24);
    const uint32_t size = (uint32_t)sec[off + 10] | ((uint32_t)sec[off + 11] << 8) | ((uint32_t)sec[off + 12] << 16) |
                          ((uint32_t)sec[off + 13] << 24);
    const uint8_t fn_len = sec[off + 32];

    // FUN_8008A00C: LBA -> BCD MSF (sector = LBA + 150, then split into
    // min/sec/frame at 75 fps).
    ...
    if (i == 0) {
      ...
      // ".." — DAT_8001C52C = 0x00002E2E at slot+8, DAT_8001C52E = 0x0000 (u16)
      // at slot+10.
      c->mem_w32(slot + 8, 0x00002E2Eu);
      c->mem_w16(slot + 10, 0);
    } else {
      for (uint32_t j = 0; j < fn_len; j++) {
        c->mem_w8(slot + 8 + j, sec[off + 33 + j]);
      }
  ...
  // Sentinel: null the next name slot's first byte (original:
  // (u8)(&DAT_80102770 + iVar3*0xc) = 0).
  if (i < 64) {
    c->mem_w8(kGuestFileEntryCache + (uint32_t)i * 24u + 8, 0);
  }

  c->mem_w32(kGuestCurCachedDir, dir_idx);
}

// Baked pixel data address inside START.BIN (16x1 pixel strip loaded to
// VRAM(944,256)).
static constexpr uint32_t kStartBinLoadImageSrc = 0x80106878u;

// Advance stage-0 SM + preload boot texgroup. native_sync only — the
// pc_faithful path splits these writes across their real substrate call sites
// (task-1 spawn for preload, FUN_80044BD4 tail for sm[0x48]:=1, FUN_80044BD4
// body for RNG stamp).
static void startBinCommonAdvance(Core *c, Asset &asset) {
  uint32_t task = c->mem_r32(CUR_TASK);
  c->mem_w16(task + 0x4a, 0);
  asset.preloadTexgroup(0, 0);
  c->game->pcSched.completeSyncWait(task, 0);
  c->mem_w16(task + 0x48, 1);
}

// One-line dispatchers — the native_sync/faithful fork lives at method
// granularity so each path reads top-to-bottom without inline branching. User
...
void Engine::startBinStageNative() {
  Core *c = core;
  ...
  startBinCommonAdvance(c, asset);
  const uint32_t task = c->mem_r32(CUR_TASK);
...
// CdlFILE records at sp+16+i*24, XA CdlFILE at sp+408), s-registers maintained
// live through the loops (nested callee prologues spill them into compared
...
// stage-swap leaf FUN_80052078, which parks the fiber forever; the stanza
// cancels it once the entry rewrites. NOTE fiber teardown unwinds via longjmp —
// no destructibles may live across a yield here.
namespace {
struct StartBinFaithfulLoop {
  uint32_t names, dest, count;
  uint32_t raSearch, raPosToInt,
      raPrintf; // guest call-site ra constants (part of the port's identity)
};
...
void Engine::startBinStageFaithful() {
  ...
  // Prologue: sp -= 456; spill ra/s4..s0 at 452..432 (live values — at fiber
  // start these are the frame-loop registers + the 0xDEAD0000 entry ra, same as
  // core B's fiber).
  c->r[29] -= 456;
  ...
  // LoadImage RECT {944,256,16,1} is a frame local at sp+400 (NOT scratchpad —
  // that's the native_sync shortcut's location).
  c->mem_w16(S + 400, 944);
  c->mem_w16(S + 402, 256);
  c->mem_w16(S + 404, 16);
  c->mem_w16(S + 406, 1);
  ...
  // because CdSearchFile's prologue spills them into its frame — compared
  // task-0 stack bytes.
  LibcdNative libcd(c);
  ...
  // SM loop (L_80106744): the state constants live in the s-regs —
  // spawnAndWait's prologue spills them (s0=3, s1=2, s2=1, s3=r19 left at 5 by
  // loop 3's exit — the RE'd live values).
  c->r[16] = 3;
  c->r[17] = 2;
  c->r[18] = 1;
  uint32_t task = c->mem_r32(CUR_TASK);
  ...
  for (;;) {
    ...
    case 0: // L_8010678C: spawn task-1 texgroup preload
      ...
      c->r[7] = 0;
      c->r[31] = 0x801067A8u;
      sched.spawnAndWait(0x80044F58u, 0, 0,
                         0); // parks here once per wait frame
      break;
