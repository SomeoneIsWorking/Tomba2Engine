...
void Asset::unpackGroup(uint32_t tablePtr, uint32_t anchorEnd) {
  ...
  if (dbg) {
    cfg_logi("unpack", "table=0x%08X count=%d src0=0x%08X ra=0x%08X", tablePtr, count, src, c->r[31]);
  }
  ...
  // unpacker reads it, sequence-numbered, so it can be checked against the disc / oracle exactly.
  {
    ...
    if (dd) {
      static int seq = 0;
      char p[300];
      snprintf(p, sizeof p, "%s/unpack_%03d_c%d.bin", dd, seq++, count);
      FILE *uf = fopen(p, "wb");
      if (uf) {
  ...
  for (int32_t i = 0; i < count; i++) {
    ...
    if (dbg) {
      cfg_logi("unpack",
               " e%d dst=(%d,%d) %dx%d src=0x%08X len=%u srcbytes: %02X %02X %02X %02X",
               i,
               c->mem_r16s(desc),
               c->mem_r16s(desc + 2),
               stride,
...
void Asset::unpackGroupFaithful(uint32_t tablePtr, uint32_t anchorEnd) {
  ...
  c->r[18] = tablePtr;
  c->mem_w32(sp + 40, c->r[22]);
  c->r[22] = anchorEnd;
  c->mem_w32(sp + 36, c->r[21]);
  c->r[21] = tablePtr + 2048; // packed source follows the table
...
void Asset::uploadImage(uint32_t desc, uint32_t src) {
  ...
  if (w > 0 && h > 0) {
    gpu_native_load_image(c, x, y, w, h, src);
  }
}
