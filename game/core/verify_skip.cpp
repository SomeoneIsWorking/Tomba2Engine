...
void VerifyHarness::skipCheck(
    uint32_t addr, void (*skipFn)(void *), void *skipCtx, void (*oracleFn)(void *), void *oracleCtx) {
  Core *c = core;
  if (!mStrictPreRam) {
  ...
  if (!mSkipSpuA) {
    mSkipSpuA = (uint8_t *)malloc(524288);
    mSkipSpuB = (uint8_t *)malloc(524288);
    mSkipSpuPre = (uint8_t *)malloc(524288);
  }
  ...
  preRegs[33] = c->lo;
  c->game->spu.bind(c);
  SPU_PeekRAM(mSkipSpuPre);
  // leg 1: the SKIP shortcut
  skipFn(skipCtx);
  memcpy(mStrictNatRam, c->ram, 0x200000);
  uint8_t skipSpad[0x400];
  memcpy(skipSpad, c->scratch, 0x400);
  uint32_t skipRegs[34];
  memcpy(skipRegs, c->r, 32 * 4);
  skipRegs[32] = c->hi;
  skipRegs[33] = c->lo;
  c->game->spu.bind(c);
  SPU_PeekRAM(mSkipSpuA);
  // rewind (guest + SPU)
  ...
  inSubstrateLeg = true;
  bool inCheck_save = inCheck;
  inCheck = false; // allow the oracle leg's no-op yields (guard is for MV legs)
  oracleFn(oracleCtx);
  inCheck = inCheck_save;
  inSubstrateLeg = false;
  c->game->pcSched.in_stage = in_stage_save;
  c->game->spu.bind(c);
  SPU_PeekRAM(mSkipSpuB);
  ...
  for (int i = 0; i < kNObsRegions; i++) {
    ...
    for (uint32_t a = R.lo; a < R.hi && bad < 16; a++) {
      ...
      if (vs != vo) {
        cfg_logi("skip-verify", "0x%08X MISMATCH [%s] @0x%08X: skip=%02X oracle=%02X", addr, R.label, a, vs, vo);
        bad++;
      }
    }
  }
    ...
    if (po == ps && po && (po & 0x1FFFFFFFu) < 0x1FFE00u) {
      for (uint32_t o = 0; o < 0x200 && bad < 16; o++) {
        if (mStrictNatRam[(po & 0x1FFFFFu) + o] != c->ram[(po & 0x1FFFFFu) + o]) {
          cfg_logi("skip-verify",
                   "0x%08X MISMATCH [area_fx_deref] @0x%08X: skip=%02X oracle=%02X",
                   addr,
                   po + o,
                   mStrictNatRam[(po & 0x1FFFFFu) + o],
                   c->ram[(po & 0x1FFFFFu) + o]);
          bad++;
        }
      }
    }
  ...
  if (memcmp(mSkipSpuA, mSkipSpuB, 524288) != 0) {
    for (uint32_t o = 0; o < 524288 && bad < 24; o++) {
      if (mSkipSpuA[o] != mSkipSpuB[o]) {
        cfg_logi("skip-verify",
                 "0x%08X MISMATCH [SPU RAM] @0x%05X: skip=%02X oracle=%02X",
                 addr,
                 o,
                 mSkipSpuA[o],
                 mSkipSpuB[o]);
  ...
  if (bad) {
    cfg_loge("skip-verify",
             "0x%08X FAILED (%d+ observable diffs) — skip leg does NOT produce the oracle's observable output.",
             addr,
             bad);
    abort();
  }
  ...
  memcpy(c->scratch, skipSpad, 0x400);
  memcpy(c->r, skipRegs, 32 * 4);
  c->hi = skipRegs[32];
  c->lo = skipRegs[33];
  c->game->spu.bind(c);
  SPU_PokeRAM(mSkipSpuA);
  inCheck = false;
  Check &k = check("skip-verify");
  if (++k.nMatch % 16 == 1) {
    cfg_logi("skip-verify", "0x%08X OK (pass #%ld)", addr, k.nMatch);
  }
}
