...
//
// Each hook is a thin function-pointer impl that reaches the game's per-Core
// Engine aggregate
// (`eng(c).*`). The framework substrate (runtime/recomp/*) calls these as
// `c->hooks->fn(c)` in place of the direct `eng(c).X()` calls it used to bake
// in, so the framework no longer names any game type. Installed alongside the
// GameConfig via tomba_install_game_config() (game_config.cpp), before any
// Game/Core is constructed, so Core's ctor snapshots a non-null c->hooks.
#include "cfg.h"
#include "core.h"
#include "engine.h"
...
// tomba_bootInit — the game's boot-init prologue, moved VERBATIM out of
// native_boot.cpp game_init. This is the transcription of FUN_80050b08's init
// prefix (no scheduler loop): rc-dispatched guest leaves interleaved with the
// native eng(c).* init calls, in the exact guest order. It moves WHOLE (not
// just the engine calls) because the engine calls are interleaved with the rc
// leaves and the trailing task0Bootstrap depends on the scheduler-table init
// (0x80051e00 / 0x80051f14) + the 0x1f800138 cur-task write that sit between
...
static void tomba_bootInit(Core *c) {
  ...
  rc0(c, 0x80089788);
  rc0(c, 0x80085b20);
  // CD init: native HLE, NOT the recomp libcd (FUN_800898a0). The recomp CdInit
  // busy-waits on the CD-controller reset handshake (no IRQ ever acks → 5
  // retries → "CD timeout" → "Init failed"). We model no CD controller; all CD
  // ops are native synchronous (cd_override.cpp). (was rc0 0x800898a0)
  c->game->cd.hleInit();
  rc1(c, 0x80080bf0, 3);
  rc1(c, 0x80080d64, 0);
  rc1(c, 0x80080ed4, 1);
  rc1(c, 0x800865f0, 0);
  // Engine frame-state + camera init reimplemented PC-native
  // (game/scene/startup.cpp), replacing the 1:1 rec_dispatch transcription.
  // FUN_800509b4 (display/GTE projection + PSX draw/disp double-buffer env)
  // stays dispatched for now — it sets DAT_801003f8 = H that eng_init_camera
  // reads, and entangles the PSX-GPU env, so it is the next target. (later-159,
  // top-down engine port from main.)
  eng(c).initFrameState(); // was rc0(c, 0x80050a0c)
  eng(c).initDisplay();    // was rc0(c, 0x800509b4) — GTE projection + display
                           // (sets H=DAT_801003f8)
  ...
  rc1(c, 0x800993a0, 1);
  // FUN_80089bac(cmd=0xe Setmode, &mode=0x80, 0) — CdControlB. The recomp body
  // busy-waits in CD_cw on the controller ack (no IRQ -> "CdlSetmode timeout").
  // We model no CD drive mode (every read is by LBA, served natively), so
  // Setmode is a native no-op. (was rc3 0x80089bac) (removed: VSync(3)
  // display-settle wait — the PC-native frame loop owns ALL timing; boot does
  // not call libetc VSync. Any code that reaches VSync now TRAPS
  // (sync_overrides.cpp vsync_trap).) FUN_80075130 font/text init reimplemented
  // PC-native (game/ui/font.cpp): owns the orchestration + direct writes + the
  // 3 engine-state callees (FUN_800963a0/80096370/800752b4); the 8 libgpu/sound
  // callees stay rec_dispatched in-context (later-182b nested-dispatch risk).
  // Replaces the rc0 transcription.
  ...
  // VSyncCallback(LAB_800506b4): native no-op — we deliver no preemptive VBlank
  // IRQ (the per-vblank callback's unmodeled interrupt-vector deref is
  // skipped). (was rc1 0x80085bb0)
  c->game->timing.vsyncCallback(); // callback ptr arg unused (no preemptive
                                   // vblank IRQ delivered)

  cfg_logi("native_boot", "init prefix complete");

  // --- task 0 initial entry: FUN_800499e8 resolves \BIN\START.BIN and
  // FUN_80052078(0) loads the stage-0 overlay to 0x80106228 + restarts task 0
  // at stage 0 (0x8010649c). It yields once (FUN_80051f80, a no-op with threads
  // stubbed) so it runs straight to completion here. The scheduler's "current
  // task" ptr DAT_1f800138 is normally set by FUN_80051e60; set it to task0 so
  // FUN_80052078/FUN_800450bc operate on task 0. ---
  c->mem_w32(0x1f800138, 0x801fe000);
  eng(c).task0Bootstrap(); // PC-native: was rc0(c, 0x800499e8) — CD subtree
                           // owned top-down
  // START.BIN loaded raw to 0x80106228: [0]=manifest count (6); entry word
  // @0x8010649c.
  cfg_logi("native_boot",
           "after FUN_800499e8: START.BIN count@0x80106228=%u "
           "entry-word@0x8010649c=0x%08X (expect 0x27BDFE38); task0 state=%u "
           "entry=0x%08X",
...
// registerOverrides installs ALL the game's override clusters into the
// process-global registry. Body lives in register_overrides.cpp (moved out of
// framework boot.cpp); declared here so the hook table names it. Takes Game*
// (not Core*): the clusters register per-Game.
extern void register_engine_overrides(Game *);

// TombaCtx lifecycle — the game's per-Core subsystem aggregate alloc/free
// (game_ctx.cpp). The framework calls these through the ctxCreate/ctxDestroy
// hooks to fill/free Core::gameCtx.
extern void *tomba_ctx_create(Core *);
extern void tomba_ctx_destroy(void *);

// extern-visible: game_config.cpp names it in the install call. A
// namespace-scope `const` object has INTERNAL linkage by default in C++, so
// `extern` is required to export the symbol. One process-global instance; both
// SBS cores snapshot the same pointer.
