...
L_80075C50:;
  ...
  {
    int _t = (c->r[3] == c->r[2]);
    c->mem_w32((c->r[21] + (uint32_t)0), c->r[0]);
    if (_t) {
      goto L_80075CC8;
    }
  }
  ...
  c->r[31] = 0x80075CB8u;
  c->mem_w16((c->r[17] + (uint32_t)-4736), (uint16_t)c->r[0]);
  func_80074BF8(c);
  c->mem_w8((c->r[16] + (uint32_t)50), (uint8_t)c->r[0]);
  goto L_80075CC8;
L_80075CC0:;
  c->r[31] = 0x80075CC8u;
  func_80074E48(c);
...
namespace {
void ov_fieldSeqSchedulerTick(Core *c) {
  eng(c).fieldSeqSchedulerTick();
}
} // namespace
void Engine::registerFieldSeqSchedulerTick() {
  overrides::install(
      0x80075A80u, "Engine::fieldSeqSchedulerTick", ov_fieldSeqSchedulerTick, gen_func_80075A80, shard_set_override);
}
