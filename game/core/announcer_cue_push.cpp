...
L_8004FA80:;
  ...
  c->mem_w32((c->r[16] + (uint32_t)0), c->r[19]);
  goto L_8004FAEC;
L_8004FAB8:;
  c->r[4] = c->r[19] + c->r[0];
...
namespace {
void ov_announcerCuePush(Core *c) {
  eng(c).announcerCuePush();
}
} // namespace
void Engine::registerAnnouncerCuePush() {
  overrides::install(
      0x8004FA38u, "Engine::announcerCuePush", ov_announcerCuePush, gen_func_8004FA38, shard_set_override);
}
