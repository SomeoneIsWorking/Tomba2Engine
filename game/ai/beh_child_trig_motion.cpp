  ...
  c->mem_w8(nd + 11, 0);
  {
    ...
    if (c->mem_r8(nd + 8) != 0) {
      ...
      do {
        ...
        c->mem_w16(rec + 0, c->mem_r16(src));
        src += 2;
        c->mem_w16(rec + 2, c->mem_r16(src));
        src += 2;
...
S1: {
  ...
  if ((uint32_t)(sp - 23) >= 9) {
    goto Lret; // sp not in [23,31]
  }
  if (leafr(c, nd, 0x8007778Cu) == 0) {
    goto Lret; // FUN_8007778C(node)
  }
  uint8_t n3 = c->mem_r8(nd + 3);
  if (n3 != 0) {
    if ((int32_t)(int8_t)n3 < 0) {
      goto L8c00; // bltz (never true for lbu)
    }
    if (n3 >= 3) {
      goto L8c00;
    }
  ...
  if (c->mem_r8(nd + 0x29) == 0) {
    goto L8c00; // node[0x29]
  }
  {
    int32_t ang;
    if (c->mem_r8(0x800E7FC7u) & 1) {
      ang = -c->mem_r16s(0x800E7ED8u);
    } else {
      ang = c->mem_r16s(0x800E7ED8u);
    }
