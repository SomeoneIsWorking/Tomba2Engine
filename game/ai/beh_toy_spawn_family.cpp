...
// (`authenticated executable/overlay evidence{0,1}.c`, symbols `A00 overlay the cited guest instructions`) — the ground
// truth for this overlay. 3 real bugs found and fixed (see the inline "BUG FIX" comments at each function):
...
// the SAME byte the OR below writes — not a separate field). Wired via `tomba::native::declareOverride` passing
// `image-qualified A00 native registration` as the setter (the only real callers are DIRECT `A00 overlay the cited
// guest address(c)` sites inside ov_a00_shard_1.c), which also makes it reachable via typed runtime address dispatch
// (native-caller tracing) — same shape as game/ai/actor_melee_engage.cpp. Field-role names beyond what's RE'd here
// (e.g. exact semantics of GBASE's individual bytes) still await a live RAM dump — see docs/engine_re.md.
//
...
static constexpr GuestFrameSpill kSpills_80127720[3] = {
    {17, 20},
    {31 /*ra*/, 24},
    {16, 16},
};
static constexpr GuestFrameSpill kSpills_8012763C[3] = {
    {17, 20},
    {31 /*ra*/, 24},
    {16, 16},
};
static constexpr GuestFrameSpill kSpills_80127510[5] = {
    {19, 28},
    {17, 20},
    {18, 24},
    {31 /*ra*/, 32},
...
inline void leaf1(Core *c, uint32_t a0, uint32_t fn) {
  c->r[4] = a0;
...
//                            function (guest 0x8004D604) indexes GBASE+a0 as a flag-byte array,
...
uint32_t beh_spawn_toy_child_type5_80127720(Core *c, uint32_t owner) {
  GuestFrame<32, 3> frame(c, kSpills_80127720);
...
//     reloads r4/a0 between the two calls, so a0 for this call is whatever guest 0x80072DDC left
...
uint32_t beh_spawn_toy_child_type4_8012763c(Core *c, uint32_t owner) {
  GuestFrame<32, 3> frame(c, kSpills_8012763C);
  const uint32_t child = leaf4Ret(c, owner, 0u, 2u, 20u, 0x80072DDCu);
  if (child == 0) {
    return 0;
  }
  ...
  // the same typed runtime address dispatch, which mutates the same shared Core register file).
  c->r[5] = 1u;
  psx::cpu::dispatchGuestToReturn0(*c, 0x8004D650u, psx::cpu::ExecutionBudget::currentTurn(*c), __func__);
  c->mem_w32(child + 0x1Cu, CHILD_TABLE_PTR);
...
uint32_t beh_spawn_toy_child_type2_80127510(Core *c, uint32_t owner, uint32_t subtype) {
  GuestFrame<40, 5> frame(c, kSpills_80127510);
  const uint32_t child = leaf4Ret(c, owner, 0u, 2u, 20u, 0x80072DDCu);
  if (child == 0) {
    return 0;
  }
...
// ---------------------------------------------------------------------------------------------
// Wiring: the only real callers found for all 5 addresses are DIRECT `A00 overlay the cited guest address(c)` sites
// inside ov_a00_shard_1.c (the surrounding cluster's top-level state dispatcher, not yet drafted —
// see the file banner). That call shape goes through the recorded binary evidence's OWN per-overlay
// image-qualified runtime dispatcher table, never typed runtime address dispatch — so installing without a setter would
// be blind to it. `tomba::native::declareOverride` is passed image-qualified A00 native registration as the setter,
// oracle-gated same as gen/native everywhere else, same pattern as game/ai/actor_melee_engage.cpp.
// ---------------------------------------------------------------------------------------------
