...
void beh_lift_platform(Core *c) {
  ...
  if (c->mem_r8(0x800bf89cu) == 2 || c->mem_r8(0x800e7eaau) != 1) {
    ...
    if (c->mem_r8(nd + 1) == 0) {
      if (c->mem_r8(s0 + 1) != 0) {
        c->mem_w8(nd + 1, 1);
        eng(c).cull.enqueueVisibleClass4(nd);
      } // FUN_80077EBC — Cull::enqueueVisibleClass4
      if (c->mem_r8(nd + 1) == 0) {
        goto L6c4; // L508 re-check
      }
    }
  } else {
    if (c->mem_r8(nd + 1) == 0) {
...
L6c4: {
  ...
  for (int i = 0; i < (int)n8; i++) {
    c->mem_w8(c->mem_r32(base + 0xc0) + 0x3f, n1);
    base += 4;
  }
  c->mem_w8(nd + 0x29, 0);
