...
// resident siblings (the FUN_739ac handler / the FUN_73cd8 handler / the FUN_8012eb54 handler): a state machine on the
// node's state byte node[4] (0 init / 1 active / 2 idle / 3 despawn). State 0 seeds box/size via FUN_80051b70 then
...
inline void tail_set1_and_render(Core *c, uint32_t obj) { // 0x8012d844 / 0x8012d848 / 0x8012d84c
  ...
  c->r[4] = obj;
  eng(c).graphicsBind.renderUpdate(); // 8012D84C jal 0x800517f8 ; 8012D850 move a0,s0
}
...
inline void step_node18(Core *c, uint32_t obj) {    // 0x8012d7d8 / inline 0x8012d6ac
  ...
  if ((int8_t)v <= 0) {
    despawn_flag_block(c, obj); // 8012D7F4/6C8 bgtz; else despawn_flag_block
  }
...
inline void advance_node32(Core *c, uint32_t obj) {    // 0x8012d78c
  ...
  if ((int16_t)v < -0x64e) {
    tail_set1_and_render(c, obj);
    return;
  } // 8012D7A8 bnez -> 0x8012d848
...
inline void gate_then_flag(Core *c, uint32_t obj, uint32_t thr) {
  ...
  if (p < thr) {
    return; // 8012D8D8 bnez -> 0x8012d8f4 epilogue
  }
...
void beh_jumptable_flag_gate(Core *c) {
  ...
  if (st != 1) {   // 8012D50C beq v1,s1(=1) -> state 1
    if (st >= 2) { // 8012D514 slti v0,v1,2 ; 8012D518 beqz -> 0x8012d530
      if (st == 2) {
        return; // 8012D534 beq v1,2 -> 0x8012d8f4 (epilogue)
      }
    ...
    if (st != 0) {
      return; // 8012D520 beqz v1 -> state 0; else 8012D528 j epilogue
    }
    ...
    c->r[4] = obj;
    c->r[5] = 0xc;
    c->r[6] = (uint32_t)n3 + 0x48;    // 8012D550 a1=0xc ; 8012D558 a2=node[3]+0x48
    eng(c).graphicsBind.recordInit(); // 8012D554 jal 0x80051b70
    if (c->r[2] != 0) {
      return; // 8012D55C bnez v0 -> 0x8012d8f4 (init busy -> epilogue)
    }
    ...
    if (n3 >= 2) {             // 8012D578 beqz -> 0x8012d5a4
      ...
      c->r[4] = obj;
      eng(c).graphicsBind.renderUpdate(); // 8012D84C jal 0x800517f8 (via j 0x8012d84c)
    ...
    c->r[4] = obj;
    eng(c).graphicsBind.renderUpdate(); // 8012D84C jal 0x800517f8
    return;                             // 8012D854 j epilogue
  }
  ...
  if (p0 < 0x16) {
    return; // 8012D5C0 bnez(p0<0x16) -> 0x8012d8f4 epilogue
  }
  if (p0 == 0x16) {                         // 8012D5C8 bne a0,0x16 -> 0x8012d5e4 (skip when !=)
    ...
    if (cnt < 0x2a30) {
      return; // 8012D5DC bnez(cnt<0x2a30) -> 0x8012d8f4 epilogue
    }
  }
  ...
  if (n3 >= 6) {
    return; // 8012D608 beqz(sltiu n3,6) -> 0x8012d8f4 epilogue
  }
  ...
  case 0: {                          // jt[0] = 0x8012d62c
    ...
    if (s5 == 0) {                   // 8012D634 beqz -> 0x8012d64c (delay v0=1)
      ...
      // 0x8012d678:
      if (c->mem_r8(0x1F800207u) < 0x17) {
        return; // 8012D684 bnez(p<0x17) -> 0x8012d8f4 epilogue
      }
      c->mem_w8(obj + 1, 1); // 8012D690 sb v0(=1),1(s0)  (j 0x8012d8f4 delay slot)
      return;                // 8012D68C j epilogue
    }
    if (s5 != 1) {
      return; // 8012D63C beq v1,1 -> 0x8012d694 ; else j epilogue
    }
    // ---- 0x8012d694: node[5]==1 ----
    uint8_t n6 = c->mem_r8(obj + 6); // 8012D694 lbu v0,6(s0)
    if (n6 == 0) {
      advance_node32(c, obj);
      return;
    } // 8012D69C beqz -> 0x8012d78c
    if (n6 != 1) {
      tail_set1_and_render(c, obj);
      return;
    } // 8012D6A4 bne v0,v1(=1) -> 0x8012d848 (node[1]=1)
  ...
  case 1: {                          // jt[1] = 0x8012d6f0
    ...
    if (s5 == 0) {                   // 8012D6F8 beqz -> 0x8012d710 (delay v0=1)
      ...
      // 0x8012d73c:
      if (c->mem_r8(0x1F800207u) < 0x1c) {
        return; // 8012D748 bnez(p<0x1c) -> 0x8012d8f4 epilogue
      }
      if (c->mem_r16s(0x1F8000DEu) < 0x3f8c) {
        return; // 8012D758/75C bnez(lh<0x3f8c) -> 0x8012d8f4 epilogue
      }
      c->mem_w8(obj + 1, 1); // 8012D768 sb v0(=1),1(s0)  (j 0x8012d8f4 delay slot)
      return;                // 8012D764 j epilogue
    }
    if (s5 != 1) {
      return; // 8012D700 beq v1,1 -> 0x8012d76c ; else j epilogue
    }
    // ---- 0x8012d76c: node[5]==1 ----
    ...
    if (n6 == 0) {
      advance_node32(c, obj);
      return;
    } // 8012D774 beqz -> 0x8012d78c
    if (n6 == 1) {
      step_node18(c, obj);
      return;
    } // 8012D77C beq v0,v1(=1) -> 0x8012d7d8
