...
//             the post-cull visibility update + bounds cull. (Scroll direction matches the guest instruction path: Z
//             increases; the guest instruction path doesn't test overflow — the beat ends by external scene advance.)
//     For beats != 5 the running body is a no-op, matching the guest instruction path's outermost gate.
//   * 2 or 3 = DESPAWN: standard pool return via Spawn::despawn (native). NB: unlike the other two
//     SOP intro handlers this one accepts BOTH 2 AND 3 as "cleanup" (guest instruction path: `bVar1 < 4`).
//   * anything else: no-op.
//
...
inline void post_cull_update(Core *c, uint32_t o) {
  rend(c)->mNodeXform.buildWithOffset(o);
} // native FUN_800518FC (NodeXform::buildWithOffset)
...
//
// BLOCK LAYOUT (fixed 2026-07-10, prologue-vortex): overlay guest 0x8010B990 stages the halfwords at
...
void spawn_narration_prop(Core *c, uint32_t obj) {
  ...
  const uint32_t blk = c->r[29] + 16u; // a1 = sp+16, as in the guest-visible behavior
  ...
  psx::cpu::dispatchGuestToReturn0(
      *c, 0x8003116Cu, psx::cpu::ExecutionBudget::currentTurn(*c), __func__); // narration-prop spawn (substrate)
...
void state_running(Core *c, uint32_t obj) {
  if (c->mem_r8(SCENE_BEAT) != NARRATION_BEAT) {
    return; // active only during the void beat
  }
...
void beh_sop_intro_narration(Core *c) {
  ...
  if (st == 1) {
    ...
    eng(c).spawn.despawn(obj); // guest instruction path: bVar1 < 4
  }
  // else: no-op
}
