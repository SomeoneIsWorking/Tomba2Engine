...
void beh_substate_edge_orchestrator(Core *c) {
  ...
  if (st != 1) {
    ...
    // ---- STATE 0 [0x8012EBAC] ----
    c->r[4] = obj;
    rec_dispatch(c, 0x8012ED84u); // FUN_8012ed84 (per-type init)
    return;                       // j 0x8012ED70 (epilogue)
  }

  // ---- STATE 1 [0x8012EBBC] ----
  c->r[4] = obj;
  rec_dispatch(c, 0x80130AC4u);            // FUN_80130ac4
  ...
  if (sub < 6) {                    // sltiu v0,v1,6 / beqz -> tail [0x8012EBE0..E4]
    ...
    case 0: // jt[0]=0x8012ec08
      c->r[4] = obj;
      rec_dispatch(c, 0x8012F494u);      // FUN_8012f494
      break;                             // j 0x8012EC74 (tail)
    case 1:                              // jt[1]=0x8012ec18
      if (c->mem_r8(0x1F800137u) == 0) { // lbu v0,0x137(0x1f80_0000); bnez -> tail
        c->r[4] = obj;
        rec_dispatch(c, 0x8012F5B4u); // FUN_8012f5b4
      }
      break; // j 0x8012EC74 (tail)
    case 2:  // jt[2]=0x8012ec3c
      c->r[4] = obj;
      rec_dispatch(c, 0x8012FD88u); // FUN_8012fd88
      break;
    case 3: // jt[3]=0x8012ec4c
      c->r[4] = obj;
      rec_dispatch(c, 0x80130524u); // FUN_80130524
      break;
    case 4: // jt[4]=0x8012ec5c
      c->r[4] = obj;
      rec_dispatch(c, 0x801313C4u); // FUN_801313c4
      break;
    case 5: // jt[5]=0x8012ec6c (NO j; falls through to tail)
      c->r[4] = obj;
      rec_dispatch(c, 0x8018C820u); // FUN_8018c820
      break;
    }
  }
  ...
  // ---- tail [0x8012EC74] ----
  c->r[4] = obj;
  rec_dispatch(c, 0x80131134u); // FUN_80131134
  c->r[4] = obj;
  rec_dispatch(c, 0x801316CCu); // FUN_801316cc
  ...
  if (c->mem_r8(obj + 3) == 0) {          // lbu v0,3(s1); bnez -> ed48 [0x8012ED34]
    ...
    c->r[4] = obj;
    c->r[5] = a1;
    rec_dispatch(c, 0x80146348u); // FUN_80146348 [0x8012ED40]
  }
  if (c->mem_r8(obj + 1) != 0) { // lbu v0,1(s1); beqz -> ed60 [0x8012ED50]
    c->r[4] = obj;
    rec_dispatch(c, 0x8012E8A8u); // FUN_8012e8a8 [0x8012ED58]
  }
