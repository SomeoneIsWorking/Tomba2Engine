...
// resident siblings (the FUN_739ac handler / the FUN_73cd8 handler / the FUN_741dc handler): a state machine on the
// node's state byte node[4] (0 init / 1 active / 2 idle / 3 despawn). It is much larger than the resident ones — most
// of the work lives in ~14 sub-functions it CALLS, which stay PSX leaves via typed runtime address dispatch:
...
// native; every sub-behavior CALL stays a reachable PSX leaf via typed runtime address dispatch (NO recursion into
// them). NO GTE, NO render packets here. RE'd 1:1 from disas 0x8012EB54..0x8012ED80 (jump table 0x80109dec =
// {ec08,ec18,ec3c,ec4c,ec5c,ec6c}, decoded from the field RAM dump). It WRITES guest node state the
// still-guest content reads -> content-INTERFACE: gated byte-exact (full RAM+scratchpad A/B vs
// original guest-body call). The idle/active field path is exercised by the gate; the input/scene-driven sub-states
...
static constexpr GuestFrameSpill kSpills_8012EB54[3] = {
    {17, 20},
    {31 /*ra*/, 24},
    {16, 16},
...
void beh_substate_edge_orchestrator(Core *c) {
  GuestFrame<32, 3> frame(c, kSpills_8012EB54);
  ...
  if (st != 1) {
    if (st >= 2) { // slti s0,2 / beqz [0x8012EB74..78]
      if (st == 2) {
        return; // beq s0,2 -> epilogue   [0x8012EB94]
      }
    ...
    if (st != 0) {
      return; // beqz s0 [0x8012EB80]; the s0<2 && !=0 path j's ED70
    }
    // ---- STATE 0 [0x8012EBAC] ----
    c->r[4] = obj;
    psx::cpu::dispatchGuestToReturn0(
        *c, 0x8012ED84u, psx::cpu::ExecutionBudget::currentTurn(*c), __func__); // FUN_8012ed84 (per-type init)
  ...
  c->r[4] = obj;
  psx::cpu::dispatchGuestToReturn0(
      *c, 0x80130AC4u, psx::cpu::ExecutionBudget::currentTurn(*c), __func__); // FUN_80130ac4
  ...
  if (sub < 6) {                    // sltiu v0,v1,6 / beqz -> tail [0x8012EBE0..E4]
    ...
    case 0: // jt[0]=0x8012ec08
      c->r[4] = obj;
      psx::cpu::dispatchGuestToReturn0(
          *c, 0x8012F494u, psx::cpu::ExecutionBudget::currentTurn(*c), __func__); // FUN_8012f494
      break;                                                                      // j 0x8012EC74 (tail)
    case 1:                                                                       // jt[1]=0x8012ec18
      if (c->mem_r8(0x1F800137u) == 0) { // lbu v0,0x137(0x1f80_0000); bnez -> tail
        c->r[4] = obj;
        psx::cpu::dispatchGuestToReturn0(
            *c, 0x8012F5B4u, psx::cpu::ExecutionBudget::currentTurn(*c), __func__); // FUN_8012f5b4
      }
      break; // j 0x8012EC74 (tail)
    case 2:  // jt[2]=0x8012ec3c
      c->r[4] = obj;
      psx::cpu::dispatchGuestToReturn0(
          *c, 0x8012FD88u, psx::cpu::ExecutionBudget::currentTurn(*c), __func__); // FUN_8012fd88
      break;
    case 3: // jt[3]=0x8012ec4c
      c->r[4] = obj;
      psx::cpu::dispatchGuestToReturn0(
          *c, 0x80130524u, psx::cpu::ExecutionBudget::currentTurn(*c), __func__); // FUN_80130524
      break;
    case 4: // jt[4]=0x8012ec5c
      c->r[4] = obj;
      psx::cpu::dispatchGuestToReturn0(
          *c, 0x801313C4u, psx::cpu::ExecutionBudget::currentTurn(*c), __func__); // FUN_801313c4
      break;
    case 5: // jt[5]=0x8012ec6c (NO j; falls through to tail)
      c->r[4] = obj;
      psx::cpu::dispatchGuestToReturn0(
          *c, 0x8018C820u, psx::cpu::ExecutionBudget::currentTurn(*c), __func__); // FUN_8018c820
      break;
    }
  }
  ...
  c->r[4] = obj;
  psx::cpu::dispatchGuestToReturn0(
      *c, 0x80131134u, psx::cpu::ExecutionBudget::currentTurn(*c), __func__); // FUN_80131134
  c->r[4] = obj;
  psx::cpu::dispatchGuestToReturn0(
      *c, 0x801316CCu, psx::cpu::ExecutionBudget::currentTurn(*c), __func__); // FUN_801316cc
  ...
  if (g == 0) { // bnez v1 -> ECB4 [0x8012EC90]
    ...
    if (n78 & 1) {                               // andi 1; beqz -> ed2c
      ...
      if (x > 0) {
        c->mem_w16(obj + 0x4e, (uint16_t)(uint32_t)(int32_t)-0x80); // blez false -> -0x80
      } else if (x < 0) {
        c->mem_w16(obj + 0x4e, 0x80); // bgez false (x<0) -> +0x80
      }
  ...
  if (c->mem_r8(obj + 3) == 0) {          // lbu v0,3(s1); bnez -> ed48 [0x8012ED34]
    ...
    c->r[5] = a1;
    psx::cpu::dispatchGuestToReturn0(
        *c, 0x80146348u, psx::cpu::ExecutionBudget::currentTurn(*c), __func__); // FUN_80146348 [0x8012ED40]
  }
  if (c->mem_r8(obj + 1) != 0) { // lbu v0,1(s1); beqz -> ed60 [0x8012ED50]
    c->r[4] = obj;
    psx::cpu::dispatchGuestToReturn0(
        *c, 0x8012E8A8u, psx::cpu::ExecutionBudget::currentTurn(*c), __func__); // FUN_8012e8a8 [0x8012ED58]
  }
