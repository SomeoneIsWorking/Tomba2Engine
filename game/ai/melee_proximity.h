...
// (authenticated executable/overlay evidence, `guest 0x8001F9DC`), the SIMPLEST member of a ~9-function family of
...
// authenticated executable/overlay evidence (the recorded binary evidence's instruction-exact C, the actual ground
// truth per CLAUDE.md), then INDEPENDENTLY RE-VERIFIED line-by-line — 3 real bugs found and fixed (see
...
// `tomba::native::declareOverride` passing tomba::native::declareOverride as the setter (the only real callers are
// direct `guest 0x8001F9DC(c)` sites in shard_1.c/shard_5.c), which also makes it reachable via typed runtime address
// dispatch. SBS-gated 0-diff; see registerOverrides().
//
// Guest ABI: a0 = self (r19 in the guest instruction path — the actor being tested), a1 = other (r16 — the actor
...
//
// Field layout used (RAW DECIMAL OFFSETS, copied verbatim from the recorded binary evidence's decimal literals
...
//                  (FUN_80077FB0) — left substrate, reached via typed runtime address dispatch, exactly as
//                  ActorMeleeEngage does (same leaf, same reason).
//   FUN_80085690 -> already-native `Trig::ratan2` (game/math/trig.h) — used directly, no
//                  typed runtime address dispatch needed (same rationale as ActorMeleeEngage: Trig is RE'd-correct but
...
public:
  ...
  // isAtApproachAnchorFramed: guest-ABI-facing twin used by the tomba::native::declareOverride trampoline (see
  ...
  // writes result to c->r[2] (v0), matching guest 0x8001F9DC's own entry/exit convention.
  void isAtApproachAnchorFramed();

  // Wire isAtApproachAnchor onto guest address 0x8001F9DC via `tomba::native::declareOverride`, passing
  // tomba::native::declareOverride (the recorded binary evidence's global call table — the only real callers found are
  // direct `guest 0x8001F9DC(c)` sites) as the setter so both that call shape and typed runtime address dispatch
