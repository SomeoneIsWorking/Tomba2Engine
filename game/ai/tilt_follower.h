...
struct TiltFollowerNode {
  ...
  uint32_t owner()        const { return mCore->mem_r32(mAt + tiltfollow::kOwner); }
  uint32_t contactState() const { return mCore->mem_r8(mAt + tiltfollow::kContactState); }

  void setPitch(uint16_t v) const { mCore->mem_w16(mAt + tiltfollow::kRotX, v); }
};
...
struct TiltSourcePart {
  ...
  int32_t childEulerX() const { return mCore->mem_r16s(mAt + tiltpart::kChildEulerX); }
};
