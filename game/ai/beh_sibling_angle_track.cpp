...
// in a0. Same SHAPE as the resident/overlay siblings (the FUN_739ac handler / the FUN_73cd8 handler / the FUN_8012eb54
// handler / the FUN_80124e74 handler / the FUN_80138fc8 handler): a state machine on the node's state byte node[4] (0
// init / 1 active / 2 idle / 3 despawn). It loads s1 = node[0x10] (a sibling/parent object-node pointer) up front and
// uses it heavily in both states.
//
...
void beh_sibling_angle_track(Core *c) {
  ...
  if (st != 1) {
    ...
    if (st != 0) {
      return; // 801395F4 beqz v1 -> state0 ; else 801395FC j 0x80139820
    }
    ...
    c->r[4] = obj;
    c->r[5] = 0xc;
    c->r[6] = 0x16;                   // 80139620 a1=0xc ; 80139628 a2=0x16 (delay) ; a0=s2
    eng(c).graphicsBind.recordInit(); // 80139624 jal 0x80051b70
    if (c->r[2] != 0) {
      return; // 8013962C bnez v0 -> 0x80139820 (init busy)
    }
  ...
  if (g < 0x17) {
    return; // 801396E4 sltiu v0,v1,0x17 ; 801396E8 bnez -> epilogue
  }
  if (g == 0x22) {
    return; // 801396EC v0=0x22 ; 801396F0 beq v1,0x22 -> epilogue
  }
  ...
  int16_t s0 =
      (int16_t)(uint16_t)(c->mem_r16(s1 + 0x50) - c->mem_r16(s1 + 0x32)); // 801397B8/BC/C4 s0 = s1[0x50]-s1[0x32]
