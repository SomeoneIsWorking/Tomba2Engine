...
void ReleaseTriggerMotion::hoverBobCycle(uint32_t obj) {
  ...
  c->r[4] = obj;
  rec_dispatch(c, 0x80077B5Cu); // FUN_80077B5C — still-substrate position commit
}
  ...
  if (c->mem_r8(obj + 0x5e) == 0) {
    ...
    if (((bitmaskByte >> (typeByte & 0x1f)) & 1u) == 0) {
      ...
      if (cVar1 != 0) {
        ...
        c->r[4] = obj;
        rec_dispatch(c, 0x80123C94u); // FUN_80123C94 — still-substrate predicate
        if (c->r[2] == 0) {
    ...
    c->r[4] = obj;
    rec_dispatch(c, 0x8012400Cu); // FUN_8012400C — still-substrate
  }
}
...
void ReleaseTriggerMotion::driftReposition(uint32_t obj, uint32_t variant) {
  ...
  if (c->mem_r8(obj + 5) == 0) {
    ...
    if (Actor(c, obj).boundsCull() != 0) { // FUN_8007778C
      ...
      c->r[4] = rec98;
      rec_dispatch(c, 0x80051794u); // still-substrate
      c->r[4] = obj + 0x54;
      c->r[5] = rec98;
      rec_dispatch(c, 0x800847F0u); // still-substrate
      c->r[4] = 0x1F8000F8u;
      c->r[5] = rec98;
      rec_dispatch(c, 0x80084360u); // still-substrate
      c->r[4] = obj;
      rec_dispatch(c, 0x80077B5Cu); // still-substrate
    }
    if (c->mem_r8(0x800BF9DDu) == 0xe) {
      ...
      c->r[4] = obj;
      rec_dispatch(c, 0x80124328u); // FUN_80124328 — still-substrate
    }
  }
}
