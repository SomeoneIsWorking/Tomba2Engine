...
static inline uint32_t leaf_ret(Core *c) {
  return c->r[2];
}
...
static void whiteFlashPhaseRamp(Core *c, uint32_t node) {
  ...
  case 1: {
    ...
    if (c->mem_r8(0x800BFA20u) < 3) {
      return;
    }
    if ((int16_t)c->mem_r16(node + 0x40) != 0) {
      return;
    }
  ...
  case 2: {
    ...
    if (v < 0x40) {
      c->mem_w16(node + 0x40, (uint16_t)(v + 8));
      return;
    }
    if (c->mem_r8(0x800BFA20u) < 4) {
      return;
    }
    c->mem_w8(node + 6, (uint8_t)(st + 1));
    return;
  ...
  default:
    return; // st >= 5: guest falls through the switch to the epilogue; no state change.
  }
}
...
static void state1_run(Core *c, uint32_t nd) {
  ...
  case 6: {
    ...
    if (s5 == 1) {
      ...
      if (v2 > 0x1000) {
        c->mem_w16(nd + 0x4A, 0x1000);
      }
      if (c->mem_r8(0x800BFA22u) > 0x19) {
...
// native port splits it into this dispatcher plus the state_* statics. The frame belongs here, at
// the guest function's entry, so it spans the whole call exactly as the guest's does.
void beh_a06_multi_actor(Core *c) {
  GuestFrame<32, 4> frame(c, kSpills_801189E8);
  uint32_t nd = c->r[4];
