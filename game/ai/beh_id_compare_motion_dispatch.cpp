...
void beh_id_compare_motion_dispatch(Core *c) {
  ...
  if (st != 1) {     // beq v1,1 -> state 1         [0x80145258]
    ...
    if (st != 0) {
      return; // beqz v1 -> state 0; else j epilogue [0x8014526C/74]
    }
  ...
  //   (DAT_800bf89c = -0x764(0x800c0000); DAT_800bf809 = -0x7f7(0x800c0000).)
  {
    ...
    if (to_tick) {
      ...
      if (n2b != 0) {                              // beqz -> 0x801453a8 (node3 dispatch) [0x80145348]
        ...
        c->r[4] = obj;
        eng(c).cull.cullWrapperFlag2(); // FUN_800777FC (native)
        if (c->r[2] == 0) {
          return; // beqz v0 -> epilogue          [0x80145390]
        }
  ...
  //            ELSE FALLS THROUGH to 0x801453EC — i.e. the same block as n3==0 (GOTCHA: not motion).
  {
    ...
    if (n3 == 0x80) { // beq 0x80 -> 0x801454f8       [0x801453DC]
      c->r[4] = obj;
      eng(c).attackOrbit.aimAtTargetAnchor(); // FUN_80145af0 (native)  [0x801454F8]
      goto second_cull;                       // j 0x80145510
    }
    if (n3 == 0x81) { // beq 0x81 -> 0x80145508       [0x801453E4]
      c->r[4] = obj;
      eng(c).attackOrbit.orbitTargetMotion(); // FUN_801458e0 (native)  [0x80145508]
      goto second_cull;                       // j 0x80145510
    }
...
second_cull:;
  ...
  eng(c).cull.cullWrapperFlag2(); // FUN_800777FC (native)
  if (c->r[2] == 0) {
    goto after_no_cull; // beqz v0 -> 0x8014560c        [0x80145518]
  }
  {
