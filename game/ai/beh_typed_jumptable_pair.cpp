...
// resident/overlay siblings (the FUN_739ac handler / the FUN_73cd8 handler / the FUN_8012eb54 handler / the
// FUN_80124e74 handler): a state machine on the node's state byte node[4] (0 init / 1 active / 2 idle / 3 despawn).
//
...
void beh_typed_jumptable_pair(Core *c) {
  ...
  if (st != 1) {
    ...
    if (st != 0) {
      return; // 80138FF4 beqz v1 -> state0; else j 0x801395ac
    }
    ...
    c->r[4] = obj;
    c->r[5] = 0xc;
    c->r[6] = a2;                     // 80139038 a1=0xc ; a2 (delay) ; a0=s0
    eng(c).graphicsBind.recordInit(); // 80139034 jal 0x80051b70
    if (c->r[2] != 0) {
      return; // 8013903C bnez v0 -> 0x801395ac (init busy)
    }
    ...
    if (n3 >= 6) {
      return; // 801391DC beqz (sltiu v1<6) -> 0x801395ac
    }
    switch (n3) { // JT0 @0x80109F44
    case 0:
    case 1:
    case 3:                     // jt0[0]/[1]/[3] = 0x8013924c
    ...
    case 2: {                   // jt0[2] = 0x80139200
      ...
      if (g0e == 0) {
        return; // 80139218 beqz v0 -> 0x801395ac
      }
      uint8_t g0f = c->mem_r8(0x800BF817u); // 80139220 lbu v1, 0xf(0x800bf808)
      if (g0f != 2) {
        return; // 80139228 bne v1,2 -> 0x801395ac
      }
  ...
  if (c->mem_r8(0x800BF816u) != 0) { // 8013927C lbu v0, 0xe(base) ; 80139284 beqz -> 0x801392b0
    ...
    if (c->mem_r8(0x800BF817u) ==
        c->mem_r8(obj + 3)) { // 8013928C lbu v1,0xf(base) ; 80139290 lbu v0,3(s0) ; 80139298 bne -> 0x801392ec
  ...
  if (n3 < 6) {                    // 801392F8 beqz (sltiu v1<6) -> 0x80139580 (tail)
    ...
    case 0: { // jt1[0] = 0x8013931c : node[5] sub-machine
      ...
      if (s5 >= 2) { // ---- 0x8013934c ----
        ...
        if (s5 == 3) {
          // ---- 0x801393dc: node[5] == 3 ----
          if (c->mem_r8(S1 + 0x2a) == c->mem_r8(obj + 0x2a)) {
            break; // 801393DC lbu v1,0x2a(a1) ; 801393E0 lbu v0,0x2a(s0) ; 801393E8 beq -> tail
          }
      ...
      // ---- 0x80139364: node[5] == 0 ----
      if (c->mem_r8(S1 + 0x2a) != c->mem_r8(obj + 0x2a)) {
        break; // 80139364 lbu v1,0x2a(a1) ; 80139368 lbu v0,0x2a(s0) ; 80139370 bne -> tail
      }
      if (c->mem_r8(S1 + 5) != 0) {
        break; // 80139378 lbu v0,5(a1) ; 80139380 bnez -> tail
      }
    ...
    case 3: {                          // jt1[3] = 0x8013947c : node[5] sub-machine
      ...
      if (s5 == 1) {
        ...
        if (n6 == 0) {                   // 801394E0 beqz v0 -> 0x801394f8
          ...
          if (c->mem_r8(obj + 0x29) == 0) {
            break; // 801394F8 lbu v0,0x29(s0) ; 80139500 beqz -> tail
          }
        ...
        if (n6 == s5) { // 801394E8 beq v0,v1 (v1==node[5]==1) -> 0x8013953c
          // ---- 0x8013953c ----
          if (c->mem_r8(obj + 0x29) != 0) {
            break; // 8013953C lbu v0,0x29(s0) ; 80139544 bnez -> tail
          }
          // 0x800e0000 + 0x7fc7 = 0x800E7FC7
          uint8_t gbit = c->mem_r8(0x800E7FC7u) & 1; // 8013954C lbu v0,0x7fc7(0x800e) ; 80139554 andi 1
          if (gbit != c->mem_r8(obj + 0x5f)) {
            break; // 80139550 lbu v1,0x5f(s0) ; 80139558 bne v0,v1 -> tail
          }
      ...
      if ((c->mem_r8(0x800BF9DBu) & 1) !=
          0) {                 // 801394B4 lbu v0,0x9db(base) ; 801394BC andi 1 ; 801394C0 beqz -> 0x801394d0
  ...
  if (c->mem_r8(obj + 1) != 0) { // 80139580 lbu v0,1(s0) ; 80139588 beqz -> 0x80139598
    c->r[4] = obj;
    eng(c).graphicsBind.renderUpdate(); // 80139590 jal 0x800517f8 (a0=s0)
  }
