...
void state1_gate(Core *c, uint32_t obj) {
  ...
  if (f & 0x80) {
    ...
    } else if ((uint32_t)(uint8_t)c->mem_r8(0x800BF817u) != (uint32_t)c->mem_r16s(obj + 0x6a)) {
  ...
  if (submit) {
    eng(c).cull.enqueueQueueC(obj); // 0x8004c378 — FUN_80077EFC (native)
  }
}
...
void beh_visibility_gate_dispatch(Core *c) {
  ...
  if (st == 1) {
    goto state1; // 0x8004c250: beq v1,1
  }
  if (st >= 2) { // 0x8004c258: slti v0,v1,2 ; beq->c270
    if (st == 2) {
      goto state2; // 0x8004c274
    }
    if (st == 3) { // 0x8004c27c
  ...
  if (st != 0) {
    return; // 0x8004c260: only state 0 left (delay-checked)
  }
...
state1: {
  ...
  if (n5e >= 16) {
    goto tail_c74c; // 0x8004c300: beq -> 0x8004c74c
  }
  ...
  case 0: // @0x8004c324
    state1_gate(c, obj);
    if (c->mem_r8(obj + 1) == 0) {
      goto tail_c74c; // 0x8004c388: beq node[1],0 -> c74c
    }
    c->r[4] = obj;
    c->r[5] = 0;
...
state2: {
  ...
  if (n5 == 1) {
    goto state2_n5_1; // 0x8004c768: beq v0,a0(==1) -> c8c8
  }
  if (n5 != 0) {
    return; // 0x8004c770: default -> EPI
  }
  ...
  if (n3 >= 18) {                  // 0x8004c784: sltiu fails -> default
    ...
    case 1:
      ...
      break; // c7bc
    case 4:
      eng(c).spawn.dropScoreGem(obj, 100);
      check_ret = false;
      break; // c7d0 FUN_8004B3F4 (native)
    case 5:
      eng(c).spawn.dropScoreGem(obj, 200);
      check_ret = false;
      break; // c7e4
    case 6:
      eng(c).spawn.dropScoreGem(obj, 500);
      check_ret = false;
      break; // c7f8
    case 7:
      eng(c).spawn.dropScoreGem(obj, 1000);
      check_ret = false;
      break; // c80c
    case 8:
      eng(c).spawn.dropScoreGem(obj, 5000);
      check_ret = false;
      break; // c820
    case 9:
      eng(c).spawn.dropScoreGem(obj, 10000);
      check_ret = false;
      break; // c834
    case 10:
      eng(c).spawn.dropScoreGem(obj, 20000);
      check_ret = false;
      break; // c848
    case 11:
      eng(c).spawn.dropScoreGem(obj, 100000);
      check_ret = false;
  ...
  if (check_ret && c->r[2] == 0) {
    return; // 0x8004c8ac: beq v0,zero,c920
  }
