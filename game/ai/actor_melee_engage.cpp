...
int32_t ActorMeleeEngage::doIt(uint32_t self, uint32_t target, uint32_t anchor) { // FUN_80112188 — UNWIRED draft
  ...
  const int32_t dz = (int32_t)((uint16_t)c->mem_r16(self + 46u) - (uint16_t)(c->mem_r16(anchor + 44u) + kindZBias));
  ...
  if (radiusSum1 < (int32_t)dist16) {
    return 0; // too far in XZ
  }
  ...
  if (yThreshold < (int32_t)yBandSum) {
  ...
  if (doReposition) {
    ...
    const int32_t facingAbs =
        (facing >= 0) ? (int32_t)(uint16_t)c->mem_r16(self + 68u) : -(int32_t)(uint16_t)c->mem_r16(self + 68u);
    ...
    if ((int16_t)facingAbs < 6657) {
      return 1;
    }
    if (c->mem_r8(self + 4u) != 1) {
      return 1;
    }
    if (c->mem_r8(self + 5u) == 19) {
      return 1;
    }
  ...
  if ((c->mem_r8(self + 325u) & 1u) != 0) {
    return 0; // bail if this bit is ALREADY set (already armed)
  }
  ...
  if (targetFaceCheck == 0) {
    return abortToDisengage();
  }
  if ((c->mem_r8(self + 0u) & 4u) != 0) {
    return abortToDisengage();
  }
