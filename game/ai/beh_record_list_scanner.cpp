...
// Same SHAPE as the sibling owned behaviors (the FUN_80071a3c handler / the FUN_739ac handler / …): a state machine on
// the node's state byte node[4]. RE'd 1:1 from disas 0x8004CE14:
//
...
inline uint32_t lh(Core *c, uint32_t a) {
  return (uint32_t)c->mem_r16s(a);
}
...
void beh_record_list_scanner(Core *c) {
  ...
  for (;;) {
    ...
    if (s2 == 0) {
      if (eng(c).bit.test7EC((int32_t)lh(c, s4 + 10), 0) != 0) {
        verdict = 0;
      }
    }
    if (verdict == -2 && (c->mem_r32(obj + 0x74) & s1) != 0) {
      verdict = 0;
    }
    if (verdict == -2 && s2 != 0) {
      if (eng(c).bit.test868((int32_t)lh(c, s4 + 10)) != 0) {
        verdict = 0;
      }
    }
    if (verdict == -2) {
      verdict = ((c->mem_r32(obj + 0x74) & s1) != 0) ? 0 : 1;
    }
    ...
    if (verdict == 1) {
      ...
      if (ret != 0) {
        c->mem_w32(s5 + 16, c->mem_r32(s5 + 16) | s1);
      }
    }
    s3 += 1;
    s4 += 16;
