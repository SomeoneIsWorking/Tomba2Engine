...
// / the FUN_80124e74 handler): a state machine on the node's state byte node[4] (0 init / 1 active / 2 idle / 3
// despawn). It is SMALL and has NO jump table — a flat per-state body:
...
static constexpr GuestFrameSpill kSpills_8012D404[2] = {
    {16, 16},
    {31 /*ra*/, 20},
}; // frame=24, abi_extract --scaffold --guestabi

void beh_cull_tick_render(Core *c) {
  GuestFrame<24, 2> frame(c, kSpills_8012D404);
  ...
  if (st != 1) {
    ...
    if (st != 0) {
      return; // 8012D434 j 0x8012d4dc (v1<2 && !=0: dead path, epilogue)
    }
    ...
    if (c->r[2] != 0) {
      return; // 8012D478 bnez v0 -> 0x8012d4dc (init busy/failed -> epilogue)
    }
  ...
  if (Actor(c, obj).boundsCull() == 0) {
    return; // 8012D4AC jal 0x8007778c — Actor::boundsCull (native); cull → epilogue
  }
