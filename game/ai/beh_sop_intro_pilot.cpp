...
inline void anim_env_setup(Core *c, uint32_t obj) {
  eng(c).script.init(obj, ANIM_ENV_PTR, ANIM_DATA_PTR);
} // FUN_80040CDC = ScriptInterp::init (init obj as script-driven: tableA=ANIM_ENV_PTR, script=ANIM_DATA_PTR)
inline void walk_start(Core *c, uint32_t obj) {
  eng(c).walkStart(obj, 8, 0);
} // native FUN_80054D14
...
inline void post_cull_update(Core *c, uint32_t obj) {
  rend(c)->mNodeXform.buildWithOffset(obj);
} // native FUN_800518FC (NodeXform::buildWithOffset)
inline void anim_graphics_tick(Core *c, uint32_t obj) {
  (void)eng(c).animTick(obj);
} // native FUN_8004190C
...
void state_init(Core *c, uint32_t obj) {
  ...
  if (try_model_attach(c, obj) != 0) {
    return; // model not ready — retry next frame
  }
...
static constexpr GuestFrameSpill kSpills_8010ACFC[3] = {
    {16, 16},
    {31 /*ra*/, 24},
    {17, 20},
...
void beh_sop_intro_pilot(Core *c) {
  GuestFrame<32, 3> frame(c, kSpills_8010ACFC);
