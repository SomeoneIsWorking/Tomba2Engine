...
void beh_camera_target_follow(Core *c) {
  ...
  if (st != 1) {
    ...
    if (c->mem_r8(p + 0x181) == 0) {
      c->mem_w16(0x1F80019Eu, 0); // 0x80059f3c (only if target[0x181]==0)
    }
    c->mem_w16(nd + 0x54, 0);
    c->mem_w8(nd + 4, (uint8_t)(st + 1)); // node[4] -> 1
    if (s16(c, p + 0x17e) < 0) {
      c->mem_w16(nd + 0x40, 0x28);
      c->mem_w16(nd + 0x42, 0x37);
    } else {
      c->mem_w16(nd + 0x40, 0x50);
      c->mem_w16(nd + 0x42, 0x6e);
    }
    c->mem_w16(nd + 0x0e, 0);
    c->mem_w8(nd + 0x2b, 0);
    c->mem_w8(nd + 0x46, 0);
  ...
  if (!goto_L1ac && set_node1_zero) {
    c->mem_w8(nd + 1, 0); // @0x8005a1a8
  }
