...
int32_t MeleeProximity::isAtApproachAnchor(uint32_t self, uint32_t other) { // FUN_8001F9DC — UNWIRED draft
  ...
  const int32_t otherAnchorZ = (int32_t)(uint16_t)c->mem_r16(other + 46u) + (int32_t)(uint16_t)c->mem_r16(other + 96u);
  ...
  const int32_t otherAnchorX = (int32_t)(uint16_t)c->mem_r16(other + 54u) + (int32_t)(uint16_t)c->mem_r16(other + 100u);
  ...
  if (radiusSum < (int32_t)dist16) {
    return 0; // too far in XZ
  }

  // ---- Y-band test: self.Y vs (other.Y + other.anchorOfsY(+98)) -----------------------------------
  const int32_t otherAnchorY = (int32_t)(uint16_t)c->mem_r16(other + 50u) + (int32_t)(uint16_t)c->mem_r16(other + 98u);
  ...
  if (yThreshold < (int32_t)(uint16_t)(dy + heightSum)) {
