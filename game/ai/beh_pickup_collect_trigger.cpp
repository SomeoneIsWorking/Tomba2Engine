...
inline void do_case0(Core *c, uint32_t obj) {
  ...
  if (sceneNode != 0) {
    if (c->mem_r8(0x800BF8EDu) == 0) {
      ...
      c->r[31] = 0x80074384u;       // ra mirror: gen jal-site (armBody spills ra)
...
bool beh_pickup_collect_trigger_body(Core *c) {
  ...
  if (sub == 2) { // pad edge -> node[5]=3
    if (c->mem_r16(0x800E7E68u) & c->mem_r16(0x1F800174u)) {
      c->mem_w8(obj + 5, 3);
    }
  } else if (sub < 3) {
    if (sub == 0) {
      ...
      iVar3 = eng(c).bgSceneTransitionSm.readyForProgress() ? 1 : 0;
      do_inc = true;
    } // FUN_80042728 native
  } else {
    if (sub == 4) {
      ...
      if (c->mem_r8(p + 4) > 1) {
        ...
        if (c->mem_r8(0x800BF8EEu) == 0) {
          c->r[31] = 0x80074548u;       // ra mirror: gen jal-site (armBody spills ra)
