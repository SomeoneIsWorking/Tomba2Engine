...
void beh_actor_tomba_proximity_combat(Core *c) { // FUN_800527C8
  ...
  uint32_t saved[9];
  for (int i = 0; i < 9; i++) {
    saved[i] = c->r[kSpills[i].reg];
  }
  c->r[29] -= 72;
  for (int i = 0; i < 9; i++) {
    c->mem_w32(c->r[29] + (uint32_t)kSpills[i].offset, saved[i]);
  }
...
L_80052B18:;
  ...
  c->mem_w16((self + (uint32_t)104), (uint16_t)c->r[4]);
  goto L_80052D08;
L_80052B70:;
  c->r[2] = (uint32_t)8064u << 16;
...
L_80052BBC:;
  ...
  c->mem_w8((c->r[3] + (uint32_t)-32660), (uint8_t)c->r[2]);
  goto L_80052D08;
L_80052C10:;
  c->r[2] = (uint32_t)8064u << 16;
  c->r[2] = (uint32_t)c->mem_r16((c->r[2] + (uint32_t)380));
...
L_80052D50:;
  ...
  c->r[2] = (uint32_t)c->mem_r16((self + (uint32_t)86));
  c->mem_w16((G + (uint32_t)86), (uint16_t)c->r[2]);
  c->mem_w8((self + (uint32_t)1), (uint8_t)c->r[3]);
  goto L_80053040;
L_80052D70:;
  c->r[3] = (uint32_t)c->mem_r8((self + (uint32_t)5));
  c->r[2] = (uint32_t)(c->r[3] < (uint32_t)5);
...
L_80052DA0:;
  ...
  c->mem_w8((self + (uint32_t)5), (uint8_t)c->r[3]);
  goto L_80052FE8;
L_80052E68:;
  c->r[4] = self + (uint32_t)0;
  rec_dispatch(c, 0x80052720u);
...
L_80053060:;
  ...
  for (int i = 0; i < 9; i++) {
    c->r[kSpills[i].reg] = c->mem_r32(c->r[29] + (uint32_t)kSpills[i].offset);
  }
  c->r[29] += 72;
  return;
}
