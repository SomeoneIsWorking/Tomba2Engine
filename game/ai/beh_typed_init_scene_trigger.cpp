...
inline void confirm_or_advance(Actor &a, uint32_t arg, uint32_t raConst) {
  ...
  if (eng(a.core()).sceneEvents.arm((uint8_t)arg) != 0) {
    a.setTriggerSub(5);
  }
}

// Special-area set for two paths (state-0 alive() + box override, tail sceneHandle release):
inline bool is_special_area(uint8_t area) {
  return area == 2 || area == 7 || area == 0x14;
}
} // namespace
...
void beh_typed_init_scene_trigger(Core *c) {
  ...
  if (st != Sta::Active) {
    if ((uint8_t)st >= (uint8_t)Sta::Idle) {
      if (st == Sta::Idle) {
        return; // state 2: idle no-op
      }
      if (st != Sta::Despawn) {
        return; // state >=4: no-op
      }
    ...
    c->r[4] = a.addr();
    c->r[5] = 0xc;
    c->r[6] = (uint32_t)(int32_t)sz;
    eng(c).graphicsBind.recordInit();
    if (c->r[2] != 0) {
      return; // init busy/failed
    }
  ...
  if (sub < 7) { // JT 0x80016BE8
    ...
    case 0: // confirm-pending: subFlag==3 → advance
      if (a.subFlag() == 3) {
        ...
        if (n3 == 0xd) {
          confirm_or_advance(a, 0x50, 0x80073F84u);
        } else if (n3 < 0xe) {
          if (n3 == 0xc) {
            confirm_or_advance(a, 0x4e, 0x80073F6Cu);
          }
        } else if (n3 == 0xe) {
          confirm_or_advance(a, 0x4f, 0x80073F9Cu);
        }
      }
      break;
    case 1:
    case 5: { // pick scene-id → Spawn::sceneEntity(sid, 0) → sceneHandle
      int16_t sv4 = 0;
      uint8_t n3 = a.type();
      bool use_table;
      if (n3 != 2) {
    ...
    case 6: { // completion → triggerSub = 2
      if (eng(c).bgSceneTransitionSm.readyForProgress()) {
        a.setTriggerSub(2); // FUN_80042728 (native)
      }
      break;
    }
    }
