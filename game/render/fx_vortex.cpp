...
struct FlareGlow {
  int numer, denom, half, clutPage, lvlBias;
};
...
public:
  ...
  uint32_t next() {
    mSeed = mSeed * 0x41C64E6Du + 0x3039u;
    return (mSeed >> 16) & 0x7FFFu;
  }
...
void Render::a0fVortexRender(uint32_t node) {
  ...
  if (anchor.sz <= 0) {
    return; // behind the camera: the emitter's FLAG check
  }
  ...
  if (penalty < -50) {
    return; // too far off-screen: the emitter drops ALL layers
  }
  if (c->mem_r8(node + kState) == 1) {
    ...
    for (const FlareGlow &f : kFlares) {
      ...
      if (lvl <= 0) {
        continue; // FUN_80114288 draws nothing for a dead level
      }
  ...
  if (!recList) {
    return; // no list -> the emitter emits nothing at all
  }
  if (!SpriteAnchor::otKeyInRange(anchor.sz, bias)) {
  ...
  spriteRecordsEmit(recList,
                    c->mem_r32(node + kClutPage),
                    anchor.px,
                    anchor.py,
                    proj_pz_to_ord(anchor.pz),
                    coreScaleX,
                    coreScaleY,
                    coreIr0,
                    kFarCore);
  ...
  for (int lat = 0; lat < 4096; lat += 512) {
    ...
    for (int j = 0;;) {
      ...
      if (SpriteAnchor::otKeyInRange(pv.sz, 0)) {
        ...
        spriteRecordsEmit(kPartRecs, partClutPage, pv.px, pv.py, proj_pz_to_ord(pv.pz), scale, scale, ir0, kFarSphere);
      }
      j += step;
      if (j + step >= 4097) {
        break;
