...
bool Render::beamNodeReached(uint32_t node) const {
  ...
  if (type >= kW4TableEntries) return false;                 // the walk's own range reject
  const uint32_t arm = c->mem_r32(kW4Table + type * 4u);
  if (arm == kArmBeamMesh) return true;
  if (arm == kArmBeamBillboard) return c->mem_r8(node + kNodeVisFlag) == 1u;
  return false;
}
...
void Render::beamQuadRender(uint32_t node) {
  ...
  if (!anchor) return;                       // no tracked object: the emitter would read address 0x2C
  ...
  const int32_t polar   = (int16_t)c->mem_r16(node + kNodePolar) +
                          (c->mem_r8(kSceneMode) < 4u ? kQuarterTurn : 0);
  const int32_t spanR = trig.rcos(polar) * kHalfWidth;       // the width vector's horizontal radius
  const WorldPt H = { (trig.rcos(azimuth) * spanR) >> 24,
                      (trig.rsin(polar) * kHalfWidth) >> 12,
                      (-trig.rsin(azimuth) * spanR) >> 24 };
  ...
  const WorldPt M = { halveTowardZero(Amid.x + N.x), halveTowardZero(Amid.y + N.y),
                      halveTowardZero(Amid.z + N.z) };
  ...
  for (int s = 0; s < nSpans; s++) {
    ...
    int xs[4], ys[4];
    for (int i = 0; i < 4; i++) {
      xsf[i] = p[i].px; ysf[i] = p[i].py;
      bx0 = bx0 < p[i].px ? bx0 : p[i].px;  bx1 = bx1 > p[i].px ? bx1 : p[i].px;
      by0 = by0 < p[i].py ? by0 : p[i].py;  by1 = by1 > p[i].py ? by1 : p[i].py;
    ...
    rq.emitOrQueue(c, /*capture=*/1, RQ_WORLD, RQ_OM_DEPTH, /*nv=*/4, kSemi ? 1 : 0, kRawTexture ? 1 : 0,
                   ...
                   /*tp_x=*/(int)(kTpageHalf & 0xFu) * 64, /*tp_y=*/(int)((kTpageHalf >> 4) & 1u) * 256,
                   /*clut_x=*/(int)(kClut & 0x3Fu) * 16, /*clut_y=*/(int)((kClut >> 6) & 0x1FFu),
                   0, 0, 0, 0, 0, 0, 1023, 511, /*tp_blend=*/(int)((kTpageHalf >> 5) & 3u));
    emitted++;
  }
  lucent::debug("beamfx", "node={:08X} kind={} split={} uvIdx={} v=({},{}) A=({},{},{}) N=({},{},{}) "
                          "H=({},{},{}) spans={} emitted={} screen=[{:.1f},{:.1f}]..[{:.1f},{:.1f}]",
                node, (int16_t)c->mem_r16(node + kNodeKind), split ? 1 : 0, uvIndex, vNear, vFar,
                A.x, A.y, A.z, N.x, N.y, N.z, H.x, H.y, H.z, nSpans, emitted,
