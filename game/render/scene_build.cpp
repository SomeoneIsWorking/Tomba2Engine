...
static void read_camera(Core *c, float Rcam[3][3], float Tcam[3]) {
  ...
  Rcam[0][0] = (int16_t)k0 / 4096.0f;
  Rcam[0][1] = (int16_t)(k0 >> 16) / 4096.0f;
  Rcam[0][2] = (int16_t)k1 / 4096.0f;
  Rcam[1][0] = (int16_t)(k1 >> 16) / 4096.0f;
  Rcam[1][1] = (int16_t)k2 / 4096.0f;
  Rcam[1][2] = (int16_t)(k2 >> 16) / 4096.0f;
  Rcam[2][0] = (int16_t)k3 / 4096.0f;
  Rcam[2][1] = (int16_t)(k3 >> 16) / 4096.0f;
  Rcam[2][2] = (int16_t)k4 / 4096.0f;
...
int NativeScenePass::collect() {
  ...
  for (int li = 0; li < 3; li++) {
    for (uint32_t n = c->mem_r32(HEADS[li]), g = 0; n >= 0x80000000u && n < 0x80200000u && g < 1024;
      ...
      if ((ri >= 0x10 && ri <= 0x14) || ri == 0x20) {
        continue;
      }
      if (c->mem_r8(n + 8) == 0) {
        continue; // no render commands -> no mesh
      }
      nmesh++;
      ...
      if (sx == 0.0f && sy == 0.0f && sz == 0.0f) {
        sx = sy = sz = 1.0f;
      } // unset scale -> identity
      float Rms[3][3];
      for (int i = 0; i < 3; i++) {
        Rms[i][0] = Rmodel[i][0] * sx;
        Rms[i][1] = Rmodel[i][1] * sy;
        Rms[i][2] = Rmodel[i][2] * sz;
      }
      ...
      for (int i = 0; i < ncmd && out->count < SCENE_MAX_OBJECTS; i++) {
        ...
        if (cmd < 0x80000000u || cmd >= 0x80200000u) {
          continue;
        }
        uint32_t geomblk = c->mem_r32(cmd + 0x40);
        if (geomblk < 0x80000000u || geomblk >= 0x80200000u) {
          continue;
        }
  ...
  if (dbg) {
    if ((mDbgFrame++ % 60) == 0) {
      cfg_logf("rendernative",
               "scene: %d live, %d 3D-mesh nodes, %d geomblk objects (H=%.0f ofx=%.0f)",
               nlive,
               nmesh,
               out->count,
               (double)out->cam.H,
