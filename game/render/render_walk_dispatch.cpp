...
// guest 0x8003C048 keeps its loop's CURRENT NODE and NEXT-NODE pointers LIVE in the real MIPS s0/s1
...
//
// RE method: instruction-exact transcription of authenticated executable/overlay evidence guest 0x8003C048 (the
// recorded binary evidence's translation is ground truth; Ghidra headless was used only to cross-check control flow
...
// (tomba::native::declareOverride, oracle-gated) as the sibling files this loop calls into.
//
// guest 0x8003C048 (no args, guest ABI): the caller passes nothing in r4-r7; the function reads the
// global render-node-list head itself.
//   - prologue: sp -= 112; spill caller's r16 @sp+88, r31 @sp+104, r19 @sp+100, r18 @sp+96,
//     r17 @sp+92 (order per authenticated executable/overlay evidence).
...
//     switch on target (16 known label values + a defensive typed runtime address dispatch-then-return default,
//     matching the recorded binary evidence's indirect-jump fallback convention used throughout this codebase's
...
//     typed runtime address dispatch's it; case's own r31/table-value 0x8003C2AC is a pure no-op (skip) entry.
...
#include <stdio.h>
// guest 0x8003C048 fallback for the oracle-gated thunk. image-qualified runtime dispatcher is process-global (shared by
// SBS core A and core B), so the oracle side MUST keep running the real guest body — see
...
constexpr uint32_t JUMP_TABLE = 0x80014DB8u;       // 33-entry (idx<33) per-node case-target table
                                                   ...
                                                   // against authenticated executable/overlay evidence's neighboring
                                                   // guest 0x8003D0BC (same 32769<<16 base).
...
// Real -112 guest-stack frame (RE: guest 0x8003C048's prologue) — see CLAUDE.md "MIRROR THE GUEST
...
// callee-save prologue/epilogue exactly (order per authenticated executable/overlay evidence,4564-4569).
struct WalkFrame {
  Core *c;
...
// gen (authenticated executable/overlay evidence):
//  - render-mode byte (0x800BF870, the SAME byte perModeDispatch reads) == 4: typed runtime address dispatch straight
...
//    guest 0x8003B054 into THIS frame's own guest-stack scratch (sp+16..), force its primitive-code byte
//    to 45, copy 11 halfwords of node+96..118 vertex/UV data to sp+56..84, bracket with
//    guest 0x80084660/guest 0x80084690 (r4=CASE188_SCR — pool-span open/close markers, the guest-side
//    counterpart), then emit via guest 0x8003B320(sp+16, sp+56, count=16).
void renderWalkCase188(Core *c) {
  ...
  if (c->mem_r8(MODE_BYTE_188) == 4u) {
    c->r[4] = node;
    c->r[31] = 0x8003C1A4u;
    psx::cpu::dispatchGuestToReturn0(*c, 0x8011BE5Cu, psx::cpu::ExecutionBudget::currentTurn(*c), __func__);
    return;
  }
  ...
  c->r[4] = c->r[18];
  c->r[31] = 0x8003C27Cu;
  psx::cpu::dispatchGuestToReturn0(*c, 0x80084660u, psx::cpu::ExecutionBudget::currentTurn(*c), __func__);
  c->r[4] = c->r[18];
  c->r[31] = 0x8003C284u;
  psx::cpu::dispatchGuestToReturn0(*c, 0x80084690u, psx::cpu::ExecutionBudget::currentTurn(*c), __func__);
  c->r[4] = sp + 16u;
  c->r[5] = sp + 56u;
  c->r[6] = 16u;
  c->r[31] = 0x8003C294u;
  psx::cpu::dispatchGuestToReturn0(*c, 0x8003B320u, psx::cpu::ExecutionBudget::currentTurn(*c), __func__);
}
} // namespace
    ...
    if (c->mem_r8(node + 1u) != 0u) {
      ...
      if (idxByte < 33u) {
        ...
        case 0x8003C0C4u:
          ...
          c->r[5] = 0u;
          c->r[31] = 0x8003C0D0u;
          psx::cpu::dispatchGuestToReturn0(*c, 0x8003F174u, psx::cpu::ExecutionBudget::currentTurn(*c), __func__);
          break;
        case 0x8003C0D8u:
          c->r[4] = c->r[16];
          c->r[31] = 0x8003C0E0u;
          psx::cpu::dispatchGuestToReturn0(*c, 0x8003EF9Cu, psx::cpu::ExecutionBudget::currentTurn(*c), __func__);
          break;
        case 0x8003C0E8u:
          c->r[4] = c->r[16];
          c->r[31] = 0x8003C0F0u;
          psx::cpu::dispatchGuestToReturn0(*c, 0x80039F4Cu, psx::cpu::ExecutionBudget::currentTurn(*c), __func__);
          break;
        case 0x8003C0F8u:
          c->r[4] = c->r[16];
          c->r[31] = 0x8003C100u;
          psx::cpu::dispatchGuestToReturn0(*c, 0x8012A43Cu, psx::cpu::ExecutionBudget::currentTurn(*c), __func__);
          break;
        case 0x8003C108u:
          c->r[4] = c->r[16];
          c->r[31] = 0x8003C110u;
          psx::cpu::dispatchGuestToReturn0(*c, 0x801295B4u, psx::cpu::ExecutionBudget::currentTurn(*c), __func__);
          break;
        case 0x8003C118u:
          c->r[4] = c->r[16];
          c->r[31] = 0x8003C120u;
          psx::cpu::dispatchGuestToReturn0(*c, 0x80129114u, psx::cpu::ExecutionBudget::currentTurn(*c), __func__);
          break;
        case 0x8003C128u:
          c->r[4] = c->r[16];
          c->r[31] = 0x8003C130u;
          psx::cpu::dispatchGuestToReturn0(*c, 0x8013DD58u, psx::cpu::ExecutionBudget::currentTurn(*c), __func__);
          break;
        case 0x8003C138u: {
          ...
          c->r[4] = node;
          c->r[31] = 0x8003C140u;
          psx::cpu::dispatchGuestToReturn0(*c, 0x800726D4u, psx::cpu::ExecutionBudget::currentTurn(*c), __func__);
        ...
        default:
          ...
          // evidence: 4446's `default: typed runtime address dispatch(c, c->r[2]); return;`) — a full RETURN from the
          // whole function, not a loop-continue, exactly like perObjRenderDispatch's and billboardEmit's own default
          // cases. Never hit by live game data: the live 33-slot table only ever holds the case values enumerated
          // above.
          c->r[4] = c->r[16];
...
void render_walk_dispatch_install() {
  ...
  tomba::native::declareOverride(0x8003C048u, "ov_renderWalk", ov_renderWalk);
}
