...
// itself, guest 0x80081FB0 at 0x80081FB0, turned out on close RE to be ~147 gen-C lines with a genuinely
...
// re-diffed line-by-line against authenticated executable/overlay evidence (one bug found+fixed, see BUG FIX comment
// inside nativePutDrawEnv) and installed via gpu_putdrawenv_install() -> tomba::native::declareOverride
// (game_tomba2.cpp). guest 0x80081FB0 stays MAPPED-not-drafted/substrate as documented below; PutDrawEnv
// reaches it via typed runtime address dispatch, which is correct with or without a future draft of it.
//
// ------------------------------------------------------------------------------------------------
...
//   1. `guest 0x80080F6C` (DrawSync) AND `guest 0x80081458` (ClearOTagR) both had an INVERTED branch
...
//      the same shape (authenticated executable/overlay evidence) and is drafted with the correct >=2 polarity.
//      Consequence: the hook is a steady-state "GPU sys is up" hook, not a boot-time init.
//   2. `guest 0x80081458`'s dummy-tail-packet constants were 0x800A5B20/0x800A5B0C; the gen-C decimals
...
// (consumed by nativeTextureWindow, see below), +20 = rgbBits (u16), +22 = dither/rgb-present flag (u8),
// +23 = tpage modeFlag (u8), +24 = "clear/background enabled" flag (u8, gates guest 0x80081FB0's
...
// ClearOTagR, called when bootFlag>=2 — see polarity note above). (2) guest 0x80081FB0(dst=drawEnvPtr+28,
...
// guest 0x8009A3E0 — caches the DRAWENV as the "current" env for later reference (classic libgpu
// PutDrawEnv semantic: remembers the last env set).
//
// SUPERSEDED (2026-07-30): guest 0x80081FB0 IS NOW PORTED — game/render/libgpu_draw_env.cpp, class
...
//
// MAPPED, NOT DRAFTED at the time this file was written: **guest 0x80081FB0** (0x80081FB0). RE'd its full structure
// from authenticated executable/overlay evidence (147 gen-C lines, frame -40, spills ra/s0/s1 == r16/r17):
//   - Calls the 4 leaves DRAFTED in this file below, in order: nativeClipTopLeft(x0,y0)->*(dst+4)
//     [SetDrawAreaTopLeft], nativeClipBottomRight(x1,y1)->*(dst+8) [SetDrawAreaBottomRight, x1/y1 computed as
//     x0+w-1/y0+h-1], nativeDrawOffset(offX,offY)->*(dst+12) [SetDrawingOffset], nativeDrawMode(modeFlag,
//     ditherFlag,rgbBits)->*(dst+16) [DR_TPAGE mode word — the SAME computation guest 0x80083DE0
//     (already drafted, wide_re_libgpu_leaves.cpp) inlines for its own mode word; independently
//     re-derived and confirmed byte-identical this session], nativeTextureWindow(texWinPtr)->*(dst+20)
//     [DR_TWIN word — SAME computation as guest 0x80083DE0's tail; the two functions' differing
...
//     `typed runtime address dispatch(c, 0x80081FB0)`, staying correct regardless of when/whether it's later drafted.
...
// nativeClipTopLeft (0x80082240) — SetDrawAreaTopLeft(x,y) word builder. DRAFT. RE'd from
// authenticated executable/overlay evidence guest 0x80082240 (37 gen-C ln, no calls, no branches beyond the clip, no
// stack frame — true leaf). Guest ABI: a0(r4)=x, a1(r5)=y (both s16). Clamps each to [0, clipMax-1] against
// GPU_CLIP_MAXW/MAXH (the SAME globals guest 0x80082734's rect clip uses — offsets 22948/22950),
// negative values clamp to 0. Returns 0xE3000000 (GP0(0xE3) SetDrawAreaTopLeft tag) |
// (clampedY&1023)<<10 | (clampedX&1023).
static void nativeClipTopLeft(Core *c) {
...
// nativeClipBottomRight (0x800822D8) — SetDrawAreaBottomRight(x,y) word builder. DRAFT. RE'd from
// authenticated executable/overlay evidence guest 0x800822D8 (37 gen-C ln). IDENTICAL shape/clamp logic to
// nativeClipTopLeft above (same GPU_CLIP_MAXW/MAXH globals), only the tag differs: 0xE4000000 (GP0(0xE4)
// SetDrawAreaBottomRight) instead of 0xE3000000.
static void nativeClipBottomRight(Core *c) {
  int32_t x = (int16_t)c->r[4];
  int32_t y = (int16_t)c->r[5];
...
// nativeDrawOffset (0x80082370) — SetDrawingOffset(x,y) word builder. DRAFT. RE'd from
// authenticated executable/overlay evidence guest 0x80082370 (9 gen-C ln, fully self-contained, no clamp — offsets are
// signed 11-bit HW fields, no range-check in the guest-visible behavior). Guest ABI: a0(r4)=x, a1(r5)=y. Returns
// 0xE5000000 (GP0(0xE5) SetDrawingOffset) | (y&2047)<<11 | (x&2047).
static void nativeDrawOffset(Core *c) {
  uint32_t x = c->r[4] & 2047u;
  uint32_t y = c->r[5] & 2047u;
  c->r[2] = 0xE5000000u | (y << 11) | x;
}

// nativeDrawMode (0x80082220) — DR_TPAGE mode-word builder. DRAFT. RE'd from authenticated executable/overlay evidence
// guest 0x80082220 (10 gen-C ln, fully self-contained, no stack frame). This is the STANDALONE
// version of the SAME computation guest 0x80083DE0 (already drafted, wide_re_libgpu_leaves.cpp) inlines
...
// (Caller guest 0x80081FB0 passes a0=*(drawEnv+23), a1=*(drawEnv+22), a2=*(drawEnv+20).)
static void nativeDrawMode(Core *c) {
...
// nativeTextureWindow (0x8008238C) — DR_TWIN word builder. DRAFT. RE'd from authenticated executable/overlay evidence
// guest 0x8008238C (32 gen-C ln, frame -16 for 4 words of LOCAL scratch that are written but never
...
// aligned struct {u8 maskX, u8 maskY, s16 offX, s16 offY} (same shape guest 0x80083DE0 reads). Returns 0
// if texWinSrc==0; else 0xE2000000 (GP0(0xE2) DR_TWIN) | (maskY>>3)<<15 | (maskX>>3)<<10 |
// ((-offY)&0xFF)>>3<<5 | ((-offX)&0xFF)>>3 — algebraically IDENTICAL to guest 0x80083DE0's DR_TWIN tail
// (verified: `((x&0xFF)>>3)<<5 == (x<<2)&0x3E0` for the Y-offset term, since both formulas only
// depend on bits[3..7] of x).
static void nativeTextureWindow(Core *c) {
...
// nativePutDrawEnv (0x800815D0) — libgpu PutDrawEnv(drawEnvPtr). DRAFT. RE'd from authenticated executable/overlay
// evidence guest 0x800815D0 (lines 15851-15895 in the gen-C — the shard groups an unrelated adjacent guest function's
// prologue/epilogue directly after this one's `return`, same recorded binary evidence shard-grouping artifact
// ClearOTagR's comment already flags; NOT ported here, reachable only via its own callers). See file header for full
// field-level RE, control flow, ABI, and the MAPPED-not-drafted note on guest 0x80081FB0. Frame -32, spills ra/s2/s1/s0
// == r18/r17/r16.
//
...
// dispatched SUBSTRATE callees (guest 0x80081FB0 spills s0/s1; the table+0x08 DMA-send fn — e.g.
// GpuDmaQueueEnqueue 0x80082D04 — spills s0..s3; guest 0x8009A3E0) spill the CALLER's s-register values
...
static void nativePutDrawEnv(Core *c) {
  c->r[29] -= 32;
  ...
  psx::cpu::dispatchGuestToReturn0(*c,
                                   ...
                                   __func__); // guest 0x80081FB0, MAPPED not drafted — see file header
  ...
  // base, not the table itself — gen dereferences it TWICE (authenticated executable/overlay evidence:
  ...
  c->r[7] = 0u;
  c->r[31] = 0x80081664u;
  psx::cpu::dispatchGuestToReturn0(*c, dmaSendFn, psx::cpu::ExecutionBudget::currentTurn(*c), __func__);
  ...
  psx::cpu::dispatchGuestToReturn0(*c,
                                   FN_MEMCPY_92,
                                   psx::cpu::ExecutionBudget::currentTurn(*c),
                                   __func__); // guest 0x8009A3E0, out-of-band shared primitive, not drafted

  epilogue(c->r[17]);
}

// ------------------------------------------------------------------------------------------------
// WIRING (verify pass, 2026-07-10): re-diffed every line of nativePutDrawEnv and the 4 leaf builders
// above against authenticated executable/overlay evidence per fleet-workflow.md §9. One real bug found+fixed (see the
// BUG FIX comment inside nativePutDrawEnv): GPU_SYS_TABLE double-deref, same missing-indirection shape already found in
// DrawSync/ClearOTagR (wide_re_libgpu_leaves.cpp). All 5 addresses are PLAIN intra-shard C calls (the cited guest
// address(c), not typed runtime address dispatch) at their call sites in authenticated executable/overlay evidence, so
// they wire via the oracle-gated tomba::native::declareOverride thunk (same shape as gpu_libgpu_leaves_install) — this
// keeps SBS core B running the pure original guest instructions body.
...
void gpu_putdrawenv_install() {
  ...
  tomba::native::declareOverride(0x800815D0u, "nativePutDrawEnv", nativePutDrawEnv);
  tomba::native::declareOverride(0x80082240u, "nativeClipTopLeft", nativeClipTopLeft);
  tomba::native::declareOverride(0x800822D8u, "nativeClipBottomRight", nativeClipBottomRight);
  tomba::native::declareOverride(0x80082370u, "nativeDrawOffset", nativeDrawOffset);
  tomba::native::declareOverride(0x80082220u, "nativeDrawMode", nativeDrawMode);
  tomba::native::declareOverride(0x8008238Cu, "nativeTextureWindow", nativeTextureWindow);
}
