...
public:
  ...
  int32_t anchorZ() const {
    return s16(0x30u);
  }
  int32_t prevBaseX() const {
    return s16(0x48u);
  } // last frame's cube base — the wrap reference
  int32_t prevBaseY() const {
    return s16(0x4Au);
  }
  int32_t prevBaseZ() const {
    return s16(0x4Cu);
  }
...
void Render::fxMoteStreakRender(uint32_t node) {
  ...
  struct Head {
    float x, y;
    bool ok;
  } head[MoteFrame::kMotes];

  for (int i = 0; i < MoteFrame::kMotes; i++) {
    // Three steps per mote, each axis reading the seed BEFORE its replacement.
    const int gx = (int32_t)s >> 16;
    s = s * mul + 1u;
    const int gy = (int32_t)s >> 16;
    s = s * mul + 1u;
    const int gz = (int32_t)s >> 16;
    s = s * mul + 1u;

    const int ax = gx + baseX, ay = gy + baseY, az = gz + baseZ; // unmasked, for the wrap test
    ProjVtx pv;
    cam.project((int16_t)(ax & (kLattice - 1)), (int16_t)(ay & (kLattice - 1)), (int16_t)(az & (kLattice - 1)), &pv);

    head[i].ok = false;
    if (pv.sz <= 0 || !kGateNoPreClamp(pv.sz)) {
      continue;
    }
  ...
  if (prev) {
    for (int i = 0; i < MoteFrame::kMotes; i++) {
      if (!head[i].ok || !prev->valid[i]) {
        continue; // the guest's sentinel case: seed history only
      }
      ...
      if (len < 1e-4f) {
        continue; // zero-length: the GPU would draw nothing
      }
  ...
  if (cfg_dbg("fxmote")) {
    cfg_logf("fxmote",
             "motes node=%08X base=(%d,%d,%d) wrapped=%d streaks=%d/%d prev=%s",
             node,
             baseX,
             baseY,
             baseZ,
