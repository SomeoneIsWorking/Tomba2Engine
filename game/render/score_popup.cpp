...
void ScorePopup::collect(Core *c, const UiGroupArgs &a) {
  ...
  if (!popup.mInDraw) {
    return; // every other caller owns its own producer
  }
...
void ScorePopup::drawCollected() {
  ...
  for (int i : order) {
    ...
    if (a.sprite) {
      ...
      rend(c)->emitUiFt4(a.x, a.y, a.wOv, a.hOv, a.templPtr, a.dataBase, a.attrByte, a.clutSemi, RQ_OVERLAY);
    }
  }
  mGroups.clear();
}
