...
int Render::dustTrailEmit(const EffectPoints &pts, const EObjXform &cam, int sub) {
  ...
  for (int i = 0; i < kRingPoints; i++) {
    ...
    if (pts.valid[i]) {
      ProjVtx pv;
      cam.project(pts.x[i], pts.y[i], pts.z[i], &pv);
      if (trailPointVisible(pv.sz)) {
        cur.valid = true;
        cur.x = pv.px;
        cur.y = pv.py;
        cur.ord = proj_pz_to_ord(pv.pz);
    ...
    if (i > 0 && cur.valid && prev.valid) {
      ...
      const int32_t seg = trigOf(c).ratan2((int32_t)(cur.y - prev.y), (int32_t)(cur.x - prev.x)) + kQuarterTurn;
      ...
      for (int side = 0; side < 2; side++) {
        ...
        float xsf[4], ysf[4];
        for (int k = 0; k < 4; k++) {
          xsf[k] = pxf[k] + (float)offX;
          ysf[k] = pyf[k] + (float)offY;
        ...
        if (cfg_dbg("fxdust")) {
          cfg_logf("fxdust",
                   "  trail seg=%d side=%d v0=(%d,%d) v1=(%d,%d) v2=(%d,%d) v3=(%d,%d)",
                   i,
                   side,
                   xs[0],
                   ys[0],
...
void Render::dustPuffEmit(uint32_t node, const EffectPoints &pts, int sub) {
  ...
  const int32_t farColour[3] = {
      (int32_t)c->mem_r8(fc + 0u) << 4, (int32_t)c->mem_r8(fc + 1u) << 4, (int32_t)c->mem_r8(fc + 2u) << 4};
  const float Tobj[3] = {(float)pts.x[kPuffPoint], (float)pts.y[kPuffPoint], (float)pts.z[kPuffPoint]};

  int32_t base[3][3];
  if (lowSub) {
    MeshQuads::rotmat(c, 0, c->mem_r16s(owner + kOwnerYaw), roll, base);
  } else {
    MeshQuads::fromGuest(c, owner + kOwnerMatrix, base);
  }
  ...
  for (int i = 0; i < 4; i++) {
    ...
    int32_t axis[3][3];
    if (lowSub) {
      MeshQuads::rotY(c, spin, axis);
    } else {
      MeshQuads::rotZ(c, spin, axis);
    }
    float Robj[3][3];
...
void Render::dustEffectRender(uint32_t node) {
  ...
  if (cfg_dbg("fxdust")) {
    cfg_logf("fxdust",
             ...
             (int)state,
             (unsigned)age,
             (int)c->mem_r8(node + kAgeIdx),
             c->mem_r32(node + kOwner),
             c->mem_r16s(node + kPosX),
             c->mem_r16s(node + kPosX + 2u),
             c->mem_r16s(node + kPosX + 4u),
             segments,
             (int)puff);
  }
}
