...
void makeXform(EObjXform* x) {
  const float R[3][3] = {{ 4096, -300,  700},
                         {  150, 3900, -820},
                         { -640,  510, 3300}};
  for (int i = 0; i < 3; i++) for (int j = 0; j < 3; j++) x->R[i][j] = R[i][j];
  x->T[0] = 1234; x->T[1] = -5678; x->T[2] = 90123;
  x->ofx = 160; x->ofy = 120; x->H = 350;
}
...
void writePacked(Core* c, const uint32_t cr[11]) {
  for (int i = 0; i < 5; i++) c->mem_w32(kSceneViewMatrix + (uint32_t)i * 4u, cr[i]);
  for (int i = 0; i < 3; i++) c->mem_w32(kSceneViewMatrix + 0x14u + (uint32_t)i * 4u, cr[5 + i]);
}
...
int run_sceneview_selftest(const char* /*exePath*/) {
  ...
  for (int i = 0; i < 3; i++) for (int j = 0; j < 3; j++) {
    if (std::fabs(R[i][j] - x.R[i][j]) > 0.5f) {
      lucent::error("sceneview", "R[{}][{}] round-tripped to {:.1f}, packed from {:.1f} — the decoder "
                    "and projActiveCr disagree about this halfword", i, j, R[i][j], x.R[i][j]);
      bad++;
    }
  }
  for (int i = 0; i < 3; i++) {
  ...
  if (std::fabs(wrongR22 - x.R[2][2]) <= 0.5f) {
    lucent::error("sceneview", "NEGATIVE CONTROL FAILED: the wrong halfword pick for R22 yields {:.1f}, "
                  "which equals the correct {:.1f}. This test cannot distinguish the bug it exists for — "
                  "choose an R22 whose two halfwords differ.", wrongR22, x.R[2][2]);
    bad++;
  }
  ...
  lucent::info("sceneview", "OK: 9 rotation + 3 translation elements round-trip pack->unpack exactly; "
