...
struct RedirCensus {
  ...
  void flush() {
    ...
    cfg_logf("redirdiag",
             ...
             drew,
             gate_off,
             off_reason[FN_ORACLE],
             off_reason[FN_PSXRENDER],
             off_reason[FN_STAGE],
             off_reason[FN_NARRATION],
             off_reason[FN_SUBSCENE],
             covered,
             em_overlay,
             em_generic,
             em_other,
    ...
    if (geomblk != 0) {
      ...
      if (cfg_dbg("redirdiag")) {
        ...
        if (!fieldNative) {
          g_redir.gate_off++;
          if (fnReason < 8) {
            g_redir.off_reason[fnReason]++;
          }
        }
        if (cmdEmitter == 0x80146478u) {
          g_redir.em_overlay++;
        } else if (cmdEmitter == GENERIC_EMITTER) {
          g_redir.em_generic++;
        } else {
          g_redir.em_other++;
          if (!g_redir.other_top) {
            g_redir.other_top = cmdEmitter;
          }
        }
      }

      bool nodeNativeCovered = fieldNative && rend(c)->nativeObjDrawn(c, node);
      if (cfg_dbg("redirdiag") && nodeNativeCovered) {
        g_redir.covered++;
      }
      ...
      if (fieldNative) {
        ...
        if (redirectGeneric && !nodeNativeCovered) {
          ...
          if (cfg_dbg("redirect")) {
            ...
            if (n++ % 256 == 0) {
              cfg_logf("redirect",
                       "cmdListDispatch node=%08X cmd=%08X geomblk=%08X otbase=%08X",
                       node,
                       cmd,
                       geomblk,
                       otbase);
          ...
          if (cfg_dbg("redirdiag")) {
            ...
            if (emitted == 0) {
              g_redir.drew_empty++;
              if (!g_redir.empty_node) {
                g_redir.empty_node = node;
              }
            }
          }
