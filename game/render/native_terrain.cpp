...
static inline void trig_lut(Core *c, int32_t angle, int *s, int *co) {
  ...
  *co = (int)((int32_t)word >> 16);
  uint32_t at = (word << 16);
  at = (at + (uint32_t)sign) ^ (uint32_t)sign;
  *s = (int)(int16_t)(at >> 16);
}
...
static void terrain_obj_matrix_host(Core *c, uint32_t node, int16_t m[9]) {
  ...
  m[1] = (int16_t)-szcy;
  m[2] = (int16_t)sy;
  m[3] = (int16_t)(cz_sxsy + cxsz);
  m[4] = (int16_t)(cxcz - sz_sxsy);
  m[5] = (int16_t)-cysx;
  m[6] = (int16_t)(sxsz - cz_cxsy);
  m[7] = (int16_t)(sxcz + sz_cxsy);
  m[8] = cycx;
...
void NativeScenePass::terrainRender() {
  ...
  int16_t mobj[9];
  terrain_obj_matrix_host(c, node, mobj);
  float Robj[3][3];
  for (int row = 0; row < 3; row++) {
    for (int col = 0; col < 3; col++) {
  ...
  for (int rr = 0; rr < 3; rr++) {
    for (int cc = 0; cc < 3; cc++) {
      Rcam[rr][cc] = Rc_i16[rr][cc] / 4096.0f;
    }
  }
  ...
  for (uint32_t rec = TERRAIN_GEOMBLK;; rec += 36) {
    ...
    for (int kk = 0; kk < 4; kk++) {
      ...
      wv[kk][0] = w0;
      wv[kk][1] = w1;
      wv[kk][2] = w2; // view-space pos (for the per-face light normal)
      float pz = w2;
      if (pz < nearp) {
        pz = nearp;
      }
    ...
    u[0] = uv0 & 0xFF;
    v[0] = (uv0 >> 8) & 0xFF;
    u[1] = (uint32_t)ctl & 0xFF;
    v[1] = ((uint32_t)ctl >> 8) & 0xFF;
    u[2] = uv2 & 0xFF;
    v[2] = (uv2 >> 8) & 0xFF;
    u[3] = (uv2 >> 16) & 0xFF;
    v[3] = (uv2 >> 24) & 0xFF;
    ...
    if (mm.light && !semi) {
      ...
      if (ln > 1e-3f) {
        ...
        for (int kk = 0; kk < 4; kk++) {
          ...
          r[kk] = (uint8_t)(rr > 255 ? 255 : rr);
          g[kk] = (uint8_t)(gg > 255 ? 255 : gg);
          b[kk] = (uint8_t)(bb > 255 ? 255 : bb);
        }
      }
    }
    if (s_dbg && drawn < 4) {
      cfg_logi("terrpc",
               "rec=%08x v0=(%.1f,%.1f z%.3f) v2=(%.1f,%.1f) tp=%04x clut=%04x semi=%d",
               rec,
               px[0],
               py[0],
               depth[0],
    ...
    drawn++;
    if (ctl <= 0) {
      break; // control sign marks the last record
    }
  }
  c->rsub.diag.endObject();
  if (s_dbg) {
    cfg_logi("terrpc", "node=%08x drew %d quads (H=%u ofx=%.1f ofy=%.1f)", node, drawn, (unsigned)H, ofx, ofy);
  }
}
