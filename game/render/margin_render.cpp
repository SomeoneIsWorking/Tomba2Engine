...
static void margin_rotpair_f(float angleUnits, float m[3][3], int rowA, int rowB, int posSin) {
  ...
  for (int i = 0; i < 3; i++) {
    m[rowA][i] = co * A[i] - s * B[i];
  }
  for (int i = 0; i < 3; i++) {
    m[rowB][i] = s * A[i] + co * B[i];
  }
}
static inline void margin_rotX(float angle, float m[3][3]) {
  margin_rotpair_f(angle, m, 1, 2, +1);
} // FUN_80084D10
static inline void margin_rotY(float angle, float m[3][3]) {
  margin_rotpair_f(angle, m, 0, 2, -1);
} // FUN_80084EB0
static inline void margin_rotZ(float angle, float m[3][3]) {
  margin_rotpair_f(angle, m, 0, 1, +1);
} // FUN_80085050
...
static void margin_build_host_transforms(Core *c, uint32_t node, std::vector<MarginCmdXform> &out) {
  ...
  for (uint32_t i = 0; i < count && i < bound; i++) {
    ...
    float anchor[3] = {(float)c->mem_r16s(cmd + 0x00), (float)c->mem_r16s(cmd + 0x02), (float)c->mem_r16s(cmd + 0x04)};
    ...
    if (sentinel == -1) {
      ...
      x.Tobj[0] = Tanchor[0] + Tnode[0];
      x.Tobj[1] = Tanchor[1] + Tnode[1];
      x.Tobj[2] = Tanchor[2] + Tnode[2];
    } else {
      ...
      if ((uint32_t)sentinel >= i || !out[sentinel].valid) {
        x.valid = false;
        continue;
      }
      ...
      x.Tobj[0] = Tanchor[0] + p.Tobj[0];
      x.Tobj[1] = Tanchor[1] + p.Tobj[1];
      x.Tobj[2] = Tanchor[2] + p.Tobj[2];
    }
    x.valid = true;
  }
}
...
void MarginRenderer::collect(Core *c, uint32_t node) {
  ...
  if (c->mem_r8(node + 0xc) != T2_WORLDGEO_TYPE) {
    return; // only world-geometry objects render here
  }
  if (!seen_.insert(node).second) {
    return; // already collected this frame
  }
...
void MarginRenderer::flush(Core *c) {
  ...
  for (uint32_t node : nodes_) {
    if (dbg_ && gpu_frame_no(c) == 2900) {
      cfg_logi("margin",
               "  node=%08x type=%02x cnt=%u",
               node,
               (unsigned)c->mem_r8(node + 0xc),
               (unsigned)c->mem_r8(node + 8));
    }
    if (c->mem_r8(node + 8) == 0 || c->mem_r8(node + 9) == 0) {
      continue; // no render commands (perObjFlush parity)
    }
    ...
    for (uint32_t i = 0; i < cnt; i++) {
      ...
      for (int r = 0; r < 3; r++) {
        for (int col = 0; col < 3; col++) {
          Robj4096[r][col] = xf[i].Robj[r][col] * 4096.0f;
        }
      }
      EObjXform w;
