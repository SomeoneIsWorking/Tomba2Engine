...
struct FxSwarmNode {
  ...
  uint32_t sizeClassTag()  const { return mCore->mem_r8(mBase + fxswarm::kSizeClass); }
  int32_t  otBias()        const { return mCore->mem_r16s(mBase + fxswarm::kOtBias); }
  uint32_t recordHead()    const { return mCore->mem_r32(mBase + fxswarm::kRecordHead); }
  uint32_t clut()          const { return mCore->mem_r16(mBase + fxswarm::kClut); }
  uint32_t texturePage()   const { return mCore->mem_r16(mBase + fxswarm::kTexturePage); }
  int32_t  particleCount() const { return mCore->mem_r16s(mBase + fxswarm::kParticleCount); }
  uint32_t particleArray() const { return mBase + fxswarm::kParticleArray; }
};
