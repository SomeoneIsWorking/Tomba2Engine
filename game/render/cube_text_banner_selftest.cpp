...
void writeMatrix(Core* c, uint32_t at, const int16_t m[9], const int32_t t[3]) {
  ...
  for (int i = 0; i < 3; i++) c->mem_w32(at + 0x14u + (uint32_t)i * 4u, (uint32_t)t[i]);
}
...
void seedBanner(Core* c) {
  for (uint32_t a = NODE; a < NODE + 0x200u; a += 4) c->mem_w32(a, 0);
  ...
  for (int i = 0; i < 3; i++) c->mem_w16(NODE + 0x54u + (uint32_t)i * 2u, 0);   // node eulers

  // One GT4 plank prim per record: counts word = (nGT4 << 16) | nGT3.
  for (uint32_t a = GEOMBLK; a < GEOMBLK + 0x100u; a += 4) c->mem_w32(a, 0);
  ...
  for (int i = 0; i < kGlyphs; i++) {
    ...
    for (uint32_t a = rec; a < rec + 0x44u; a += 4) c->mem_w32(a, 0);
  ...
  for (uint32_t i = 0; i < sizeof(kText); i++) c->mem_w8(TEXT + i, (uint8_t)kText[i]);
...
struct Shot { std::vector<float> xy; };
...
float maxDelta(const Shot& a, const Shot& b) {
  if (a.xy.size() != b.xy.size()) return -1.0f;
  float worst = 0;
...
int run_cubetext_selftest(const char* path) {
  ...
  lucent::info("cubetesttest", "camera A -> {} prims / {} verts; camera B -> {} verts",
               game->rq.n, verts, camB.xy.size() / 2);
  ...
  if (verts != 40) {
    lucent::error("cubetesttest", "expected 40 verts (6 planks + 4 letters, one record is a SPACE), got {}",
                  verts);
    return 1;
  }
  ...
  if (ctrl <= 1.0f) {
    lucent::error("cubetesttest",
                  ...
                  "this test to mean anything.", ctrl);
    return 1;
  }
