...
uint16_t packDelta(int8_t b) {
  return (uint16_t)((uint8_t)b << 8);
}
...
enum : uint32_t {
  kVtxScratch_X0 = 32,
  kVtxScratch_Y0 = 34,
  kVtxScratch_Z0 = 36, // -> GTE VXY0 (sp+32), VZ0 (sp+36)
  kVtxScratch_X1 = 40,
  kVtxScratch_Y1 = 42,
  kVtxScratch_Z1 = 44, // -> GTE VXY1 (sp+40), VZ1 (sp+44)
  kVtxScratch_X2 = 48,
  kVtxScratch_Y2 = 50,
  kVtxScratch_Z2 = 52, // -> GTE VXY2 (sp+48), VZ2 (sp+52)
  kVtxScratch_X3 = 56,
  kVtxScratch_Y3 = 58,
  kVtxScratch_Z3 = 60, // -> GTE VXY3 (sp+56), VZ3 (sp+60)
};
...
struct MarginQuadRecord {
  ...
  uint32_t uv0ClutBase() const {
    return c->mem_r32(addr + 0);
  } // -> pool uv0|clut (+ node clut bias)
  uint32_t planeWord() const {
    return c->mem_r32(addr + 4);
  } // dual-purpose: continuation flag
    // (tested as signed >0) AND, masked
    // to kPlanePayloadMask, the uv1|tpage payload.
  uint32_t uv2Word() const {
    return c->mem_r32(addr + 8);
  } // -> pool uv2 raw; its high 16 bits
    // (sign-extended) also become pool uv3.
  uint32_t colorCode0Src() const {
    return c->mem_r32(addr + 12);
  } // -> pool rgb0|code (masked+re-tagged)
  uint32_t rgb1Raw() const {
    return c->mem_r32(addr + 16);
  } // -> pool rgb1, raw copy
  uint32_t rgb2Raw() const {
    return c->mem_r32(addr + 20);
  } // -> pool rgb2, raw copy
  uint32_t rgb3Raw() const {
    return c->mem_r32(addr + 24);
  } // -> pool rgb3, raw copy

  int8_t z0() const {
    return (int8_t)c->mem_r8(addr + 15);
  }
  int8_t z1() const {
    return (int8_t)c->mem_r8(addr + 19);
  }
  int8_t z2() const {
    return (int8_t)c->mem_r8(addr + 23);
  }
  int8_t z3() const {
    return (int8_t)c->mem_r8(addr + 27);
  }

  int8_t x0() const {
    return (int8_t)c->mem_r8(addr + 28);
  }
  int8_t x1() const {
    return (int8_t)c->mem_r8(addr + 29);
  }
  ...
  int8_t y0AndFogByte() const {
    return (int8_t)c->mem_r8(addr + 30);
  }
  int8_t y1() const {
    return (int8_t)c->mem_r8(addr + 31);
  }
  int8_t x2() const {
    return (int8_t)c->mem_r8(addr + 32);
  }
  int8_t x3() const {
    return (int8_t)c->mem_r8(addr + 33);
  }
  int8_t y2() const {
    return (int8_t)c->mem_r8(addr + 34);
  }
  int8_t y3() const {
    return (int8_t)c->mem_r8(addr + 35);
  }

  bool continues() const {
    return (int32_t)planeWord() > 0;
  }
};
...
void WidescreenMarginQuad::emit(Core *c) {
  ...
  obj = c->r[4];
  GuestReg<17> pool(c);
  pool = c->mem_r32(kPktPoolPtr);
  GuestReg<20> zScratch(c);
  zScratch = kGteFlagScratch;
  GuestReg<22> flagScratch(c);
  flagScratch = kGteFlagScratch;
  ...
  GuestReg<19> recArrayField(c);
  recArrayField = (uint32_t)obj + 80;
  GuestReg<21> clutBiasLow(c);
  clutBiasLow = (uint32_t)c->mem_r8((uint32_t)obj + 7) << 5;
  GuestReg<16> node(c);
  node = c->mem_r32((uint32_t)obj + 60);
  ...
  lucent::debug("wmq",
                "obj={:08X} pool={:08X} recArrayPtr={:08X} node={:08X}",
                (uint32_t)obj,
                (uint32_t)pool,
                recAddr,
                (uint32_t)node);
  ...
  for (;;) {
    ...
    if ((int32_t)c->mem_r32(zScratch) < 0) {
      goto skipRecord;
    }
    ...
    if ((int32_t)c->mem_r32(flagScratch) < 0) {
      goto skipRecord;
    }
    ...
    if (objRenderKind != 1 && objRenderKind != 3) {
      if ((objRenderSubKind >= 3 && objRenderSubKind < 6) || (objRenderSubKind >= 9 && objRenderSubKind < 11)) {
