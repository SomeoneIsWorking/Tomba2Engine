...
static inline PVtx project(const SceneObject *o, const SceneCamera *cam, float H, float nearp, int mx, int my, int mz) {
...
int NativeScenePass::drawObject(const SceneObject *o, const SceneCamera *cam) {
  ...
  for (int q = 0; q < n4; q++, rec += 44) {
    ...
    int v[4] = {
        (int)((uv0 >> 8) & 0xFF), (int)((uv1 >> 8) & 0xFF), (int)((uv23 >> 8) & 0xFF), (int)((uv23 >> 24) & 0xFF)};
    c->game->rq.drawWorldQuad(c, px, py, dp, u, v, r, g, b, tp, clut, 0, nullptr);
    drawn++;
  }
  return drawn;
