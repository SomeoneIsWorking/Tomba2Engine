...
struct DrawEnvFields {
  ...
  int32_t  clipX()      const { return mCore->mem_r16s(mBase + drawenv::kClipX); }
  int32_t  clipY()      const { return mCore->mem_r16s(mBase + drawenv::kClipY); }
  int32_t  clipW()      const { return mCore->mem_r16s(mBase + drawenv::kClipW); }
  uint32_t clipXRaw()   const { return mCore->mem_r16(mBase + drawenv::kClipX); }
  uint32_t clipYRaw()   const { return mCore->mem_r16(mBase + drawenv::kClipY); }
  uint32_t clipWRaw()   const { return mCore->mem_r16(mBase + drawenv::kClipW); }
  uint32_t clipHRaw()   const { return mCore->mem_r16(mBase + drawenv::kClipH); }
  int32_t  offsetX()    const { return mCore->mem_r16s(mBase + drawenv::kOffsetX); }
  int32_t  offsetY()    const { return mCore->mem_r16s(mBase + drawenv::kOffsetY); }
  uint32_t offsetXRaw() const { return mCore->mem_r16(mBase + drawenv::kOffsetX); }
  uint32_t offsetYRaw() const { return mCore->mem_r16(mBase + drawenv::kOffsetY); }

  uint32_t texWindowAddr() const { return mBase + drawenv::kTexWindow; }
  uint32_t texturePage()   const { return mCore->mem_r16(mBase + drawenv::kTexturePage); }
  uint32_t dither()        const { return mCore->mem_r8(mBase + drawenv::kDither); }
  uint32_t drawOnDisplay() const { return mCore->mem_r8(mBase + drawenv::kDrawOnDisp); }
  bool     clearsBackground() const { return mCore->mem_r8(mBase + drawenv::kClearFlag) != 0; }

  // The GP0 colour field, 0x00BBGGRR — ORed straight into the clear primitive's command word.
  uint32_t clearColor() const {
    return (mCore->mem_r8(mBase + drawenv::kClearBlue) << 16) |
           (mCore->mem_r8(mBase + drawenv::kClearGreen) << 8) |
...
struct DrawEnvPacket {
  ...
  void setClipTopLeft(uint32_t v)     { mCore->mem_w32(mBase + drenv::kClipTopLeft, v); }
  void setClipBottomRight(uint32_t v) { mCore->mem_w32(mBase + drenv::kClipBottomRight, v); }
  void setDrawOffset(uint32_t v)      { mCore->mem_w32(mBase + drenv::kDrawOffset, v); }
  void setDrawMode(uint32_t v)        { mCore->mem_w32(mBase + drenv::kDrawMode, v); }
  void setTexWindow(uint32_t v)       { mCore->mem_w32(mBase + drenv::kTexWindow, v); }
  void setMaskBits(uint32_t v)        { mCore->mem_w32(mBase + drenv::kMaskBits, v); }
  void setClearCommand(uint32_t v)    { mCore->mem_w32(mBase + drenv::kClearCommand, v); }
  void setClearTopLeft(uint32_t v)    { mCore->mem_w32(mBase + drenv::kClearTopLeft, v); }
  void setClearSize(uint32_t v)       { mCore->mem_w32(mBase + drenv::kClearSize, v); }
  void setTagWordCount(uint32_t v)    { mCore->mem_w8(mBase + drenv::kTagWordCount, (uint8_t)v); }
};
...
struct ClearRectScratch {
  ...
  uint32_t base() const { return mCore->r[29]; }

  void setClearX(uint16_t v) { mCore->mem_w16(base() + clearrect::kX, v); }
  void setClearY(uint16_t v) { mCore->mem_w16(base() + clearrect::kY, v); }
  void setClearW(uint16_t v) { mCore->mem_w16(base() + clearrect::kW, v); }
  void setClearH(uint16_t v) { mCore->mem_w16(base() + clearrect::kH, v); }

  uint32_t clearX() const { return mCore->mem_r16(base() + clearrect::kX); }
  uint32_t clearY() const { return mCore->mem_r16(base() + clearrect::kY); }
  uint32_t clearW() const { return mCore->mem_r16(base() + clearrect::kW); }
  int32_t  clearHSigned() const { return mCore->mem_r16s(base() + clearrect::kH); }

  // The packet takes the rect as two words, exactly as the two u16 pairs sit in memory.
  uint32_t topLeftWord() const { return mCore->mem_r32(base() + clearrect::kX); }
  uint32_t sizeWord()    const { return mCore->mem_r32(base() + clearrect::kW); }
};
