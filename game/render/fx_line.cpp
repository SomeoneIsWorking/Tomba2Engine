...
struct ShockwaveRingNode {
  ...
  int scale() const {
    return (int16_t)c->mem_r16(addr + kRingScale);
  }
...
inline void strokeSegment(
    Core *c, RenderQueue &rq, const ProjVtx &a, const ProjVtx &b, float ox, float oy, unsigned char grey, int blend) {
  const float ax = a.px + ox, ay = a.py + oy, bx = b.px + ox, by = b.py + oy;
  const float dx = bx - ax, dy = by - ay;
  const float len = sqrtf(dx * dx + dy * dy);
  if (len < 1e-4f) {
    return; // degenerate: the GPU would draw nothing
  }
...
void Render::worldLineDraw(int ax, int ay, int az, int bx, int by, int bz) {
  ...
  if (pa.sz <= 0 || pm.sz <= 0 || pb.sz <= 0) {
    return; // behind the camera: the emitter's own reject
  }
  ...
  const float cd[4] = {proj_pz_to_ord(pa.pz),
                       proj_pz_to_ord((pa.pz + pm.pz) * 0.5f),
                       proj_pz_to_ord((pb.pz + pm.pz) * 0.5f),
                       proj_pz_to_ord(pb.pz)};

  // The grey ramp, walked from the first vertex's screen position — the rope's shimmer.
  int rampIdx = (pa.sx + pa.sy) & kRampMask;
  unsigned char col[4];
  for (int i = 0; i < 4; i++) {
    col[i] = c->mem_r8(kGreyRamp + (uint32_t)rampIdx);
    rampIdx = (rampIdx + kRampStep) & kRampMask;
  }
  ...
  for (int s = 0; s + 1 < 4; s++) {
    ...
    const float len = sqrtf(dx * dx + dy * dy);
    if (len < 1e-4f) {
      continue; // a degenerate segment draws nothing
    }
...
void Render::shockwaveRingRender(uint32_t node) {
  ...
  EObjXform xf;
  projComposeObjectHost(Robj, Tobj, &xf);
  ...
  for (int step = 0; step < kRingSpans; step++) {
    ...
    if (behind) {
      rejected++;
      continue;
    } // the guest's own negative-flag reject
...
void Render::ropeAnchorRender(uint32_t node) {
  ...
  ObjScope objScope(c, node);
  worldLineDraw((int16_t)c->mem_r16(anchor),
                (int16_t)c->mem_r16(anchor + 2),
                (int16_t)c->mem_r16(anchor + 4),
                (int16_t)c->mem_r16(node + kOwnPosX),
                (int16_t)c->mem_r16(node + kOwnPosX + 2),
...
void Render::ropeChainRender(uint32_t node) {
  ...
  if (axis >= 3) {
    return; // the guest writes no point at all for these states — nothing to draw
  }
  ...
  for (int k = 0; k < kChainPts_N; k++) {
    ...
    if (k) {
      worldLineDraw(prev.x, prev.y, prev.z, p.x, p.y, p.z);
    }
    prev = p;
  }
}
...
void Render::tetherLineRender(uint32_t node) {
  ...
  const WorldPt self{(int16_t)c->mem_r16(node + kOwnPosX),
                     (int16_t)c->mem_r16(node + kOwnPosX + 2),
                     (int16_t)c->mem_r16(node + kOwnPosX + 4)};
  ...
  if (mode == 0) {
    ...
    worldLineDraw(self.x,
                  self.y,
                  self.z,
                  (int16_t)c->mem_r16(kFixedAnchorX),
                  (int16_t)c->mem_r16(kFixedAnchorY),
                  (int16_t)c->mem_r16(kFixedAnchorZ));
  } else if (mode == 2) {
    worldLineDraw(self.x,
                  self.y,
                  self.z,
                  s16(self.x + (int16_t)c->mem_r16(node + kTetherOffX)),
                  s16((int16_t)c->mem_r16(node + kTetherOffY) + kTetherRise),
                  self.z);
  } else if (mode == 3) {
    ...
    const WorldPt tgt{(int16_t)c->mem_r16(tracked + kTrackedX),
                      (int16_t)c->mem_r16(tracked + kTrackedY),
