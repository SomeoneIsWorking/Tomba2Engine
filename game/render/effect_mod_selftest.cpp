...
uint32_t buildPool(Core *c, PoolRng &rng) {
  for (uint32_t a = POOL; a < POOL_END; a += 4) {
    c->mem_w32(a, 0);
  }
  uint32_t p = POOL;
  for (;;) {
    const Op &op = OPS[rng.below(NOPS)];
    if (p + op.stride > POOL_END - 64) {
      break;
    }
    for (uint32_t k = 0; k < op.stride; k += 4) {
      c->mem_w32(p + k, rng.next());
    }
...
int check(Core *c, const char *name, uint32_t guestAddr, const std::function<void()> &nat, uint32_t lo, uint32_t hi) {
  ...
  if (c->recMissed) {
    return -1; // oracle couldn't evaluate -> skip, never a silent pass
  }
  ...
  for (int r = 0; r < NREG; r++) {
    for (uint32_t i = 0; i < REGIONS[r].words; i++) {
      ...
      if (mine[r][i] != o) {
        ...
        if (reported++ < 30) {
          cfg_logi("effecttest",
                   "MISMATCH %-9s %s+0x%04x  mine=%08x oracle=%08x",
                   name,
                   REGIONS[r].name,
                   i * 4,
                   mine[r][i],
...
int run_effectmod_selftest(const char *path) {
  ...
  struct Case {
    const char *name;
    uint32_t addr;
    std::function<void(uint32_t, uint32_t)> nat;
  };
  const Case cases[] = {
      {"semiOn",
       0x8003F3F4u,
       [&](uint32_t lo, uint32_t hi) {
         rendr->effectSemiOn(NODE, lo, hi);
       }},
      {"semiOff",
       0x8003F4C4u,
       [&](uint32_t lo, uint32_t hi) {
         rendr->effectSemiOff(NODE, lo, hi);
       }},
      {"clutSwap",
       0x8003F344u,
       [&](uint32_t lo, uint32_t hi) {
         rendr->effectClutSwap(NODE, lo, hi);
       }},
      {"flatTint",
       0x8003F594u,
       [&](uint32_t lo, uint32_t hi) {
         rendr->effectFlatTint(NODE, lo, hi);
       }},
      {"colorAdd",
       0x8003D584u,
       [&](uint32_t lo, uint32_t hi) {
         rendr->effectColorAdd(NODE, lo, hi);
       }},
  };
  ...
  for (int t = 0; t < NC; t++) {
    ...
    cfg_logi("effecttest",
             "%-9s : %s  (%d mismatching words over %d verified iters, %d oracle-skipped)",
             cases[t].name,
             bad ? "FAIL" : "ok",
             bad,
             ran,
  ...
  cfg_logi("effecttest",
           "DONE: %d passes, %d runs (%d oracle-skipped), %d mismatching words -> %s",
           NC,
           total_runs,
           total_skip,
           total_bad,
