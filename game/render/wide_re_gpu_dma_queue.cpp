...
// the whole call graph from authenticated executable/overlay evidence the cited guest instructions bodies (ground
// truth) line-by-line, with every branch polarity checked twice against the gen-C before being transcribed.
//
// Wide-RE tier (docs/fleet-workflow.md §6): UNWIRED / UNVERIFIED. Nothing here is called from
// anywhere (no tomba::native::declareOverride registration, no tomba::native::declareOverride) — dead code that only
// needs to COMPILE. A wiring pass MUST re-diff every line against the generated C (and run SBS) before registering +
// gating.
//
// ------------------------------------------------------------------------------------------------
...
// from authenticated executable/overlay evidence guest 0x80082D04 this session: the head/tail counters are at
...
//   +23212 (0x5AAC)  GPU_DMA_ARG0_PTR: pointer, target for guest 0x80082C68's (already-drafted, see
//                     wide_re_libgpu_leaves.cpp) reset-time a0 stash — not read by the 4 fns here.
//   +23216 (0x5AB0)  GPU_DMA_ARG1_PTR: pointer, zeroed by guest 0x80082C68's reset — not read here.
...
//                     section (guest 0x80085C9C interrupt-mask-set/get, not itself drafted this
//                     session — still substrate, reached via typed runtime address dispatch).
...
//                     `gpu_timeout_arm`/`gpu_timeout_chk` in runtime/psx/sync_overrides.cpp
//                     (registered in PlatformHle, so `typed runtime address dispatch(c, 0x800834A0/0x800834D4)` reaches
//                     the native no-op HLE, not the guest busy-wait body).
//
...
//   +0x00: fn-ptr to dispatch (typed runtime address dispatch target).
...
//     interrupt-mask set/get (guest 0x80085C9C), ISR-register (guest 0x80085B80), Drain (0x80082FB4,
//     mutual recursion: full queue -> drain-then-retry), and typed runtime address dispatch(fn) directly on the fast
//     (idle) path.
//   0x80082FB4 GpuDmaQueueDrain() — the GPU-DMA-completion INTERRUPT HANDLER BODY (installed as such
//     by Enqueue's deferred path via guest 0x80085B80(mode=2, &0x80082FB4), and unregistered the same
//     way with fn=0 when the ring empties past its last entry) — drains ring entries via
//     typed runtime address dispatch, fires the one-shot GPU_QSTAT completion handler when the queue empties.
...
//     (DrawSync itself is guest 0x80080F6C, already drafted in wide_re_libgpu_leaves.cpp) — an internal
...
// session (still substrate), reached uniformly via typed runtime address dispatch so a future native port of any of
...
// guest 0x80082D04 (0x80082D04) — GpuDmaQueueEnqueue(fn, argValOrPtr, sizeBytes, arg3). VERIFIED & WIRED 2026-07-10
// (see the install-fn note at the bottom of this file; the prior register-liveness bug is documented in
// docs/findings/render.md). RE'd from authenticated executable/overlay evidence guest 0x80082D04 (~165 gen-C ln). The
// single highest-value target in the band (824 free-roam typed runtime address dispatch hits / 600 frames). Guest ABI:
// a0=fn-ptr to later dispatch, a1=scalar value OR pointer-to-data (shape picked by a2), a2=size in BYTES of data to
...
void Render::gpuDmaQueueEnqueue() {
  ...
  // real MIPS s0-s3 live in the CPU registers for the whole function; every nested typed runtime address dispatch
  ...
  // native, core A wrote a stale pointer-shaped value (leftover from guest 0x80081218's r17)
  ...
  // (guest 0x80085C9C) is a true leaf (no sp adjust, no ra spill) — r31 mirroring is a no-op for it,
  // left unset like every other leaf call in this port.
  c->r[4] = 0;
  psx::cpu::dispatchGuestToReturn0(*c, FN_INT_MASK_SET, psx::cpu::ExecutionBudget::currentTurn(*c), __func__);
  ...
  // it's a MIPS branch-delay-slot store (authenticated executable/overlay evidence: `{ _t = (started==0);
  ...
  // FN_ISR_REGISTER (guest 0x80085B80) pushes a -24 guest frame and spills ra — mirror gen's r31.
  c->r[31] = 0x80082E38u;
  psx::cpu::dispatchGuestToReturn0(*c, FN_ISR_REGISTER, psx::cpu::ExecutionBudget::currentTurn(*c), __func__);
...
// guest 0x80082FB4 (0x80082FB4) — GpuDmaQueueDrain(). VERIFIED & WIRED 2026-07-10 (was DRAFT; see ovhit caveat at the
// install site — fires 0x0 in current autonav coverage). RE'd from authenticated executable/overlay evidence guest
// 0x80082FB4 (~130 gen-C ln). This is the completion-callback ring's DRAIN body — both called synchronously (by Enqueue
// and Sync) AND installed as the GPU-DMA-completion INTERRUPT HANDLER itself (Enqueue's deferred path registers
// `&0x80082FB4` via guest 0x80085B80(mode=2, ...)). Guest ABI: no args. Returns v0: 1 if the DMA channel was already
// busy on entry (nothing drained), -1 on timeout, else the remaining queue depth after draining.
//
// Frame -32, spills ra (sp+24), s1/r17 (sp+20), s0/r16 (sp+16) — the guest-visible behavior reuses r16/r17 as
...
// r16=BUSY_BIT/r17=READY_BIT assignment right before the drain loop, authenticated executable/overlay evidence),
...
// guest 0x80083364 (0x80083364) — GpuDmaQueueSync(mode). VERIFIED & WIRED 2026-07-10 (was DRAFT). RE'd from
// authenticated executable/overlay evidence guest 0x80083364 (~70 gen-C ln). Same mode-0-BLOCKS / mode-nonzero-POLLS
// shape as the real SDK `DrawSync(mode)` (a DIFFERENT function, already drafted as guest 0x80080F6C in
// wide_re_libgpu_leaves.cpp) — this is an internal sync primitive scoped to THIS queue. Guest ABI: a0=mode. Returns v0:
// -1 on timeout; mode==0 success returns the (nonzero) ready-bit mask value 0x04000000 (transcribed literally — the
// guest-visible behavior really does return the raw AND-masked bit, not a normalized bool); mode!=0 returns the queue
// depth (or 1 if depth was 0 and the channel wasn't idle+ready).
//
...
void Render::gpuDmaQueueSync() {
  ...
  // gen's final r3 per exit path (guest 0x80083364 shard_0.c): normal exits (both modes) have
...
// guest 0x80082424 (0x80082424) — GpuDmaSend(arrayPtr, count). VERIFIED & WIRED 2026-07-10 (was DRAFT). RE'd from
// authenticated executable/overlay evidence guest 0x80082424 (~50 gen-C ln, self-contained). The actual OT-linked-list
// DMA KICK: programs the last-element address + count into the status block, writes the DMA channel control register
...
// cleared normally (transcribed literally — the guest-visible behavior's post-wait return value really is the
...
// §9 — re-verified line-by-line against authenticated executable/overlay evidence (found + fixed 9 missing-ra-mirror
// bugs, see the `c->r[31] = 0x<gen-return-addr>` comments added above each non-leaf call site: every callee here that
// pushes its own guest-stack frame — Drain, the ISR-register leaf guest 0x80085B80, and the caller-supplied
// `fn`/ring-entry/completion-handler dispatch targets — spills the CALLER's r31 into ITS OWN frame, so r31 must carry
// the live gen return-address constant at each such call site or the spilled byte diverges from gen; guest 0x80085C9C
// (int-mask set) and the GPU-DMA timeout arm/chk HLE no-ops are true leaves and don't need it). All 4 addresses are
// reached as plain intra-shard C calls from the substrate (guest 0x80082D04 etc. in authenticated executable/overlay
// evidence), NOT via typed runtime address dispatch — tomba::native::declareOverride (runtime/psx/override_registry.h,
// a thin forwarder over tomba::native::declareOverride) is the correct install path: it keeps SBS core B running the
// pure original guest instructions body via the registry's oracle-gated dispatch.
namespace {
...
void gpu_dma_queue_install() {
  ...
  // INSIDE the then-still-substrate, UNOWNED guest 0x80082734 (the "LoadImage-style FIFO streamer",
  // mapped but not yet drafted at the time). That streamer is now DRAFTED, RE-VERIFIED (see
  // game/render/wide_re_gpu_loadimage_streamer.cpp) and WIRED via tomba::native::declareOverride
  ...
  // the whole guest binary (grepped authenticated executable/overlay evidence) — so Enqueue's fast (immediate-
  ...
  tomba::native::declareOverride(0x80082D04u, "ov_gpuDmaQueueEnqueue", ov_gpuDmaQueueEnqueue);
  tomba::native::declareOverride(0x80082FB4u, "ov_gpuDmaQueueDrain", ov_gpuDmaQueueDrain);
  tomba::native::declareOverride(0x80083364u, "ov_gpuDmaQueueSync", ov_gpuDmaQueueSync);
  tomba::native::declareOverride(0x80082424u, "ov_gpuDmaSend", ov_gpuDmaSend);
}
