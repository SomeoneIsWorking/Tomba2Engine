...
static inline const float (*shadow_verts(const ProjVtx *p, int nv, int semi, float vv[4][3]))[3] {
  if (semi || !gpu_vk_shadows_active()) {
    return nullptr; // only opaque world casts shadows
  }
  for (int k = 0; k < 4; k++) {
    int s = k < nv ? k : nv - 1;
    vv[k][0] = p[s].vx;
...
static inline void engine_shade_face(Core *c, const ProjVtx *p, int nv, uint8_t r[4], uint8_t g[4], uint8_t b[4]) {
  ...
  if (cfg->num_points > 0) {
    ...
    for (int i = 0; i < cfg->num_points; i++) {
      ...
      float att = 1.0f - d / rad;
      if (att < 0.0f) {
        att = 0.0f;
      }
      att *= att; // soft quadratic falloff
      float pdl = (d > 1e-3f) ? (nx * dx + ny * dy + nz * dz) / d : 0.0f;
      if (pdl < 0.0f) {
        pdl = 0.0f;
      }
  ...
  for (int k = 0; k < nv; k++) {
    ...
    r[k] = (uint8_t)(rr > 255 ? 255 : rr);
    g[k] = (uint8_t)(gg > 255 ? 255 : gg);
    b[k] = (uint8_t)(bb > 255 ? 255 : bb);
  }
}
...
static SortKey game_sort_key(Core *c, const ProjVtx *p, int nv, uint32_t code, int zsf) {
  ...
  if (policy == 1 || policy == 2) {
    ...
    if (zsf <= 0) {
      return SortKey::unknown(); // no ZSF captured yet this run — we cannot tell
    }
    int64_t sum = 0;
    for (int k = 0; k < nv; k++) {
      sum += p[k].sz;
  ...
  if (delta & 3) {
    return SortKey::unknown(); // otbase not derived from the active base
  }
  int32_t shift = delta >> 2;
  if (shift < -128 || shift > 127) {
    return SortKey::unknown(); // outside int8 cmd[0x3F] — foreign OT
  }
...
void Render::submitPolyGt3Native(Core *c) {
  ...
  for (uint32_t i = 0; i < count; i++, rec += 36) {
    ...
    if (p[0].sx >= xmax && p[1].sx >= xmax && p[2].sx >= xmax) {
      continue;
    }
    if (p[0].sy >= 240 && p[1].sy >= 240 && p[2].sy >= 240) {
      continue;
    }
    ...
    u[1] = uv1 & 0xFF;
    v[1] = (uv1 >> 8) & 0xFF;
    u[2] = c->mem_r16(rec + 34) & 0xFF;
    v[2] = (c->mem_r16(rec + 34) >> 8) & 0xFF; // uv2 (high half of rec+32)
    for (int k = 0; k < 3; k++) {
      px[k] = p[k].px;
      py[k] = p[k].py;
    ...
    px[3] = px[2];
    py[3] = py[2];
    depth[3] = depth[2]; // 4th vert = v2 (degenerate -> a triangle)
    u[3] = u[2];
    v[3] = v[2];
    r[3] = r[2];
    g[3] = g[2];
    ...
    if (!semi) {
      engine_shade_face(c, p, 3, r, g, b); // engine-native lighting (opaque only)
    }
    if (cfg_dbg("eprojv")) {
      static long objn = -1;
      uint32_t geomblk = c->rsub.diag.currentGeomblk();
      if ((long)geomblk != objn) {
        objn = (long)geomblk;
        const EObjXform &x = rend(c)->mActiveXform;
        cfg_logf("eprojv",
                 "cmd=%08x R=[%.4f %.4f %.4f | %.4f %.4f %.4f | %.4f %.4f %.4f] T=(%.2f,%.2f,%.2f) H=%.0f",
                 geomblk,
                 (double)x.R[0][0],
                 (double)x.R[0][1],
                 (double)x.R[0][2],
      ...
      cfg_logi("eprojv",
               "cmd=%08x gt3 idx=%u v0=(%.2f,%.2f,%.2f) v1=(%.2f,%.2f,%.2f) v2=(%.2f,%.2f,%.2f)",
               geomblk,
               i,
               (double)px[0],
               (double)py[0],
    ...
    {
      char tag[32];
      snprintf(tag, sizeof tag, "gt3_native@%08X", c->rsub.diag.currentGeomblk());
      sil_bbox_log_verts(tag, px, py, depth, 3, cur_render_node(c), rec, r, g, b);
    }
    {
      float vv[4][3];
      const float (*sv)[3] = shadow_verts(p, 3, semi, vv); // dynamic shadow verts (carried on the item)
      ...
      if (!rend(c)->mNativeDrawSuppress) {
        rqOut.drawWorldQuad(
            c, px, py, depth, u, v, r, g, b, tp, clut, semi, sv, skey, skey >= 0 ? key_to_ord(skey) : 0.0f);
      }
    }
  }
  c->r[2] = rec;
...
void Render::submitPolyGt4Native(Core *c) {
  ...
  for (uint32_t i = 0; i < count; i++, rec += 44) {
    ...
    float area = (p[1].px - p[0].px) * (p[2].py - p[0].py) - (p[2].px - p[0].px) * (p[1].py - p[0].py);
    if (area <= 0) {
      continue; // backface (matches MAC0<=0 drop)
    }
    // frustum cull (right/bottom edges only, as the original) over all 4 verts.
    int xmax = submit_xmax(c);
    if (p[0].sx >= xmax && p[1].sx >= xmax && p[2].sx >= xmax && p[3].sx >= xmax) {
      continue;
    }
    if (p[0].sy >= 240 && p[1].sy >= 240 && p[2].sy >= 240 && p[3].sy >= 240) {
      continue;
    }
    ...
    if (!semi) {
      engine_shade_face(c, p, 4, r, g, b); // engine-native lighting (opaque only)
    }
    if (cfg_dbg("eprojv")) {
      uint32_t geomblk = c->rsub.diag.currentGeomblk();
      cfg_logf("eprojv",
               "cmd=%08x gt4 idx=%u v0=(%.2f,%.2f,%.2f) v1=(%.2f,%.2f,%.2f) v2=(%.2f,%.2f,%.2f) v3=(%.2f,%.2f,%.2f)",
               geomblk,
               i,
               (double)px[0],
               (double)py[0],
    ...
    {
      char tag[32];
      snprintf(tag, sizeof tag, "gt4_native@%08X", c->rsub.diag.currentGeomblk());
      sil_bbox_log_verts(tag, px, py, depth, 4, cur_render_node(c), rec, r, g, b);
    }
    {
      float vv[4][3];
      const float (*sv)[3] = shadow_verts(p, 4, semi, vv); // dynamic shadow verts (carried on the item)
      ...
      if (key.guestDrop) {
        continue; // GATE 4 — see the GT3 twin above
      }
      const int skey = key.linked ? key.key : -1;
...
static bool geomblkRecordsFitInRam(uint32_t geomblk, uint32_t gt3, uint32_t gt4) {
  ...
  const uint64_t end = (uint64_t)geomblk + kGeomHdrBytes + (uint64_t)gt3 * kGt3RecBytes + (uint64_t)gt4 * kGt4RecBytes;
...
void Render::fieldEntityRender(uint32_t es) {
  ...
  if (cfg_dbg("groundproj")) {
    static int n = 0;
    if (n++ < 3) {
      cfg_logf("groundproj",
               "es=%08x count=%u base=%08x T=(%.0f,%.0f,%.0f) H=%.0f R0=(%.3f,%.3f,%.3f)",
               es,
               count,
               base,
               (double)w.T[0],
               ...
               (double)w.R[0][1] / 4096,
               (double)w.R[0][2] / 4096);
      uint32_t cmd0 = base + (uint32_t)c->mem_r16(es + 0x10) * 4;
      uint32_t s0d = c->mem_r32(cmd0);
      uint32_t rec = cmd0 + 4 + ((s0d & 0xFF) * 36); // skip GT3s to first GT4 record (44B)
      for (int k = 0; k < 2; k++) {
        uint32_t r2 = rec + k * 44;
        int16_t vx = c->mem_r16s(r2 + 20), vy = (int16_t)(c->mem_r32(r2 + 20) >> 16), vz = c->mem_r16s(r2 + 24);
        ProjVtx pv;
        rend(c)->projVertexActive(vx, vy, vz, &pv);
        cfg_logf("groundproj",
                 "   gt4[%d] model=(%d,%d,%d) -> px=%.1f py=%.1f pz=%.1f sx=%d sy=%d",
                 k,
                 vx,
                 vy,
                 vz,
  ...
  for (; p < end; p += 2) {
    ...
    c->rsub.diag.setGeomblk(
        cmd); // sil_bbox_log diag: tag this entity's cmd record (Render::gt3gt4 is NOT the caller here)
    c->r[4] = cmd + 4;
    c->r[5] = otbase;
    c->r[6] = s0 & 0xFF;
    submitPolyGt3Native(c);
