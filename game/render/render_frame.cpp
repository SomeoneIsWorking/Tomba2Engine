...
// ov_render_frame DIRECTLY (a plain C call) instead of dynamically dispatching MAIN.EXE 0x8003f9a8. The render-queue
...
// path — WITHOUT touching the native frame loop / game state, so the SAME deterministic guest state can be rendered
// both ways and diffed (native must match PSX under 1x / 4:3 / 30fps). Set by PSXPORT_RENDER_PSX / REPL `renderpsx
// on|off`. This is a verification instrument, NOT a shipped behavior toggle. g_render_psx REMOVED (2026-07-02,
// deglobalize-game): now Render::mPsxRender / psxRender(). g_dualview  REMOVED (2026-07-02, deglobalize-game): now
// Render::mDualview / dualview().
...
// byte-identical to the recorded guest behavior — rendering can never cause an SBS divergence. pc_render vs
...
// context than the guest instruction path's, which is exactly what diverged (f26, guest-stack spills at a foreign sp).
void Render::frame() {
  Core *c = mCore;
  if (cfg_dbg("rfprobe")) {
    static int n = 0;
