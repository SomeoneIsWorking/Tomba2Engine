...
public:
  ...
  // overlay's own g_ov_a00_override[] table, so DIRECT generated calls are intercepted alongside
  // rec_dispatch ones — which matters here because the two leaves are reached by direct C call
  // (ov_a00_func_801465EC/801467BC(c)), never by rec_dispatch. That covers both of their call
  // sites uniformly: this cluster's own dispatcher, and a duplicate tail-shared copy of the same
  // call sequence the recompiler's CFG discovery folded into FUN_80147FC4.
  //
  // (An older version of this comment claimed the leaves were wired via a RAW ov_a00_set_override
