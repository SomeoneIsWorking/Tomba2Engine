...
Mat3 rotMatrix(int16_t ax, int16_t ay, int16_t az) {
  ...
  Mat3 r;
  r.m[0][0] = cz * cy;                r.m[0][1] = -sz * cy;               r.m[0][2] = sy;
  r.m[1][0] = cz * sx * sy + cx * sz; r.m[1][1] = cx * cz - sz * sx * sy; r.m[1][2] = -cy * sx;
  r.m[2][0] = sx * sz - cz * cx * sy; r.m[2][1] = sx * cz + sz * cx * sy; r.m[2][2] = cy * cx;
  return r;
}
...
struct GlyphXform { Mat3 R; float T[3]; };
...
public:
  ...
  bool valid() const { return mH > 0.0f; }
...
void emitQuad(Core* c, uint32_t node, const ViewProjector& proj, const GlyphXform& g,
              const int16_t vx[4], const int16_t vy[4], const int16_t vz[4],
  ...
  for (int i = 0; i < 4; i++) {
    ...
    if (!proj.project(view, px[i], py[i], dep[i], c)) {
      lucent::debug("cubetext", "quad DROPPED at corner {} (behind the eye): model=({},{},{})",
                    i, vx[i], vy[i], vz[i]);
      return;
    }
    ln.add("[{} m=({},{},{}) v=({:.1f},{:.1f},{:.1f}) s=({:.1f},{:.1f})]",
           i, vx[i], vy[i], vz[i], view[0], view[1], view[2], px[i], py[i]);
  }
...
void emitPlanks(Core* c, uint32_t node, const ViewProjector& proj, const GlyphXform& g,
                uint32_t geomblk) {
  if (!geomblk) return;
  ...
  for (int t = 0; t < n4; t++, rec += 44) {
    ...
    const uint32_t col[4] = { code0 & COL_MASK, (code0 << 4) & COL_MASK,
                              rgb2w & COL_MASK,  (rgb2w << 4) & COL_MASK };
    const uint32_t uv0 = c->mem_r32(rec + 8), uv1 = c->mem_r32(rec + 12), uv23 = c->mem_r32(rec + 16);
    emitQuad(c, node, proj, g, vx, vy, vz, col, uv0, uv1,
             (uint32_t)(uint16_t)uv23, (uint32_t)(uint16_t)(uv23 >> 16));
  }
}
...
void emitGlyph(Core* c, uint32_t node, const ViewProjector& proj, const GlyphXform& g,
               uint8_t ch, uint32_t clut) {
  if (ch == kSpace) return;                      // FUN_80039E80 returns -1: no glyph packet at all
  const uint32_t u0 = (uint32_t)(ch & 31u) * 8u;
  const uint32_t v0 = ((uint32_t)(ch + 32u) >> 5) * 16u + 8u;
  uint32_t u1 = (u0 + 8u) & 0xFFu;
  if (u1 == 0u) u1 = 255u;                       // the guest's wrap fixup, reproduced
  const uint32_t v2 = (v0 + 16u) & 0xFFu;
  const uint32_t col = (kGlyphOpByte << 24) | kGlyphRgb;
  const uint32_t cols[4] = { col, col, col, col };
  emitQuad(c, node, proj, g, kGlyphX, kGlyphY, (const int16_t[4]){ kGlyphZ, kGlyphZ, kGlyphZ, kGlyphZ },
           cols,
           u0 | (v0 << 8) | (clut << 16),
...
void CubeTextBanner::render(Core* c, uint32_t node) {
  ...
  const Mat3 nodeR = rotMatrix(c->mem_r16s(node + NODE_EULER + 0),
  ...
  lucent::debug("cubetext", "node={:08X} glyphs={} nodeT=({},{},{}) ofx={} ofy={} H={} clear={}",
                node, glyphs, nodeT[0], nodeT[1], nodeT[2], ofx, ofy, H, clearVariant);
  ...
  for (uint32_t i = 0; i < glyphs; i++) {
    ...
    GlyphXform g;
    g.R = matMul(nodeR, rotMatrix(c->mem_r16s(rec + REC_EULER + 0),
                                  c->mem_r16s(rec + REC_EULER + 2),
                                  c->mem_r16s(rec + REC_EULER + 4)));
    const float lp[3] = { (float)c->mem_r16s(rec + REC_POS_X),
                          (float)c->mem_r16s(rec + REC_POS_Y),
                          (float)c->mem_r16s(rec + REC_POS_Z) };
    for (int k = 0; k < 3; k++)
    ...
    if (textPtr && !textEnded) {
      ...
      if (ch == 0) textEnded = true;          // the guest's own end-of-string break, glyphs only
      else         emitGlyph(c, node, proj, g, ch, clut);
    }
  }
}
