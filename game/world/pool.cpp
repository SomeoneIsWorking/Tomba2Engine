...
void Pool::resetControlBlock() {
  ...
  c->r[5] = 0;
  c->r[6] = 104;
  call_fn(c, 0x8009A420u); // memset(b, 0, 104)
...
void Pool::clearBf548Region() {
  Core *c = core;
  for (uint32_t off = 0; off < 700; off++) {
    c->mem_w8(0x800BF548u + off, 0);
  }
}
...
void Pool::initTypedPools() {
  ...
  for (const PoolSpec &p : pools) {
    for (uint32_t i = 0; i < p.count; i++) {
      ...
      for (uint32_t off = 0; off < p.stride; off++) {
        c->mem_w8(slot + off, 0);
      }
  ...
  c->mem_w32(0x1F80013Cu, 0x800F2410u);
  c->mem_w32(0x1F800140u, 0x800F2410u);
  c->mem_w16(0x1F800146u, 0);
  c->mem_w16(0x1F800144u, 0);
  c->mem_w32(0x1F800148u, 0x800F26C8u);
  c->mem_w32(0x1F80014Cu, 0x800F26C8u);
  c->mem_w16(0x1F800152u, 0);
  c->mem_w16(0x1F800150u, 0);
  c->mem_w32(0x1F800154u, 0x800F2738u);
  c->mem_w32(0x1F800158u, 0x800F2738u);
  c->mem_w16(0x1F80015Eu, 0);
  c->mem_w16(0x1F80015Cu, 0);
}
...
void Pool::init() {
  ...
  for (int i = 0; i < 520; i++) {
    c->r[4] = 0x800F2740u + (uint32_t)i * 68u;
    c->r[5] = 0;
    c->r[6] = 68;
...
void Pool::reset75240() {
  ...
  c->mem_w16(b + 42, 0x7FFF);
  c->mem_w16(b + 44, 0x7FFF);
  c->mem_w16(b + 46, 0x1FFF);
  c->mem_w16(b + 48, 0x1FFF);
  c->mem_w32(b + 0, 768);
  c->mem_w32(b + 24, 0);
  c->mem_w32(b + 20, 0);
  c->r[4] = b;
  call_fn(c, 0x80075824u);
  c->r[4] = b;
  call_fn(c, 0x80099490u);
  c->mem_w32(b + 0, 0);
  c->mem_w8(b + 50, 0);
  c->mem_w8(b + 51, 0);
}
...
void Pool::setupViewScroll() {
  ...
  if (c->mem_r8(A + 16) == 0) {
    ...
    c->mem_w16(S0 + 366, (uint16_t)v);
    c->mem_w16(S0 + 368, (uint16_t)v);
  } else {
  ...
  if (mode == 3) {
    ...
    if (c->mem_r8(0x1F800134u) != 0) {
      ...
      if (s17e < 0) {
        c->mem_w16(S0 + 50, (uint16_t)(c->mem_r16(S0 + 50) + 70));
      }
...
void Pool::selectStateIndex(uint8_t area) {
  ...
  uint32_t s0 = 0;
  bool have = false;
  if (m != 15 && c->mem_r8(0x800BF873u) != 0) {
    s0 = 42;
    have = true;
  }
  if (!have && q == 6) {
