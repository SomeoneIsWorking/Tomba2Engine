...
void pushQuad(
    Core *c, const int *xs, const int *ys, const int *us, const int *vs, const AttrDecode &pa, const TpageDecode &tp) {
...
void Panel::pushFill(Core *c, uint32_t rectPtr, int32_t uvIndex, uint16_t attr, int32_t otBucket) {
  ...
  pushFillAt(c,
             c->mem_r16s(rectPtr + 0u),
             c->mem_r16s(rectPtr + 2u),
             c->mem_r16s(rectPtr + 4u),
             c->mem_r16s(rectPtr + 6u),
             uvIndex,
             attr);
}
...
void Panel::pushFillAt(Core *c, int rx, int ry, int rw, int rh, int32_t uvIndex, uint16_t attr) {
...

  {
    ...
    if ((np++ & 127) == 0) {
      cfg_logf("panelq",
               "fill #%ld uv=%d rect=(%d,%d %dx%d) attr=%04X semi=%d raw=%d",
               np,
               uvIndex,
               rx,
               ry,
...
void Panel::pushCornersAt(Core *c, int rx, int ry, int rw, int rh, uint16_t style, uint32_t shadow) {
  const uint16_t attr = (style & 0x40u) ? (uint16_t)(style + 0x0Du) : (uint16_t)((style + 6u) | (shadow ? 0x80u : 0u));
  const int ox = c->game->gpu.s_off_x, oy = c->game->gpu.s_off_y;
...

  {
    ...
    if ((np++ & 127) == 0) {
      cfg_logf("panelq",
               "corner #%ld rect=(%d,%d %dx%d) style=%04X shadow=%u attr=%04X",
               np,
               rx,
               ry,
               rw,
...
void panelFillTap(Core *c) {
  ...
  if (UiGroupCapture::routePanelFill(c, rectPtr, uvIndex, attr, otBucket)) {
    return;
  }
