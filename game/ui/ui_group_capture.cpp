...
UiGroupCapture* raisedScope(Core* c) {
  if (c->game->oracle || c->rsub.mode.psxRender()) return nullptr;   // read-only overlay gate
  Engine& e = eng(c);
  if (e.pauseMenu.capture.capturing())   return &e.pauseMenu.capture;
  if (e.startPage.capture.capturing())   return &e.startPage.capture;
  if (e.optionsPage.capture.capturing()) return &e.optionsPage.capture;
  if (e.cardMenu.capture.capturing())    return &e.cardMenu.capture;
  return nullptr;
}
}  // namespace
...
void UiGroupCapture::route(Core* c, const UiGroupArgs& a) {
  ...
  if (UiGroupCapture* s = raisedScope(c)) s->fileGroup(a);
}

bool UiGroupCapture::routePanelFill(Core* c, uint32_t rectPtr, int32_t uvIndex, uint16_t attr,
                                    int32_t otBucket) {
  UiGroupCapture* s = raisedScope(c);
  if (!s) return false;                       // no page owns this panel — the tap draws it inline
  s->filePanelFill(c, rectPtr, uvIndex, attr, otBucket);
  return true;
}

bool UiGroupCapture::routePanelCorners(Core* c, uint32_t rectPtr, uint16_t style, uint32_t shadow,
                                       int32_t otBucket) {
  UiGroupCapture* s = raisedScope(c);
  if (!s) return false;
...
void UiGroupCapture::filePanelFill(Core* c, uint32_t rectPtr, int32_t uvIndex, uint16_t attr,
                                   int32_t otBucket) {
  PageChromeItem it;
...
void UiGroupCapture::filePanelCorners(Core* c, uint32_t rectPtr, uint16_t style, uint32_t shadow,
                                      int32_t otBucket) {
  PageChromeItem it;
