...
L_8007D9C4:;
  ...
  c->r[2] = c->r[2] - c->r[3];
  c->mem_w16((c->r[16] + (uint32_t)84), (uint16_t)c->r[2]);
  goto L_8007DA1C;
L_8007D9F0:;
  c->r[2] = (uint32_t)c->mem_r16((c->r[16] + (uint32_t)64));
  c->r[3] = (uint32_t)8064u << 16;
...
static void ov_dialog_box_sm_step(Core *c) {
  DialogBoxSm::step(c);
}
...
void dialog_box_sm_install() {
  ...
  overrides::install(0x8007D594u, "DialogBoxSm::step", ov_dialog_box_sm_step, gen_func_8007D594, shard_set_override);
}
