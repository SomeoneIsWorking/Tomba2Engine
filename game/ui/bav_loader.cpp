...
static inline void bav_lock_set(Core *c,
                                uint32_t a0) { // FUN_80099450(a0): a0==1 -> *LOCK=0 (delay-slot sw zero); else *LOCK=1
...
static uint32_t bav_load_native(Core *c) {
  ...
  if (bav_lock_ready(c) == 1) {
    return (uint32_t)-1; // beq v0,1 -> exit, v0=-1
  }
  ...
  if (arg_slot >= 16) {
    ...
    for (int32_t i = 0; i < 16; i++) {
      if (c->mem_r8(G_D18 + (uint32_t)i) == 0) {
        found = i;
        break;
      }
    }
  ...
  int32_t off = 0;                               // s0 = cumulative byte offset
  {
    ...
    if (bu >= 0) {                                        // slti(v1,0) -> if v1<0 skip
      for (int32_t i = 0; i <= bu; i++) {                 // slt(bu,i) -> while i<=bu
        ...
        if (i & 1) {
          c->mem_w16(rec + 14, (uint16_t)word); // odd  -> +14
        } else {
          c->mem_w16(rec + 12, (uint16_t)word); // even -> +12
        }
      }
    }
  }
  ...
  // BONUS: log the loaded cel's atlas params (under the `bavload` debug channel).
  {
    if (c->game->verify.on("bavload")) {
      ...
      cfg_logi("bavload",
               "CEL slot=%d desc=%08x kind=%u(bpp~%u) frames(clamp)=%u uv_entries=%d vram_base_word=%05x size=%u "
               "rec0.w12=%04x rec0.w14=%04x",
               slot_f,
               desc,
               kind,
               bpp,
               clamp,
               (int)(c->mem_r8(desc + 22) & 0xff) + 1,
               a0v,
               (unsigned)off,
               r0_12 & 0xffff,
               r0_14 & 0xffff);
      ...
      for (int i = 0; i <= bu2; i++) {
        ...
        cfg_logi("bavload",
                 "  F%-2d word=%04x  vram XY=(%4u,%3u)  size_words=%u",
                 i,
                 word & 0xffff,
                 wa % 1024u,
                 wa / 1024u,
