...
static void eng_load_stage(Core *c) {
  ...
  if (stage == 3) {
    ...
    psx::cpu::dispatchGuestToReturn0(*c,
                                     ...
                                     __func__);             // synchronous overlay load (CD platform)
  ...
  psx::cpu::dispatchGuestToReturn0(
      *c, 0x80080930u, psx::cpu::ExecutionBudget::currentTurn(*c), __func__); // task stack/handle alloc
  c->mem_w32(param_1 + 4, c->r[2]);                                           // param_1[1] = its result
  c->r[2] = (int32_t)stage; // (guest instruction path returns param_2; harmless)
}
...
// run is correct; no guest-continuation handshake is needed (unlike the running dispatcher, later-169).
//
...
// START->DEMO->GAME init transitions run via guest call (FUN_800499e8) with in_stage==0, where switch is a no-op
...
static void eng_stage_transition(Core *c) {
  ...
  c->r[4] = task + 0xc;
  c->r[5] = stage;
  eng_load_stage(c); // native (was typed runtime address dispatch(0x800450BCu))
  task = c->mem_r32(0x1f800138u);
  c->mem_w16(task, 3);       // task state = 3 (RESTART fresh at the new entry)
  c->mem_w8(task + 0x6f, 0); // task[0x6f] = 0
  c->r[16] = s0;
  c->r[29] = sp;
  c->r[31] = ra;         // epilogue: restore s0/sp/ra (for the boot no-op-return path)
...
// Called once at boot via guest call (FUN_800499e8) (in_stage==0); FUN_80052078's terminal yield is a no-op return
...
static void eng_task0_boot(Core *c) {
  ...
  psx::cpu::dispatchGuestToReturn0(*c,
                                   0x8008B8F0u,
                                   psx::cpu::ExecutionBudget::currentTurn(*c),
                                   __func__); // resolve the file's CD directory entry into buf (platform CD)
  if (c->r[2] != 0) {
    c->r[4] = buf;
    c->r[31] = 0x80049A34u; // jal 0x8008a110 @0x80049a2c -> ra
    psx::cpu::dispatchGuestToReturn0(
        *c, 0x8008A110u, psx::cpu::ExecutionBudget::currentTurn(*c), __func__); // MSF (in buf) -> LBA
    c->mem_w32(0x800BE1E0u, c->r[2]);                                           // stage[0].lba
    c->mem_w32(0x800BE1E4u, c->mem_r32(buf + 4));                               // stage[0].size = buf[+4]
  } else {
    c->r[4] = 0x8001546Cu;
    c->r[5] = name;         // "Not found file name "
    c->r[31] = 0x80049A24u; // jal 0x8009a730 @0x80049a1c -> ra
    psx::cpu::dispatchGuestToReturn0(
        *c, 0x8009A730u, psx::cpu::ExecutionBudget::currentTurn(*c), __func__); // error report
  }
  c->r[4] = 0;            // FUN_80052078(0): transition to stage 0
  c->r[31] = 0x80049A50u; // jal 0x80052078 @0x80049a48 -> ra
  eng_stage_transition(
      c); // native (was typed runtime address dispatch(0x80052078u)) — frame still down for byte-faithful stack scratch
...
// the cross-frame DMA-wait poll loop handed back IN-CONTEXT via the guest-continuation handshake (later-169).
//
...
// (verified: after them the slot's state-byte is 1 as the guest instruction path leaves it), so they are safe to
// dispatch natively here. We then `requestGuestContinuation` to the loop head 0x80075410 with the MIPS frame set up
// byte-faithfully (sp-=0x20; s0/s1/ra saved), so the guest instruction path loop + its epilogue restore correctly and
// the deep yield resumes exactly as the PSX path would — same handshake engine.cpp uses for deep-yield handlers. The
// cel-system callees (FUN_80096480/80096980/80096a40 — bav_loader cel loader + upload state machine + DMA-done event
// poll) stay dispatched: they write the guest cel-system globals still-guest content reads, and FUN_80096590 is itself
// an existing native override.
// =================================================================================================
