...
static void demo_log(const char *who, Core *c) {
  ...
  cfg_logf("demo",
           "%s sm[0x48]=%u 0x4a=%u 0x68=%u 0x6b=%u 0x50=%u",
           who,
           c->mem_r16(sm + 0x48),
           c->mem_r16(sm + 0x4a),
           c->mem_r8(sm + 0x68),
...
uint32_t Demo::s3SubMachine() {
  ...
  if (proceed) {
    ...
    if (timer == 1) {
      ...
      if (haveCmp) {
        if (cmp != cursor) {
          eng(c).sfx.trigger(0x15, 0, 0); // cursor-move blip
        }
  ...
  if (proceed) {
    ...
    if (timer == 1) {
      ...
      if (edges & 0x20u) {     // Right -> select item 1 (Load Game)
        if (c->mem_r8(sm + 0x68) != 1) {
          c->r[31] = 0x80106AA4u;
          eng(c).sfx.trigger(0x15, 0, 0);
        }
        c->mem_w8(sm + 0x68, 1);
        c->mem_w16(sm + 0x4a, 0);
      } else if (edges & 0x80u) { // Left -> select item 0 (New Game)
        if (c->mem_r8(sm + 0x68) != 0) {
          c->r[31] = 0x80106A78u;
          eng(c).sfx.trigger(0x15, 0, 0);
        }
        c->mem_w8(sm + 0x68, 0);
...
void Demo::s7Phase() {
  ...
  cfg_logf("demo",
           "s7_phase sm[0x4a]=%u sm[0x48]=%u sm[0x6e]=%u sm[0x5a]=%u (ra=0x%08X)",
           phase,
           c->mem_r16(sm + 0x48),
           c->mem_r8(sm + 0x6e),
           c->mem_r16(sm + 0x5a),
...
static uint32_t demo_menu_machine(Core *c) {
  ...
  if (s4a >= 4) {
    ...
    if (!skip) {
      ...
      static int w = 0;
      if (!w++) {
        cfg_logf("demo", "OP.STR opening movie skipped (NO_FMV/headless); running teardown -> s2");
      }
    }
...
static void demo_frame_s7(Core *c) {
  ...
  uint16_t phase = c->mem_r16(sm + 0x4a);
  cfg_logf("demo",
           "s7 attract phase=%u sm[0x6e]=%u sm[0x5a]=%u bf870=%u sm56=%02x",
           phase,
           c->mem_r8(sm + 0x6e),
           c->mem_r16(sm + 0x5a),
  ...
  if (phase == 0) {
    ...
    if (t != 0) {
      ...
      advance = 1; // timer expired
    }
    if (advance) {
      c->mem_w16(sm + 0x4a, c->mem_r16(sm + 0x4a) + 1); // sm[0x4a]++ -> phase2
    }
  } else if (phase == 2) {
...
void Demo::frame() {
  ...
  case 4:
    demo_frame_s4(c);
    break; // LOAD GAME (memcard slot browser)
  case 5:
    demo_frame_s5(c);
    return; // left DEMO -> GAME (no frame-ctr bump; we're gone)
  case 6:
    demo_frame_s6(c);
    break;
  case 7:
    demo_frame_s7(c);
    break; // attract demo (idle timeout)
  default:
    if (cfg_dbg("demo")) {
      static int w = 0;
      if (!w++) {
        cfg_logf("demo", "ov_demo_frame: sm[0x48]=%u not yet native (frontier — s4 needs conversion)", s48);
      }
    }
    break;
  }
