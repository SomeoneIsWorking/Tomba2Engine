...
int ScriptInterp::loadNextEntry(uint32_t obj, uint32_t kindArg) {
  ...
  if (opWord & OP_FLAG_COND) {
    result = ADVSTEP_CONDEND;
  }

  cfg_logf("script",
           "advance obj=%08X ptr=%08X op=%04X kind=%04X arg=%u -> %08X (idx %d)",
           obj,
           cursor,
           opWord,
           kind,
...
int ScriptInterp::op34ClaimGate(uint32_t obj) {
  ...
  if (phase == 0) {
    ...
    if (claimTag != 0) {
      if (c->mem_r8(kClaimGateByte) == 0) {
        c->mem_w8(kClaimGateByte, claimTag);
      }
      if (argARaw & 0x8000u) {
        return 1; // "no-wait" argA -> advance immediately
      }
    }
    c->mem_w8(obj + OBJ_SCRATCH_78, (uint8_t)(phase + 1));
    return 0;
...
int ScriptInterp::advanceStep(uint32_t obj, uint32_t kindArg) {
  ...
  if (ret >= 7u) {
    return -1; // matches table[3]'s literal -1 (index 3 IS 0x80041080)
  }
  switch (ret) {
  case 0: { // table[0] = 0x80040FE0
    ...
    int32_t r3;
    if (state == 2u) {
      c->mem_w8(obj + OBJ_FLAGS_71, state);
      r3 = 1;
    } else {
...
void ScriptInterp::step(uint32_t obj) {
  ...
  for (;;) {
    ...
    if (faRet != 1) {
      c->r[2] = 0u;
      return;
    } // any non-1 return exits (matches `beq s0, s2, LOOP`)
  }
}
...
int ScriptInterp::stepAngleToward(uint32_t anglePtr, int16_t targetAngle, int16_t step) {
  ...
  if (maskedDelta > 0x7FFu) {
    dirStep = (int16_t)-step;
  }
...
int ScriptInterp::op36MoveTowardScriptTarget(uint32_t obj) {
  ...
  if (phase != 1) {
    if (phase > 1) {
      if (phase != 2) {
        return epilogue(0); // dead per guest byte-shape, kept for parity
      }
      goto commit; // LAB_80043310 direct
    }
    if (phase != 0) {
      return epilogue(0); // dead: phase is guaranteed 0 in this arm
    }
    ...
    // ---- phase 0: init the movement (solve a sqrt+2-div step schedule; compute turn angle(s)). ---
    {
      ...
      if (stepDiv == 0) {
        stepDiv = 1; // clamp: guest re-checks `(quotient & 0xffff)==0` post-trunc
      }
...
commit: {
  ...
  uint16_t stepsRemaining = (uint16_t)(c->mem_r16(obj + OBJ_STEPS_REM_44) - c->mem_r16(obj + OBJ_STEP_DIV_64));
  if ((int16_t)stepsRemaining <= 0) {
    stepsRemaining = 0;
  }
...
int ScriptInterp::op31TurnTowardTarget(uint32_t obj) {
  ...
  const int16_t argA = (int16_t)c->mem_r16(obj + OBJ_ARG_A_72);
  const uint32_t target = ((uint16_t)argA & 0x8000u) ? c->mem_r32(SCRATCH_SECONDARY_ACTOR_214) : obj;
  c->r[17] = target;
  ...
  // ---- phase 0: compute a fresh target angle into obj+0x76 per the low-15-bit mode in argA. ------
  {
    ...
    if (mode == 2) {
      ...
      const int32_t y =
          (int32_t)(int16_t)c->mem_r16(target + OBJ_POS_X_36) - (int32_t)(int16_t)c->mem_r16(SCRATCH_ANCHOR_X_164);
      const int32_t x =
      ...
      const int32_t y =
          (int32_t)(int16_t)c->mem_r16(target + OBJ_POS_X_36) - (int32_t)(int16_t)c->mem_r16(obj + OBJ_POS_X_36);
      const int32_t x =
      ...
      const int32_t y =
          (int32_t)(int16_t)c->mem_r16(target + OBJ_POS_X_36) - (int32_t)(int16_t)c->mem_r16(obj + OBJ_FNPTR_HI_76);
      const int32_t x =
...
void eov_op04SceneFlagRendezvous(Core *c) {
  c->r[2] = (uint32_t)eng(c).script.op04SceneFlagRendezvous(c->r[4]);
}
void eov_loadNextEntry(Core *c) {
  c->r[2] = (uint32_t)eng(c).script.loadNextEntry(c->r[4], c->r[5]);
}
void eov_op05WaitFrames(Core *c) {
  c->r[2] = (uint32_t)eng(c).script.op05WaitFrames(c->r[4]);
}
void eov_op06TestSceneFlag(Core *c) {
  c->r[2] = (uint32_t)eng(c).script.op06TestSceneFlag(c->r[4]);
}
void eov_op34ClaimGate(Core *c) {
  c->r[2] = (uint32_t)eng(c).script.op34ClaimGate(c->r[4]);
}
void eov_op36MoveTowardScriptTarget(Core *c) {
  c->r[2] = (uint32_t)eng(c).script.op36MoveTowardScriptTarget(c->r[4]);
}
void eov_op31TurnTowardTarget(Core *c) {
  c->r[2] = (uint32_t)eng(c).script.op31TurnTowardTarget(c->r[4]);
}
...
void eov_matchesActiveByKind(Core *c) {
  c->r[2] = (uint32_t)eng(c).script.matchesActiveByKind(c->r[4]);
}
void eov_mirrorGlobalStatusByte(Core *c) {
  c->r[2] = (uint32_t)eng(c).script.mirrorGlobalStatusByte();
}
void eov_advanceGauge(Core *c) {
  c->r[2] = (uint32_t)eng(c).script.advanceGauge(c->r[4], c->r[5]);
}
} // namespace
