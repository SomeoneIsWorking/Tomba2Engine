...
void SceneTransition::areaMaskTrigger(uint8_t area, uint8_t sub) {
  ...
  auto impl = [&](uint8_t a, uint8_t s) {
    ...
    if (cur == 5) {
      c->mem_w8(0x800BF9DBu, (uint8_t)(c->mem_r8(0x800BF9DBu) | 0x02));
      return;
    }
    if (cur == 6) {
      c->mem_w8(0x800BF9DBu, (uint8_t)(c->mem_r8(0x800BF9DBu) | 0x04));
      return;
    }
    if (cur == 7) {
      c->mem_w8(0x800BF9DBu, (uint8_t)(c->mem_r8(0x800BF9DBu) | 0x08));
      return;
    }
    if (cur == 8) {
      c->mem_w8(0x800BF9DBu, (uint8_t)(c->mem_r8(0x800BF9DBu) | 0x10));
    }
  };
  ...
  if (ro >= 0 || so >= 0) {
    if (nb++ < 40) {
      cfg_logi("scene_transitionverify", "MISMATCH area=%u sub=%u ram@%x spad@%x", area, sub, ro, so);
    }
  } else if (++ng % 50 == 0) {
    cfg_logi("scene_transitionverify", "%ld matches", ng);
  }
}
...
void SceneTransition::clearSwapBlock(uint32_t node) {
  Core *c = core;
  if (c->mem_r8(node + 0x146) == 4 && c->mem_r8(node + 0) == 2) {
    return;
  }
...
int SceneTransition::stepSwapWaiter(uint32_t node) {
  ...
  auto impl = [&](uint32_t n) -> int {
    ...
    auto second_branch_gate = [&]() -> bool {
      if (c->mem_r8(n + 0x29) == 0) {
        return false;
      }
      if (c->mem_r8(E7EA9_ARMED) == 0) {
        return false;
      }
      if (c->mem_r8(E7FFB_INHIBIT) != 0) {
        return false;
      }
      return true;
    };
    auto pause_latch_tail = [&]() {
      if (c->mem_r8(IF800137_PAUSE) == 0) {
        c->mem_w8(IF800137_PAUSE, 2);
      }
    };
    ...
    case 0: {
      ...
      if (c->mem_r8(BF818_SWAP_PHASE) != 5) {
        return 0;
      }
      if (c->mem_r8(BF817_SWAP_KEY) != c->mem_r8(n + 3)) {
        return 0;
      }
      c->mem_w8(n + 6, (uint8_t)(case_id + 1));
      beginSwap(n);
    ...
    case 1: {
      ...
      if (dir == 0) {
        ...
        if (v < -1024) {
          c->mem_w16(n + 0x50, (uint16_t)-1024);
          clampHit = 1;
        }
      } else if (dir == 1) {
        int16_t v = (int16_t)(c->mem_r16(n + 0x50) + 64);
    ...
    case 2: {
      if (c->mem_r8(BF818_SWAP_PHASE) != 2) {
        return 0;
      }
    ...
    case 3: {
      ...
      if (c->mem_r8(BF818_SWAP_PHASE) != 6) {
        return 0;
      }
      if (c->mem_r8(BF80F_SUSPEND) == 0) {
        c->mem_w8(n + 6, (uint8_t)(case_id + 1));
    ...
    case 4: {
      if (c->mem_r8(BF818_SWAP_PHASE) != 4) {
        return 0;
      }
      if (c->mem_r8(BF80F_SUSPEND) != 0) {
        return 0;
  ...
  if (ro >= 0 || so >= 0 || rv_n != rv_s) {
    if (nb++ < 40) {
      cfg_logi("subswapverify",
               "MISMATCH node=%08x case=%u ram@%x spad@%x rv_n=%d rv_s=%d",
               node,
               c->mem_r8(node + 6),
               ro,
               so,
    ...
    cfg_logi("subswapverify", "%ld matches", ng);
  }
  return rv_n;
}
