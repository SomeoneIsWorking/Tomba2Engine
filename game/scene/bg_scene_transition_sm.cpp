...
void BgSceneTransitionSm::fadeRect(Core *c, uint32_t color) {
  cfg_logf("fadesites", "[fadesite] BgSm-state%u color=%06x dir=%u", c->mem_r8(P + 4), color, c->mem_r8(P + 3));
...
void BgSceneTransitionSm::audioStub26470(Core *c) {
  if (midTransitionGate(c)) {
    audioFadeTarget(c, 0);
  }
}
void BgSceneTransitionSm::audioStub26510(Core *c) {
  if (midTransitionGate(c)) {
    audioFadeTarget(c, -1);
  }
}
...
void BgSceneTransitionSm::verifyBody(Core *c) {
  ...
  if (ro >= 0 || so >= 0) {
    if (nb++ < 40) {
      cfg_logi("bgscenesmverify",
               "MISMATCH st=%u ram@%x (nat=%02x ora=%02x) spad@%x",
               c->mem_r8(P + 4),
               ro,
               ro >= 0 ? ramN[ro] : 0,
               ro >= 0 ? c->ram[ro] : 0,
    ...
    cfg_logi("bgscenesmverify", "%ld matches", ng);
  }
}

void BgSceneTransitionSm::step() {
  verifyBody(core);
}
...
L_advance:;
  ...
  c->mem_w8((c->r[16] + NODE_SUBSTATE), (uint8_t)c->r[2]);
  goto L_ret0;
...
static void eov_opSceneEventArmWait(Core *c) {
  BgSceneTransitionSm::opSceneEventArmWait(c);
}
static void eov_opClearSceneFlag80a(Core *c) {
  BgSceneTransitionSm::opClearSceneFlag80a(c);
}
