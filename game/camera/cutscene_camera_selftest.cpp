...
void snap(Core *c, uint32_t buf[NREG][MAXW]) {
  for (int r = 0; r < NREG; r++) {
    for (uint32_t i = 0; i < REGIONS[r].words; i++) {
      buf[r][i] = c->mem_r32(REGIONS[r].base + i * 4);
    }
  }
}
void restore(Core *c, uint32_t buf[NREG][MAXW]) {
  for (int r = 0; r < NREG; r++) {
    for (uint32_t i = 0; i < REGIONS[r].words; i++) {
      c->mem_w32(REGIONS[r].base + i * 4, buf[r][i]);
    }
  }
}
...
void seed(Core *c, CamTestState &ts) {
  for (int r = 0; r < NREG; r++) {
    for (uint32_t i = 0; i < REGIONS[r].words; i++) {
      c->mem_w32(REGIONS[r].base + i * 4, ts.rnd());
    }
  }
  for (int i = 0; i < 64; i++) {
    c->mem_w32(P0 + i * 4, ts.rnd());
    c->mem_w32(P1 + i * 4, ts.rnd());
  }
...
int check(Core *c,
  ...
  if (cmpStack) {
    for (uint32_t i = 0; i < STKW; i++) {
      stkMine[i] = c->mem_r32(stkBase + i * 4);
    }
    for (uint32_t i = 0; i < STKW; i++) {
      c->mem_w32(stkBase + i * 4, stkIn[i]); // reset for the oracle leg
    }
  }
  ...
  c->recMissTolerant = false;
  if (c->recMissed) {
    return -1; // oracle couldn't evaluate this synthetic state -> skip (not a failure)
  }
  ...
  for (int r = 0; r < NREG; r++) {
    for (uint32_t i = 0; i < REGIONS[r].words; i++) {
      ...
      if (mine[r][i] != o) {
        ...
        if (ts.reported++ < 30) {
          cfg_logi("camtest",
                   "MISMATCH %-10s %s+0x%03x  mine=%08x oracle=%08x",
                   name,
                   REGIONS[r].name,
                   i * 4,
                   mine[r][i],
  ...
  if (cmpStack) {
    for (uint32_t i = 0; i < STKW; i++) {
      ...
      if (stkMine[i] != o) {
        ...
        if (ts.reported++ < 30) {
          cfg_logi(
              "camtest", "MISMATCH %-10s stack sp-0x%02x  mine=%08x oracle=%08x", name, (STKW - i) * 4, stkMine[i], o);
        }
      }
    }
  }
...
int run_camera_oracle(const char *path) {
  ...
  const Case cases[] = {
      {"trackXZ", 0x8006D960u, 1},        {"trackY", 0x8006DA54u, 1},      {"distSolve", 0x8006D2ACu, 0},
      {"pitch", 0x8006D654u, 0},          {"yFloor", 0x8006C80Cu, 0},      {"heading", 0x8006DCF4u, 0},
      {"angleStep", 0x8006E010u, 0},      {"rotBuild", 0x8006E464u, 0},    {"lookAt", 0x8006D02Cu, 0},
      {"snapFollow", 0x8006E3B0u, 1},     {"mainFollow", 0x8006E0F0u, 0},  {"simpleFollow", 0x8006E3F4u, 1},
      {"trackFollow", 0x8006E228u, 1},    {"snapFollowA", 0x8006E294u, 1}, // driver mode 2
  ...
  for (int ci = 0; ci < NC; ci++) {
    ...
    for (int it = 0; it < ITERS; it++) {
      if (cfg_on("PSXPORT_SELFTEST_VERBOSE")) {
        cfg_logi("camtest", " %s iter %d seed=%08x", t.name, it, ts.seed);
        fflush(stderr);
      }
      seed(c, ts);
      ...
      if (ci == 22 || ci == 23) {
        c->mem_w8(0x800BF870u, (uint8_t)(ts.rnd() % 22));
      }
      ...
      if (ci == 24) {
        c->mem_w8(CAM + 0x76, (uint8_t)(ts.rnd() % 12)); // shakeTail: sweep every state + default
      }
      ...
      if (ci == 26 && (ts.rnd() & 3)) {
        c->mem_w8(0x1F800236u, (uint8_t)(3 + (ts.rnd() & 1)));
      }
      ...
      case 18:
        ...
        break; // a1=TGT (nonzero) — matches oracle a1
      case 19:
        nat = [&] {
          cam.headBuildB();
        };
    ...
    cfg_logi("camtest",
             "%-12s : %s  (%d mismatching words over %d verified iters, %d oracle-skipped)",
             t.name,
             bad ? "FAIL" : "ok",
             bad,
             ran,
  ...
  cfg_logi("camtest",
           "DONE: %d methods, %d runs (%d oracle-skipped), %d mismatching words total -> %s",
           NC,
           total_runs,
           total_skip,
           total_bad,
