...
void MusicCoord::cutIfDialog() {
  if (dialogToneActive() && xa_stream_is_looping(&core->game->xa)) {
...
struct VoiceChannel {
  ...
  uint32_t flags() const {
    return c->mem_r32(base);
  }
  void setFlags(uint32_t v) {
    c->mem_w32(base, v);
  }
  void setPan(uint16_t v) {
    c->mem_w16(base + 0x06u, v);
    c->mem_w16(base + 0x04u, v);
  }
  void setVolume(uint16_t v) {
    c->mem_w16(base + 0x12u, v);
    c->mem_w16(base + 0x10u, v);
  }
  int16_t baseVol() const {
    return (int16_t)c->mem_r16(base + 0x28u);
  }
  int16_t fadeTarget() const {
    return (int16_t)c->mem_r16(base + 0x2Au);
  }
  int16_t fadeCur() const {
    return (int16_t)c->mem_r16(base + 0x2Cu);
  }
  void setFadeCur(int32_t v) {
    c->mem_w16(base + 0x2Cu, (uint16_t)v);
  }
  int16_t gain2Target() const {
    return (int16_t)c->mem_r16(base + 0x2Eu);
  }
  uint16_t gain2Cur() const {
    return (uint16_t)c->mem_r16(base + 0x30u);
  }
  void setGain2Cur(int32_t v) {
    c->mem_w16(base + 0x30u, (uint16_t)v);
  }
  bool lowVolArmed() const {
    return c->mem_r8(base + 0x33u) != 0;
  }
  void disarmLowVol() {
    c->mem_w8(base + 0x33u, 0);
  }
};
...
void MusicCoord::voiceMixTick(uint32_t voice_base) {
  ...
  // PSXPORT_DEBUG=vmt — voiceMixTick trace (RE/SBS diagnostic: docs/findings/audio.md "native_sync vs
...
void MusicCoord::tick() {
  ...
  if (s != mPrev || a != mPa || l != mPl) {
    lucent::debug("coord",
                  "song={} tone={} xa_active={} loop={} pending={}",
                  s,
                  dialogToneActive(),
                  a,
                  l,
                  c->game->cd.pending_music);
    mPrev = s;
    mPa = a;
    mPl = l;
  }
  if (dialogToneActive()) {
    if (xa_stream_is_looping(&c->game->xa)) {
      xa_stream_stop(&c->game->xa); // dialog up: silence ingame music (kept pending)
    }
  } else if (c->game->cd.pending_music && !xa_stream_is_active(&c->game->xa)) {
    xa_stream_play(&c->game->xa,
                   c->game->cd.pm_chan,
                   c->game->cd.pm_start,
                   c->game->cd.pm_end,
                   1); // dialog over: resume ingame music
...
void MusicCoord::fieldBgmDirector() {
  ...
  if (started) {
    ...
    if (gctx(c)->music_list.nowPlaying() < 0) {
      started = 0;
    } else {
      return;
    }
  ...
  if (memcmp(b + 0x30, "pQES", 4) || memcmp(b + 0x26b4, "pBAV", 4)) {
    return;
  }
  int song = 8; // default field theme (longest area seq)
  if (const char *s = cfg_str("PSXPORT_FIELD_SONG")) {
    int v = atoi(s);
    if (v >= 0 && v < 10) {
      song = v;
    }
  }
  if (gctx(c)->music_list.playArea(b, 0x50000, song) == 0) {
    started = 1;
  }
}
