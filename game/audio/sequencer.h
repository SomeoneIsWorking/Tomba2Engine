...
// recorded binary evidence's indirect-call discovery never saw them — they only run via the hybrid interpreter
// today (game/game_tomba2.cpp SEQ_TICK_WRAPPER, called once per ov_frame_update).
//
// 2026-07-09 wide-RE wave — SsSeqCalled (0x80090BD0) cluster. RE'd via authenticated executable/overlay evidence:
...
// of the current guest-visible behavior (32784u<<16 = 0x80100000, +19492/19496/19504/21680/21682 decimal).
//
...
//                            True leaf, no stack frame in the guest-visible behavior.
...
//                            (authenticated executable/overlay evidence); FIELD SEMANTICS beyond +72/+74/+152/
...
//                            bit6/bit7. Control flow fully RE'd (authenticated executable/overlay evidence).
...
//                            (authenticated executable/overlay evidence). SsSeqCalled itself, after dispatching
...
// typed runtime address dispatch(c, addr) call-outs, same pattern frameTick() already uses for SsSeqCalled itself.
...
//     kept register-literal with goto/labels named after the guest addresses (the guest-visible behavior has
...
//     voice-register write (0x80095530, authenticated executable/overlay evidence, ~320 lines, a KON/pan-table
//     loop over the hardware voice-bit array) stays MAPPED/typed runtime address dispatch — too large/deep for this
//     pass, see docs/engine_re.md.
//   bit6/bit7 (0x80092080) -> channelEnvelopeRampTick(): ADSR ramp. TRUE LEAF in the guest-visible behavior (no
...
//     L_80092284). No unowned callees (the guest-visible behavior's trailing `guest 0x800922A0(c)` after the real
...
//       write of the combined seq|chan<<8 arg to a scratch global (0x80105D0C) that the guest-visible behavior
...
//
// 2026-07-10 (2nd wide-RE wave, disjoint band) — closes the "0x80095530 stays MAPPED/typed runtime address dispatch,
...
//   channelVoiceRegisterWrite() (0x80095530, authenticated executable/overlay evidence, 320 lines) — the "SPU
//     voice-register write leaf" channelPitchSlideTick() calls via typed runtime address dispatch. ABI a0(r4)=
...
//     retired comparison path's SPU-adjacent scratch state byte-for-byte.
//   channelVoiceSelectPrep() (0x800962B0, authenticated executable/overlay evidence, 49 lines) — TRUE LEAF, no
...
// tomba::native::declareOverride + VERIFIED (SBS-full 0-diff through f9030; MIRROR_VERIFY 23k armed tick
...
// seqChannelDispatch routes their bits to the substrate via typed runtime address dispatch): channelReleaseClear,
...
// cluster wiring", 2026-07-10). NOTE: historical two-core comparison skipped this audio block,
...
public:
  ...
  //   guest 0x800909C0 (authenticated executable/overlay evidence): if the user-cb slot (0x800AC430) is
  //   non-null, typed runtime address dispatch() it; then unconditionally typed runtime address dispatch() the
  //   SsSeqCalled pointer at 0x800AC42C (today: seqChannelDispatch() below, still routed through typed runtime address
  //   dispatch since this draft doesn't rewrite frameTick's own dispatch target — see header note above).
  void frameTick();
  ...
  //     typed runtime address dispatch()es to, plus its own callee. See header comment for RE detail / confidence.
  ...
  // verification-gated (the substrate leg runs the original guest body; the native leg runs this implementation). This
  // is the SAME mechanism typed runtime address dispatch's runtime address dispatch() ultimately reaches for MAIN-shard
  // addresses, and ALSO the mechanism direct in-body calls like `guest 0x800910F0(c)` inside
  // guest 0x80090BD0 reach — one registration covers both call paths. See sequencer.cpp for the
