...
static uint16_t rd16(const uint8_t *b, long o) {
  return b[o] | (b[o + 1] << 8);
}
static uint32_t rd32(const uint8_t *b, long o) {
  return b[o] | (b[o + 1] << 8) | (b[o + 2] << 16) | ((uint32_t)b[o + 3] << 24);
}
static uint16_t rd16be(const uint8_t *b, long o) {
  return (b[o] << 8) | b[o + 1];
}
static uint32_t rd24be(const uint8_t *b, long o) {
...
static int
adpcm_decode_full(const uint8_t *buf, long off, int nbytes, int16_t *out, int outcap, int *loop_start, int *loops) {
  int h1 = 0, h2 = 0, n = 0;
  *loop_start = -1;
  *loops = 0;
...
int na_vab_open_at(NaVab *v, const uint8_t *buf, long o) {
  v->buf = buf;
  v->base = o;
  if (memcmp(buf + o, "pBAV", 4)) {
    return -1;
  }
  v->ver = rd32(buf, o + 4);
  v->id = rd32(buf, o + 8);
  v->fsize = rd32(buf, o + 12);
  v->ps = rd16(buf, o + 16 + 2);
  v->ts = rd16(buf, o + 16 + 4);
  v->vs = rd16(buf, o + 16 + 6);
  v->progtab = o + 0x20;
  v->tonetab = o + 0x820;
...
static long vag_off(const NaVab *v, int idx, int *sz) {
  long off = 0;
  for (int i = 1; i < idx; i++) {
    off += (long)rd16(v->buf, v->vagtab + i * 2) * 8;
  }
  *sz = (int)rd16(v->buf, v->vagtab + idx * 2) * 8;
  return v->body + off;
}
...
void na_tone_read(const NaVab *v, int t, NaTone *o) {
  ...
  o->pan = b[a + 3];
  o->centre = b[a + 4];
  o->shift = (int8_t)b[a + 5];
  o->nmin = b[a + 6];
  o->nmax = b[a + 7];
  o->adsr1 = rd16(b, a + 0x10);
  o->adsr2 = rd16(b, a + 0x12);
  o->prog = rd16(b, a + 0x14);
  o->vag = b[a + 0x16];
}
...
int na_vab_decode_vag(const NaVab *v, int vagIdx, int16_t *out, int outcap, int *loop_start, int *loops) {
  int sz;
  long off = vag_off(v, vagIdx, &sz);
  if (sz <= 0) {
    *loop_start = -1;
    *loops = 0;
    return 0;
...
static int prog_pick_tones(const NaVab *v, int prog, int note, int *out, int cap) {
  ...
  int nt = v->buf[v->progtab + prog * 16];
  if (!nt) {
    return 0;
  }
  ...
  for (int i = 0; i < nt && n < cap; i++) {
    NaTone t;
    na_tone_read(v, start + i, &t);
    if (note >= t.nmin && note <= t.nmax) {
      out[n++] = start + i;
    }
  }
  return n;
...
// ===========================================================================================
static void calc_vc_delta(
    int zs, int speed, int log_mode, int dec_mode, int inv_increment, int16_t Current, int *increment, int *divinco) {
...
static void voice_run_env(NaVoice *v) {
  int inc = 0, divinco = 0;
  int16_t uoflow = 0;
  if (v->phase == NA_ADSR_ATTACK && v->env == 0x7FFF) {
    v->phase++;
  }
  switch (v->phase) {
  case NA_ADSR_ATTACK:
  ...
  if (v->divider & 0x8000) {
    ...
    if (v->phase == NA_ADSR_DECAY && v->env < v->SusLevel) {
      v->phase++;
    }
    if (v->phase == NA_ADSR_RELEASE && v->env == 0) {
      v->active = 0;
    }
  }
}
...
static NaVagPcm *vag_get(NaSeq *s, int vagIdx) {
  ...
  int sz;
  long off = vag_off(s->vab, vagIdx, &sz);
  if (sz <= 0) {
    return NULL;
  }
  int cap = (sz / 16) * 28 + 64;
...
static void voice_keyon(
    NaVoice *vc, NaSeq *s, const NaTone *t, int note, int vel, int chan, int chan_vol, int chan_pan, uint64_t age) {
...
static void voice_render(NaVoice *v, int *outL, int *outR) {
  ...
  if (i1 >= v->nsamp) {
    i1 = (v->loops && v->loop_start >= 0) ? v->loop_start : i0;
  }
  int frac = v->pos_frac;
...
static NaVoice *seq_alloc_voice(NaSeq *s) {
  ...
  for (int i = 0; i < NA_NVOICE; i++) {
    ...
    if (s->voice[i].age < oldest) {
      oldest = s->voice[i].age;
      best = &s->voice[i];
    }
  }
  return best;
...
static int seq_event(NaSeq *s) {
  ...
  switch (hi) {
  case 0x90: {
    int n = s->buf[s->p++], v = s->buf[s->p++];
    seq_note_on(s, ch, n, v);
    break;
  }
  case 0x80: {
    int n = s->buf[s->p++], v = s->buf[s->p++];
...
void na_seq_free(NaSeq *s) {
  for (int i = 0; i < 256; i++) {
    free(s->vagcache[i].data);
    s->vagcache[i].data = NULL;
  }
}

static double seq_spt(NaSeq *s, int rate) {
  return s->tempo_us * rate / (s->ppqn * 1.0e6);
}
...
int na_seq_render(NaSeq *s, int16_t *out, int nframes, int rate) {
  ...
  while (produced < nframes) {
    ...
    s->_pending -= 1.0;
    if (s->ended && !any) {
      break; // song over and all voices drained
    }
  }
  return produced;
}
