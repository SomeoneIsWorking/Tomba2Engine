...
//
// Build:  cmake --build build --target menu_bg_export.
...
private:
  ...
  uint32_t m_hunk_bytes = 0, m_frames_per_hunk = 0, m_hunk_count = 0;
...
bool FindFile(ChdDisc &disc, const std::string &path, IsoFile *out) {
  ...
  for (size_t k = 0; k < parts.size(); k++) {
    ...
    for (uint32_t i = 0; i < nsec; i++) {
      disc.ReadSector(lba + i, buf.data() + (size_t)i * kUserDataSize);
    }
    for (uint32_t s = 0; s < buf.size() && !found; s += kUserDataSize) {
      for (uint32_t pos = s; pos < s + kUserDataSize;) {
        ...
        const uint32_t e_lba = rd32(&buf[pos + 2]), e_size = rd32(&buf[pos + 10]);
        pos += len;
...
std::vector<uint8_t> ReadFile(ChdDisc &disc, const IsoFile &f) {
  ...
  std::vector<uint8_t> buf((size_t)nsec * kUserDataSize);
  for (uint32_t i = 0; i < nsec; i++) {
    disc.ReadSector(f.lba + i, buf.data() + (size_t)i * kUserDataSize);
  }
  buf.resize(f.size);
  return buf;
}
...
void apply_archive(const uint8_t *arc, uint32_t len, bool *has_title, bool *has_clut) {
  ...
  const uint8_t *entry = arc + 4;
  const uint8_t *src = arc + 0x800;
    ...
    for (int r = 0; r < field; r++) {
      for (int c = 0; c < stride; c++) {
        ...
        if (vx >= 0 && vx < VRAM_W && vy >= 0 && vy < VRAM_H) {
          g_vram[vy * VRAM_W + vx] = px[r * stride + c];
        }
      }
    }
    if (has_title && x == 640 && y == 256) {
      *has_title = true;
    }
    if (has_clut && x == 640 && y >= 504 && y <= 511) {
      *has_clut = true;
    }
    src += srclen;
  }
...
int main(int argc, char **argv) {
  ...
  if (!disc_path) {
    std::fprintf(stderr, "no disc image (arg/env/drop-in)\n");
    return 1;
  }
  ChdDisc disc;
  if (!disc.Open(*disc_path)) {
    std::fprintf(stderr, "cannot open CHD %s\n", disc_path->c_str());
    return 1;
  }
  std::printf("disc: %s\n", disc_path->c_str());
  ...
  for (uint32_t s = 0; s < nsets; s++) {
    ...
    if (ht || hc) {
      std::printf("set %2u: archive[%u,%u)%s%s\n",
                  ...
                  h0,
                  h1,
                  ht ? "  <= places (640,256) TITLE PAGE" : "",
                  hc ? "  <= places (640,~507) CLUT" : "");
    }
    if (ht && title_set < 0) {
      title_set = (int)s;
    }
  ...
  if (title_set < 0) {
    std::fprintf(stderr, "no set places exactly at VRAM (640,256) — title page not found\n");
    return 1;
  }
  ...
  uint32_t order[2];
  int no = 0;
  order[no++] = (uint32_t)title_set;
  if (clut_set >= 0 && clut_set != title_set) {
    order[no++] = (uint32_t)clut_set;
  }
  for (int k = 0; k < no; k++) {
    uint32_t s = order[k], h0 = rd32(&idx[s * 2048]), h1 = rd32(&idx[s * 2048 + 4]);
    bool d0 = false, d1 = false;
    apply_archive(&img[h0], h1 - h0, &d0, &d1);
  }
