#!/usr/bin/env python3
"""Drivable core sessions for the Tomba! 2 oracle comparison.

Two cores, one narrow interface: the Lightrec product driven through its REPL over pipes, and the
independent Beetle full-console reference driven through psxport's `tools/oracle/console.py` JSON
protocol. The title-owned policy in `oracle_tomba2.py` drives either without knowing which it is:
`hold(buttons)`, `step(frames)`, `read(address, size)`.
...
class CoreSession(Protocol):
    ...
    vblank_stepped: bool  # True when step(1) is one VBlank rather than one game frame
...
class NativeReplSession:
    """The built tomba2_port binary, driven through PSXPORT_REPL=1 over stdin with its lucent log on
    ...
    name = "native"
    vblank_stepped = False  # the product's frame driver runs exactly one game frame per step
    ...
    def step(self, frames: int) -> None:
        ...
        self.read(0x801FE00C, 4)  # the reply is the barrier: it prints only after the frames ran
        self.frames += frames
...
class ConsoleSession:
    ...
    name = "console"
    vblank_stepped = True  # Beetle steps by VBlank; the title decides what a game frame is
    _MAX_STEP = 3600
    _MAX_READ = 256
