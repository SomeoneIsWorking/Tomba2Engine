...
static long vab_file_off(const uint8_t *snd, int vi) {
  return (long)(snd[vi * 2] | (snd[vi * 2 + 1] << 8)) * 0x800;
}
...
int main(int argc, char **argv) {
  ...
  long sndlen;
  uint8_t *snd = load(sndpath, &sndlen);
  ...
  struct Item {
    ...
    int vab;
      ...
      {"SND song8", "", 0x1f58, 21},
      {"SND song9", "", 0x25b0, 7},
      {"A05 #0", "scratch/disc_files/BIN/A05.BIN", 0x3756c, 7},
      {"A05 #1", "scratch/disc_files/BIN/A05.BIN", 0x37710, 7},
      ...
      {"A0K #0", "scratch/disc_files/BIN/A0K.BIN", 0x14634, 7},
      {"A0K #1", "scratch/disc_files/BIN/A0K.BIN", 0x147d8, 7},
      {"A0L #0", "scratch/disc_files/BIN/A0L.BIN", 0xea44, 7},
      {"A0L #1", "scratch/disc_files/BIN/A0L.BIN", 0xfb0c, 7},
  ...
  for (int i = 0; i < N; i++) {
    long flen;
    uint8_t *fb = items[i].file[0] ? load(items[i].file, &flen) : snd;
    NaVab vab;
    NaSeq s;
    ...
    if (okv == 0 && oks == 0) {
      int done = 0;
      while (done < RATE * SECS) {
        int got = na_seq_render(&s, seg + done * 2, RATE * SECS - done, RATE);
        if (got <= 0) {
          break;
        }
        done += got;
  ...
  fseek(w, 0, SEEK_SET);
  uint32_t riff = 36 + datasz, fmtlen = 16, srate = RATE, byterate = RATE * 4;
