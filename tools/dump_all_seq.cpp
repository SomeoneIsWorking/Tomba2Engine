...
struct Chd {
  chd_file *c = nullptr;
  uint32_t hb = 0, fph = 0, hc = 0, cached = UINT32_MAX;
  std::vector<uint8_t> buf;
  bool Open(const std::string &p) {
    if (chd_open(p.c_str(), CHD_OPEN_READ, nullptr, &c) != CHDERR_NONE) {
      return false;
    }
  ...
  bool Sec(uint32_t lba, uint8_t *out) {
    uint32_t hk = lba / fph, off = (lba % fph) * RAW;
    if (hk >= hc) {
      return false;
    }
    if (hk != cached) {
      if (chd_read(c, hk, buf.data()) != CHDERR_NONE) {
        return false;
      }
      cached = hk;
    }
    const uint8_t *raw = buf.data() + off;
    uint32_t d = (raw[15] == 2) ? 24 : 16;
    memcpy(out, raw + d, USR);
    return true;
  }
};
...
static std::string Norm(std::string n) {
  if (auto s = n.find(';'); s != std::string::npos) {
    n.resize(s);
  }
  for (char &c : n) {
...
static void Walk(Chd &disc, uint32_t lba, uint32_t size, const std::string &path, const fs::path &outdir) {
  uint32_t nsec = (size + USR - 1) / USR;
  std::vector<uint8_t> dir((size_t)nsec * USR);
  for (uint32_t i = 0; i < nsec; i++) {
    if (!disc.Sec(lba + i, dir.data() + (size_t)i * USR)) {
      return;
    }
  }
  for (uint32_t s = 0; s < dir.size(); s += USR) {
    uint32_t pos = s;
    while (pos < s + USR) {
      uint8_t len = dir[pos];
      if (!len) {
      ...
      uint32_t e_lba = LE32(&dir[pos + 2]), e_size = LE32(&dir[pos + 10]);
      pos += len;
      if (nlen == 1 && (nm[0] == 0 || nm[0] == 1)) {
        continue; // . and ..
      }
      std::string name = Norm(nm);
      std::string full = path;
      full += '/';
      full += name;
      if (flags & 0x02) {
        Walk(disc, e_lba, e_size, full, outdir);
        continue;
      } // subdir
      // file: read it, scan for pQES/pBAV
      uint32_t fsec = (e_size + USR - 1) / USR;
      std::vector<uint8_t> fb((size_t)fsec * USR);
      for (uint32_t i = 0; i < fsec; i++) {
        disc.Sec(e_lba + i, fb.data() + (size_t)i * USR);
      }
      fb.resize(e_size);
      long nq = 0, nv = 0;
      for (size_t i = 0; i + 4 <= fb.size(); i++) {
        if (!memcmp(&fb[i], "pQES", 4)) {
          nq++;
        } else if (!memcmp(&fb[i], "pBAV", 4)) {
          nv++;
        }
      }
      if (nq || nv) {
        ...
        fs::path op = outdir / full.substr(1);
...
int main(int argc, char **argv) {
  ...
  uint8_t sec[USR];
  if (!d.Sec(16, sec) || memcmp(sec + 1, "CD001", 5)) {
    fprintf(stderr, "no PVD\n");
    return 1;
  }
