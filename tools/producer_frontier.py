...
def main() -> int:
    ...
    for a in addrs:
        ...
        (owned if ok else unowned).append((a, detail))
    ...
    print(f"# queried {len(addrs)} address(es): {len(owned)} OWNED (listed), {len(unowned)} UNOWNED")
    for a, detail in owned:
        print(f"0x{a}   # {detail}")
    ...
    print(f"\n[frontier] {len(owned)} owned, {len(unowned)} UNOWNED (these remain judgement calls):",
          file=sys.stderr)
    for a, _ in unowned:
