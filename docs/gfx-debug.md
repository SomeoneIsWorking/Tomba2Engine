...
`ringcensus`, `quadrtpt`, `attract` — each invented for one effect, each carrying its own hand-maintained
...
| **`debug fadeshot`** | On every guest instruction path screen-fade call `FUN_8007E9C8(color=a0)` (interp.cpp hook), log `call=N color=0xRRGGBB ra=` and capture `s_tex` to `scratch/screenshots/fade_NNN.ppm`. Deterministically captures a transition's fade frames (frame-indexed shots DRIFT across launches). Headless shows the CORRECT single menu→cutscene fade-in; the corrupt EXTRA menu fade only forms windowed (later-250). | `debug` channel |
| **live debug server** | NON-BLOCKING TCP server (`runtime/psx/dbg_server.c`) — inspect the port WHILE the user plays live (windowed). `r`/`rw`/`stage`/`scene`/`provat x y`/`vkvram`/`shot`/`debug <chan>`/`frame`. | `PSXPORT_DEBUG_SERVER=1` (port 5959); `external/psxport/tools/dbgclient.py <cmd...>` |
...
| **`debug otattr` + REPL `otattr`** | OT/GTE SUBMISSION ATTRIBUTION: traces every guest-submitted packet-pool store and every GTE RTPS/RTPT back to the emitting guest fn (typed runtime address dispatch shadow stack) + world-object node (same `cur_render_node` fallback the native submit path uses). Turn the channel on BEFORE the frame you want to inspect, then run the REPL `otattr` command to re-walk the last OT read-only and print `pool= op= fn= caller= node= beh@node+1C=` per packet, plus a per-(fn,node) GTE call-count histogram. Built for bug #45 (multi-quad batching) + bug #34 (dialog-panel emitter) — see `game/render/ot_attr.h`. Blind to fully-native (non-dispatched) draw paths — see the class comment for the limitation. | `debug otattr` then REPL `otattr` |
| **REPL `otattr watch/who/trace`** | LAST-WRITER PROVENANCE — answers "who WROTE this word" (any address, incl. scratchpad), independent of call-flow, for the staging-buffer case where `otattr`'s call-path attribution only names the batcher. `watch <addr> <len>` registers a region (8 slots, 64 KB total); `who <addr> [len]` dumps the word-granular last-writer, coalescing same-writer runs; `trace <addr>` prints the last writer + a copy-loop heuristic (does the writer's stores fan out across many 4KB pages this frame?) and points at a Ghidra follow-up when the source isn't statically determinable. Same shadow-stack signal as `otattr`, so it's ALSO blind past a plain (non-jalr/typed runtime address dispatch) nested call — verified on bug #45's `0x8003F9A8`: last-writer matches call-path attribution exactly (no hidden RAM-copy hop), so the true emitter leaf there needs a Ghidra decompile of `0x8003F9A8`, not this tool. See `docs/config.md` `otattr` section. | `debug otattr` then REPL `otattr watch/who/trace` |
| **`debug beamfx`** | The BEAM / see-saw ribbon producer (`Render::beamQuadRender`, guest FUN_8003B704). One line per producer call — node, kind, uv index, both world endpoints, the half-extent vector and the SCREEN BBOX of what was actually emitted — plus a per-walk SUMMARY from `Render::fieldObjectsRender` carrying the denominator (`objListWalk4 live nodes inspected=` / `routed to FUN_8003B704=`). Built so its three different silences cannot be confused: `inspected=0` = the chain was empty, `routed=0` = no node asked for a beam, `emitted=0` = the producer declined. The bbox it predicts is checkable against an A/B pixel diff. | `debug beamfx` / `PSXPORT_DEBUG=beamfx` |

| **`debug plumefx`** | The FOUR-COPY RADIAL PLUME producer (`Render::radialPlumeRender`, guest FUN_8002BC9C — `game/render/fx_plume.cpp`). One line per producer call: node, subtype byte, the animation-script frame (and whether it is the script's last), the mesh the script selected, the sort bias, the three node angles, the node world position, and `quads=` — how many records the writer's own ordering test actually let through. Its silences are separated on purpose: `no mesh loop` = the node carries no script/table (the guest emits nothing either), `mesh=00000000` = the script frame selects no mesh, `quads=0` = every record failed the ordering reject, and no line at all = the walk never reached the node. A call whose subtype is 0x14/0x15 is tagged `[sprite-half reached — NOT PORTED]`, because that branch of the controller uses a different emitter family and has no producer. | `debug plumefx` / `PSXPORT_DEBUG=plumefx` |

| **`debug attract`** | Per-frame decision of the DEMO/title ATTRACT world producer (`Render::renderAttract`, `s48==7`): whether it DREW the attract world or DECLINED because the attract item's world is not built yet. Prints `world=DRAWN|SKIPPED(item not built)`, the phase machine's own `itemBuilt` latch (`*(u8)0x1F80019A`, set by phase0's tail `0x80106D1C`, cleared by phase2 `0x80106E0C`), `phase` (`sm[0x4a]`), the item selector (`*0x800bf870`), the three field entity HEADS (`0x800FB168`/`0x800F2624`/`0x800F2738`) and **`head0.link`** = `head0+0x24`, the link word whose garbage caused the pre-existing `UNMAPPED read8 @ 0x07035D41` abort (kanban #86). **A DECLINED frame logs as loudly as a drawn one on purpose** — "the attract world is missing" and "there is no attract world yet" must not read identically, so the line always carries the state behind the decision. Expect ~2 SKIPPED frames per 1361-frame attract cycle (the cooperative area load yields); many consecutive SKIPPED frames means the latch is not being set and the world is genuinely absent. `head0 in RAM=0` says the link word was NOT read (head0 is not a RAM address), so `head0.link=00000000` there means *not sampled*, not zero. | `debug attract` / `PSXPORT_DEBUG=attract` |
| **`debug heads0`** | Per-frame census of `Render::fieldObjectsRender`'s **HEADS[0] (0x800FB168) arm**. HEADS[0] has no render walk of its own: its nodes reach the picture through the three CULL RENDER QUEUES (class +0xC -> queue -> per-type table on +0xB), which is what `game/render/queue_dispatch.{h,cpp}` models and what this arm now applies in place of the old flush-all. Per node it prints `class/type/queue/target/arm/meshArm/onFrameSubmitList`, then a VERDICT line carrying every denominator: `examined` (census entries — THREE per node per frame, since fieldObjectsRender runs three times), `meshArm`, `onSubmitList`, `drawnByGuest`, `frameSubmitListEntries` and `nativeCullPushes`. **Read those last two before believing any negative:** `frameSubmitListEntries=0` means the submission read found nothing at all (instrument), not that the nodes were rejected; `nativeCullPushes=0` is NORMAL under `PSXPORT_GATE=1`, where the whole cull family runs as the substrate (`PSXPORT_DEBUG=ovhit`: `Cull::cullWrapper native=0 / oracle=86368`) so the push-site census in `cull.cpp` is structurally blind. The membership read uses the walks' per-frame SNAPSHOT pair (A `0x1F800140/0x1F800146`, B `0x1F80014C/0x1F800152`, C `0x1F800158/0x1F80015E`), NOT the accumulator pair the cull pushes onto — the accumulator is reset by the first walk of each frame and can only ever answer zero, which is how an earlier instrument produced a false "queued by nobody, 45/45". | `debug heads0` / `PSXPORT_DEBUG=heads0` |
| **`debug cullpush`** | The cull's own PUSH-TIME submission census (`Cull::submitLogFlush`): per frame `culled / kept / routedToAQueue / capRejected / recorded / overflow`, then one line per push with the node's class, type, queue, cap-acceptance and the arm its type resolves to. It answers "what did the cull submit" at the only sample point that can. **It is silent on the `PSXPORT_GATE=1` leg by construction** (the cull runs as the substrate there) — a run with no `cullpush` lines has measured NOTHING, and the channel says so with a denominator line rather than printing nothing at all. | `debug cullpush` / `PSXPORT_DEBUG=cullpush` |

### A CHANNEL INSIDE A NATIVE OVERRIDE PRINTS NOTHING UNDER `PSXPORT_GATE=1` — check before trusting silence

`PSXPORT_GATE=1` sets `Game::the retired alternate-execution flag=1`, and `override_registry.cpp`'s single dispatch decision
then runs the `gen` (guest) body for **every** registered address — measured 2026-08-06:
`PSXPORT_DEBUG=ovhit` reports `native=0` for **all 482** of them. So any `lucent::debug` living in a
native override body is structurally silent in the standard measurement mode. Confirmed silent that
way: **`walk`** (in `Render::renderWalk`) and **`quadrtpt`** (in `QuadRtptSubmit::submitQuad`).
Channels reached from `Render::fieldObjectsRender` / the present path (`beamfx`, `plumefx`, `ropeline`,
`nofx`, `ringcensus`, …) are plain calls and fire normally. **Before reading a quiet channel as a negative,
...
1. **`PSXPORT_ORACLE=1` is an OT-order diagnostic, not ground truth.** It is cheap and useful for
   ...
   **WHICH FLAG, and why it is not a detail (kanban #78, measured 2026-08-06).** `PSXPORT_RENDER_PSX=1`
   ...
   leans on `sprite_is_bg_texpage`, i.e. on a pc_render producer having published this frame's backdrop
   atlas page — and on the RENDER_PSX leg no pc_render producer runs. Measured area 0 free-roam f3100
   under `PSXPORT_GATE=1 PSXPORT_RENDER_PSX=1` (`PSXPORT_PRIMDUMP=3100:3102`): 972 prims walked = 617
   `is3d=1` world polys + 355 sprites, and **355/355 sprites carried `bg=0`** — 352 of them the 16×16
   tiles of the backdrop atlas at texpage (896,0). All of them therefore landed in `RQ_HUD`, the topmost
   2D band, and painted over the world the substrate had just drawn. The capture was SKY AND SEA ONLY,
   ...
   **The general lesson, which outlives this bug:** an instrument that shares mechanism with the thing it
   ...
   `PSXPORT_PRIMDUMP=<a>:<b>` gives the denominator (prims walked, split by `is3d`/`bg`) in one run.
...
| `tools/render_cmp.py` | Native-vs-PSX render compare at 1x/4:3/30fps → `scratch/screenshots/cmp_triptych.png` + diff %. A DIAGNOSTIC GATE (real-depth ownership is the target, NOT matching PSX). `g_render_psx`/`PSXPORT_RENDER_PSX` renders the field via the PSX guest instruction path for the compare. | CLI |
| `tools/disas.py <addr> [--ram dump]` | MAIN.EXE / overlay disassembly with resolved load/store addrs — read the recorded guest behavior body before reimplementing an engine fn. | CLI |
...
`psxport_settings.ini` (gitignored), restored next launch (`runtime/psx/mods.c`, `mods.h`). To
