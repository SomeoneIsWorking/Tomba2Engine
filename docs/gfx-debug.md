...
| **`debug heads0`** | Per-frame census of `Render::fieldObjectsRender`'s **HEADS[0] (0x800FB168) arm**, the one list the walk still flushes WHOLESALE ("table not yet RE'd"). Prints `flushed=` with a per-CLASS histogram (node+0xC, the guest's own queue-routing key) plus the three cull render-queue counters read raw. Built for kanban #77 — the guest has **no render walk over HEADS[0] at all** (its three render walks are `gen_func_8003C048` over HEADS[1]/table 0x80014DB8, objListWalk4 over HEADS[2]/table 0x80015000, and `gen_func_8003BB50` over the cull's **queue B**/table 0x80014A70), so anything this arm draws that the cull never queued is geometry vanilla does not show. Its silences are separated: no line = the walk never ran; `live=0` = the list was empty. **The queue counters print raw on purpose:** they read `(0,0,0)` at display-pass time because the queues are already reset, so a derived "not queued" boolean would be a lie — see the banner in `render_walk.cpp`. | `debug heads0` / `PSXPORT_DEBUG=heads0` |
