...
## area-slots-classify-998e4
- **status:** verified
- **frames:** 1500
- **gate:** PSXPORT_SBS_MODE=full PSXPORT_SBS_EXIT_FRAME=1500 PSXPORT_PAD_REPLAY=replays/bugs/seesaw-weight.pad
- **evidence:** 50/50 identical, ovhit 1440/1440 balanced. All four output arms are exercised in the normal field replay (the buffer is rebuilt for 24 slots every call), so unlike most ports today this one is not partial.
...
- **frames:** 1500
- **gate:** PSXPORT_SBS_MODE=full PSXPORT_SBS_EXIT_FRAME=1500 PSXPORT_PAD_REPLAY=replays/bugs/seesaw-weight.pad
- **evidence:** 50/50 identical, ovhit 214/214 balanced; five-mutation table rejects extra store, dropped spill, bad ra, deleted call, wrong frame. PARTIAL: 214 calls is low and the HIT branch requires the leading test to return non-negative AND the object to be phase 1 / kind 129 — a replay that never strikes a reacting object leaves the whole struck-state block (5 of the 6 byte stores) unexercised, so the 0-diff proves little about it.

## actor-targeting-1fae0
- **status:** partial
- **frames:** 1500
- **gate:** PSXPORT_SBS_MODE=full PSXPORT_SBS_EXIT_FRAME=1500 PSXPORT_PAD_REPLAY=replays/bugs/seesaw-weight.pad
- **evidence:** 50/50 identical, ovhit 2855/2855 balanced; five mutations rejected. PARTIAL: four sequential gates, and a replay that never completes an acquisition exercises only the early returns — the publish block (angle, type-selected param, contact stamp, global) needs all four to pass plus a clear contact-kind slot. Whether the type-14/57 arm ever ran is separately unproven.
