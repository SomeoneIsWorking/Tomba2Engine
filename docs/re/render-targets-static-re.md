...
3. OBJECT ROTATION. rec_dispatch 0x80085480 (Math::rotmat) with a0 = node+0x54, a1 = 0x1F800000 -> writes a libgte MATRIX (5 CR-packed words of s16 at +0, s32 t[3] at +0x14) into scratchpad. The Euler angles are the three s16 at node+0x54/+0x56/+0x58.
4. COLUMN SCALE. Read three s16 at node+0x68, node+0x6A, node+0x6C, sign-extend each into a s32 at sp+48/+52/+56, then rec_dispatch 0x80084520 (Math::matColScale) with a0 = 0x1F800000, a1 = sp+48 -> each element M[i][j] becomes (M[i][j] * f[j]) >> 12. Column scaling COMMUTES with the camera left-multiply that follows, so a host port may apply it either before or after the compose.
...
24. SPRITE TAIL — MODEL. table = (u8 [node+3] == 8) ? 0x8011587C : 0x80115864 ; rec0 = *(u32*)(table + frameIdx * 4). rec_dispatch 0x800328EC(a0 = rec0) — the shared wrapper over the 36-byte FOUR-CORNER writer FUN_8002847C, i.e. Render::altSpriteEmit / emitAnimQuadRecords.
...
13. BRANCH B, prelude: call ov_a04_func_8013AD90(node) — the 342-point white-tile field (see calls). This runs BEFORE any of the state stepping below and is unconditional within branch B.
