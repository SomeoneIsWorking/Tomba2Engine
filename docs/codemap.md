...
| Tomba! 2 field transition | Run the sm[0x4a]==5 sub-scene/door/area fade transition machine and its four workers, including the synchronous area load they trigger | `game/scene/field_transition.*` | `FieldTransition::step` | `docs/engine_re.md` |
...
| Memory-card LOAD/SAVE page sub-machine | Advance the card browser (DEMO title load page and pause-menu load/save pages) and own CRD image residency in the AREA slot: activate after the synchronous load, retire before the in-game area-asset reload | `game/scene/card_load_machine.*` | `tomba::scene::stepCardLoadMachine` | `docs/engine_re.md` |
...
| Tomba! 2 authored full-screen 2D page background, and its widened margins | Own each authored full-screen page's gradient once, and continue its own edge colour across the canvas an authored 4:3 page leaves showing | `game/render/page_gradient.*` (the gradients and the band rule), `game/render/page_backdrop.*` (drawing) | `tomba::render::PageGradient::optionsPage`, `PageBackdrop::pushAuthored`, `PageBackdrop::pushMargins` | `docs/issues/0010-full-screen-2d-pages-keep-a-4x3-extent-in-widescreen.md` |
...
| Independent behavior comparison | Drive the Lightrec product and PSXPort's Beetle full-console reference to title-declared state checkpoints and compare declared main-RAM ranges frame by frame | `tools/oracle_compare.py` (report), `tools/oracle_tomba2.py` (Tomba! 2 predicates, declared/excluded state, game-frame barrier, input schedule), `external/psxport/tools/oracle/compare.py` and `compare_cores.py` (the shared driver, REPL and console sessions), `tools/gate.py` `native_environment` (the one product launch environment) | `tools/oracle_compare.py --frame-step 1` | `docs/findings/tooling.md` "Oracle comparison" |
