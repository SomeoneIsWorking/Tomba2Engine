...
Totals: 1037 native fns, 872 owned addresses, 1028 LIVE / 9 ORPHAN. 474 override install sites over 474 addresses.
...
| 0x800286CC | LIVE | `Render::fxAnimSpriteRender` | game/render/fx_sprite.cpp:571 |  | The FUN_800286CC emitter, rebuilt: read the effect node's own animatio… |
...
| 0x8002A834 | LIVE | `Render::swingStarburstRender` | game/render/fx_swing.cpp:46 |  | Guest FUN_8002A834's ten-copy weapon-charge starburst, rebuilt from th… |
...
| 0x8002B3A4 | LIVE | `Render::fxSpriteEmit` | game/render/fx_sprite.cpp:417 |  | the shared sprite-family body owns this controller's ring-rotation bra… |
...
| 0x800328EC | LIVE | `Render::altSpriteEmit` | game/render/fx_sprite.cpp:646 |  | ── FUN_800328EC family producers ─────────────────────────────────────… |
...
| 0x80033080 | LIVE | `Render::impactBurstRender` | game/render/fx_sprite.cpp:412 |  | the WEAPON-IMPACT burst (kanban #15), a COMPOSITE render fn: it is not… |
...
| 0x8007E1B8 | LIVE | `Render::emitMenuFt4` | game/render/render_walk.cpp:627 |  | emitMenuFt4 / emitMenuSprites — the MENU-specialized wrappers over the… |
...
| 0x800BF9B4 | LIVE | `Render::worldVoidBeat` | game/render/render_walk.cpp:736 |  | Per-frame WORLD-pass gates (render.h): one definition each, read by BO… |
...
| 0x800F2418 | LIVE | `Render::areaCacheTrustTick` | game/render/render_walk.cpp:781 |  | AREA-SCOPED CACHE trust latches (see render.h mSceneTableTrusted/mBack… |
...
| 0x801062E4 | LIVE | `Render::renderTitle` | game/render/render_walk.cpp:529 |  | #2 DEMO/TITLE front-end (stage 0x801062E4). Substate s2 (sm[0x48]==2) … |
| 0x801062E4 | LIVE | `Render::titleNative` | game/render/render_walk.cpp:712 |  | titleNative — see render.h. Read-only producer for the DEMO/title fron… |
...
| 0x801063F4 | LIVE | `Engine::frame` | game/core/engine.cpp:2993 |  | One native loop iteration of the guest body 0x801063F4: dispatch sm[0x… |
| 0x801063F4 | ORPHAN | `Engine::stageMain` | game/core/engine.cpp:3121 | 0x801063F4 | OLD guest-loop entry (prologue + coro-redirect into the guest loop |
| 0x8010641C | LIVE | `Demo::s1` | game/scene/demo.cpp:73 | 0x80106F80 | s1 0x8010641C — wait/advance: v0 = inner menu input machine 0x80106f80… |
...
| 0x8010649C | LIVE | `Render::renderStartBoot` | game/render/render_walk.cpp:521 |  | #1 START.BIN boot (0x8010649C): the loader shows a black screen (empty… |
...
| 0x80106690 | LIVE | `Render::menuChrome` | game/render/render_walk.cpp:640 |  | menuChrome — see render.h. The black backdrop + the 2 logo sprites (FU… |
...
| 0x80106824 | LIVE | `Render::menuItemsAndCursor` | game/render/render_walk.cpp:690 |  | menuItemsAndCursor — see render.h. Reproduces FUN_80106824(param1, par… |
| 0x80106824 | LIVE | `Render::s3MenuNative` | game/render/render_walk.cpp:722 |  | s3MenuNative — see render.h. The page-1 menu (sm[0x48]==3, reached by … |
...
| 0x80106B98 | LIVE | `Engine::fieldRunFaithful` | game/core/engine.cpp:1541 | 0x8001CF2C 0x800263E8 0x80045580 0x8005082C 0x80050894 0x800508A8 … | FIELD RUNNING sub-machine 0x80106b98 — native control flow + state bod… |
| 0x80106C24 | LIVE | `Render::attractItemLive` | game/render/render_attract.cpp:83 |  | attractItemLive — IS THERE AN ATTRACT WORLD THIS FRAME? (kanban #86, t… |
| 0x80106F80 | LIVE | `demo_menu_machine` | game/scene/demo.cpp:681 | 0x8001CF00 0x8008CCE0 0x8008CD40 0x8009C820 0x8009C8BC 0x80106F80 | s1's inner menu input machine (0x80106F80): an 8-way state machine on … |
| 0x801070B4 | LIVE | `Engine::fieldRunXFaithful` | game/core/engine.cpp:2719 | 0x8005082C 0x80050894 0x8006C77C | FIELD RUNNING sub-machine VARIANT 0x801070b4 (sm[0x4c]==3, the mid-tra… |
...
| 0x80107AFC | LIVE | `Engine::transitionMain` | game/core/engine.cpp:2129 |  | the MAIN door/sub-scene transition (sm[0x4c]==1..4). sm[0x4e]: |
| 0x80107D3C | LIVE | `Engine::transitionD3c` | game/core/engine.cpp:2198 |  | transition variant (sm[0x4c]==5/6). sm[0x4e]: 0 load, 1 effect |
| 0x80107E20 | LIVE | `Engine::transitionE20` | game/core/engine.cpp:2224 |  | transition variant (sm[0x4c]==7). sm[0x4e]: 0 setup+load, 1 |
| 0x80107F3C | LIVE | `Engine::transitionF3c` | game/core/engine.cpp:2256 |  | transition variant (sm[0x4c]==8), a 7-state machine. NB case 0 |
| 0x8010810C | LIVE | `Engine::submitPage810c` | game/core/engine.cpp:480 |  | page-1 dim-fade branch (task+0x6B == 1, "draw main pause menu" — |
| 0x801082B0 | LIVE | `Engine::submitPage810cFaithful` | game/core/engine.cpp:510 | 0x8007E9C8 | pc_faithful mirror of ov_game_gen_8010810C's page-1 (pause-menu dim) b… |
| 0x801082B8 | LIVE | `Engine::submitPage810cFaithful` | game/core/engine.cpp:510 | 0x8007E9C8 | pc_faithful mirror of ov_game_gen_8010810C's page-1 (pause-menu dim) b… |
| 0x801086E0 | LIVE | `Engine::stageAreaInit` | game/core/engine.cpp:152 |  | sm[0x48] == 0 — area INIT: advance to running (sm[0x48]=2), reset the |
...
| 0x8010882C | LIVE | `Engine::stageRunning` | game/core/engine.cpp:556 |  | sm[0x48]==2 RUNNING, per-frame variant: dispatch sm[0x4a] handler. han… |
| 0x8010882C | LIVE | `Engine::submode0` | game/core/engine.cpp:608 | 0x80109450 | GAME sub-mode-0 bridge 0x8010882c (sm[0x4c]/sm[0x4e] dispatch) — nativ… |
| 0x80108A60 | LIVE | `Engine::fieldTransition` | game/core/engine.cpp:2321 |  | sm[0x4a]==5 transition dispatcher on sm[0x4c]. 0/9 = done |
| 0x80108B0C | LIVE | `Engine::devTeleportApply` | game/core/engine.cpp:1018 |  | FIELD PER-FRAME UPDATE 0x80108b0c — native control flow (the field fra… |
...
| 0x80109450 | LIVE | `Render::renderSopNarration` | game/render/render_walk.cpp:588 |  | #5 SOP INTRO NARRATION (overlay-sig 0x3C021F80 @ 0x80109450): the WORL… |
...
| 0x8010C7F4 | LIVE | `Render::fxParticleFieldRender` | game/render/fx_sprite.cpp:992 |  |  |
...
| 0x80110C14 | LIVE | `Render::fxRingSpriteRender` | game/render/fx_sprite.cpp:866 |  |  |
...
| 0x80113768 | LIVE | `Render::fxCuedSpriteRender` | game/render/fx_sprite.cpp:789 |  | (A0A overlay, area 10) — surfaced by the 22-AREA nofx sweep, which no … |
...
| 0x8012D9E8 | LIVE | `Render::fxRotSpriteTailRender` | game/render/fx_sprite.cpp:753 |  | 's SPRITE TAIL. This controller is two emitters in one function: a lar… |
| 0x8012DA04 | LIVE | `beh_typed_anim_spawn` | game/ai/beh_typed_anim_spawn.cpp:45 |  |  |
| 0x8012E868 | LIVE | `Render::fxAltAnimSpriteRender` | game/render/fx_sprite.cpp:700 |  | (A01 overlay) — the animation-script member of the family. Same shape … |
...
| 0x8013D454 | LIVE | `Render::waterJetSpriteRender` | game/render/fx_sprite.cpp:724 |  | 's SPRITE branch — the water jet's other half. The mesh branch (non-ze… |
