...
**2026-07-22:** CORRECTION (USER 2026-07-22): the spiky thing is NOT an enemy or a world prop - it is the HUD WEAPON CAROUSEL. So this is a missing HUD element, not missing world geometry, and the first suspect changes: it is a 2D HUD-layer producer, so obj_depth/billboard promotion is the WRONG lead. Check instead why its prims never reach RQ_HUD: the field's 2D-only OT walk keeps genuine 2D-overlay polys (s_ot_2d_drawn path in runtime/psx/gpu_native.cpp) but a HUD producer whose span is misclassified as world - or dropped by native-cover - would vanish exactly like this. Its screen position is fixed, so it is trivially re-checkable from any camera. Title updated.
