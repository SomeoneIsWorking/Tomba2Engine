...
USER 2026-07-16: jumping over an item picks it up; normally you have to touch the item. Suspect the pickup trigger's collision volume/overlap test (beh_pickup_collect_trigger family) — vertical extent too tall or Y ignored in the overlap test. May be a faithful-port divergence or genuine stock behavior misremembered — verify against the retired comparison path oracle first. Investigate when the pickup/collision RE is on the frontier.
