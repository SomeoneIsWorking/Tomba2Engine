...
NUMBERS (tools/warpsweep.sh, real PSXPORT_RENDER_PSX=1 reference at same frame; base.report vs fix2.report):
  ...
  area 21  61375 -> 61375  REMAINS HERE, but the original COMPOSITE diagnosis was false for the reached state. Generated control flow plus the later live capture show variant 1 / phase 1 calls helper 0x8010BB64 (four gouraud POLY_G sky bands) and then RETURNS before the tilemap loop. The tilemap-only experiment still measured worse (61375 -> 64650), because it forced a branch the captured state did not execute. Card #49 now owns the reached early gradient; a later variant/phase that actually reaches the tilemap remains separate reachability work.
...
**2026-07-23:** 2026-07-23 SPLIT + CLOSED for the tilemap family. Areas 10 and 11 are FIXED here. The two areas that are NOT this cause moved to their own cards: #47 (area 14 — GTE scene geometry, no backdrop drawer at all) and #48 (area 21 — initially diagnosed as a composite, but now corrected below; routing it through the plain tilemap producer measured WORSE, 61375 -> 64650, so it was explicitly excluded). This card covers the shared tilemap-family backdrop producer only, which is done and gated.

**2026-07-23:** 2026-07-23 OPERATOR VERIFIED + INTEGRATED (commit 261be0c). Confirmed on my own build: area 10's cave backdrop and area 11's night sky + moon both RENDER (were black) — scratch/screenshots/bd/a10.png, a11.png; seaside sky/clouds unchanged and dialog box correct; codemap --dup-installs = 0; build clean. The agent's SBS gate re-ran on the final binary: 560 A/B-identical checkpoints, 0 diverge, through f16770. NOTE the agent's split cards were RENUMBERED on integration to avoid colliding with the House-on-the-Point card (#47, filed from the user's live session): area 14 waterfall is now #48, and Area 21 moved to #49 (whose reached-branch diagnosis was corrected on 2026-08-24).

**2026-08-24:** Corrected the stale Area 21 branch description above. The reached branch is the four-band
gradient and returns before the tilemap loop; #49 contains the guest-instruction and live-state evidence.
