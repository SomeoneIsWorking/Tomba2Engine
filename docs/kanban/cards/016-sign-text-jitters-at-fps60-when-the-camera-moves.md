...
Root cause, established statically in #64: textLabelEmit draws the node in two halves and only the GLYPH half produces a display-pass record (a Render::WqRec per character, corners + the cmd+0x18 matrix factored against the scene camera, emitted through the float camera path). The MESH half — func_8003F174 = Render::subPartWalk, the sub-part transforms + geomblk submits that draw the signboard/planks — captures nothing and only emits guest packets. So letters lerp at 60fps and the board steps at 30Hz. That is this card's jitter and #64's per-letter offsets, from one cause.
