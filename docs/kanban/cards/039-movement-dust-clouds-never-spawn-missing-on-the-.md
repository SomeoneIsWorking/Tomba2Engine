...
RE TARGET: the movement state machine that requests the effect — ActorTomba and the walk/run state transitions (game/player/, and Engine::walkStart 0x80054D14 is already owned). Find the spawn call, then find its gate. Tools: 'debug otattr' attributes OT packets to emitting function AND caller; tools/find_refs.py <dump> <addr> --rw r finds who reads a state word; the Ghidra evidence workflow for Ghidra. Also worth checking whether the effect family is the 8-byte-sprite-record family already tapped in game/render/fx_sprite.cpp (FUN_80027A4C and callers 27CB4/27E5C/281EC) — issue8_9_re.md line 3149 of findings/render.md associates the 27A4C caller family with grab/dust semi-quads, so the DRAW side may already be owned while the SPAWN side is not.
...
**2026-07-23:** 2026-07-23 PORTED (not tapped) and integrated as commit a4acc07. Dispatch point found from guest data: FUN_80031558 stamps the dust VISUAL node with node+0x0B=0x20 and node+0x18=FUN_80029F6C, so it is a TYPE-0x20 custom-render-fn node — the SAME dispatch as the torch flame — and fieldObjectsRender was skipping it purely because no producer was registered for that fn. game/render/fx_dust.cpp draws both layers from the node's own state with float projection (projComposeCamera / projComposeObjectHost): the trail (FUN_80029664, ring slots at node+0x38 turned into gouraud quads with a screen-space perpendicular) and the puff mesh (FUN_80027768). NO gen_func and NO gte_op in the picture path. CORRECTNESS GATE (vertex-level, stronger than eyeballing): at seesaw-weight f226 the guest submits six 0x3A quads at first-vertices (190,66)(194,74)(198,77)(206,60)(210,68)(210,75); the native producer gives (191,66)(195,75)(198,77)(207,60)(211,69)(210,77) — every vertex within 2 px, residual is float-vs-fixed. LERP GATE (the payoff of porting): a new EffectLerp tier keyed by node, blended at Fps60::mT — measured real 214 -> interp 210 -> real 207, then 207 -> 193 -> 182, then 182 -> 173 -> 163; every in-between sits between its two real neighbours. OPERATOR-VERIFIED on integration: item menu 0/76800 vs psx_render, dialog opaque+legible at bucket-softlock f1200, build clean, dup-installs 0. RESIDUAL, honestly scoped: the PUFF MESH layer is ported-unverified — across 4000 frames of seesaw-weight the dust node's ring state never leaves {0,1}, so the guest itself only emits trail quads there; the mesh path is implemented from the RE but has never been on screen. Needs a repro reaching ring state 2/3. CONFIRMED ACTION LIST (what the user asked for): state 6 = starting to walk (kind 1) + mid-walk transition (kind 2); states 5/50 = run->walk / skid; state 7 = turning while running. Stopping is a material refresh only, no spawn. No dust on surface materials > 9. Style = subtype material+1.
