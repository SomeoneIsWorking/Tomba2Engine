...
INVENTORY (2026-08-16), 19 files:

Tomba2Engine/tools/ — 6
  ab_leg.sh  ab_replay.sh  beh_ab.sh  pan_gate.sh  precommit_gate.sh  warpsweep.sh
...
ORDER — by how often the thing is run and how much a silent failure costs:
  1. tools/precommit_gate.sh — the one everybody runs, and the one where a silently-skipped check is
     most expensive (it already guards one check on '[ -f tools/check_cpp_style.py ]', so shipping the
     gate without the script SKIPS the check rather than failing).
  2. psxport/scripts/sync-submodules.sh — its denominator discipline is good and must survive the port.
  3. tools/pan_gate.sh, tools/ab_*.sh, tools/beh_ab.sh, tools/warpsweep.sh — A/B and sweep drivers.
  4. the build_*.sh / clean.sh / scratch_reset.sh helpers.
  5. decomp.sh — the Ghidra headless wrapper; careful, it is load-bearing for all RE work.
