...
**Status:** 35 verified · 13 ported-unverified · 2 hack · 3 todo · 3 blocked
...
- **notes:** PORTED + SBS-gated 2026-07-29; READABILITY PASS COMPLETE 2026-07-30. Both bodies in this file (0x80023D48 cylinderResolve, 0x8002423C landOnObjectTop) now read as named control flow over named values: L_8002xxxx gotos replaced by real if/else + early returns, register chains lifted into named locals or GuestReg<N> proxies, the frame is a GuestFrame<80,10>/GuestFrame<32,3> from the abi_extract --scaffold --guestabi contract, and the eight call sites are typed guest call (c, kRa..., the cited guest address). The three per-record field tables (actor r17 / other r22 / anchor r30) stay SEPARATE - actor+0x30 is a 16.16 Y, anchor+0x30 is the anchor's Y. Re-gated 2026-07-30: port_check PASS on both methods; recorded 1500-frame SBS gate re-run gives 50/50 A/B-identical, ovhit 0x80023D48 7397/7397 and 0x8002423C 1949/1949. The stack hazard stays HANDLED: callees go through typed guest-call entry boundaries, never the Trig methods (callee audit in docs/re/collision-resolve-23d48.md proves rsin's own-frame ra word is the only guest-stack byte at stake). Decision structure, sign convention and the +0x80/+0x82 two-radii observation are documented in the same file.
...
- **deps:** field-world (sceneNative)
- **owner:** -
- **notes:** Track B LANDED: font->queue + panel taps + dialogTextNative + gauge text-row tap (FUN_8004EB94, parity=partial — needs gauge-popping drive). FUN_80078988's icon/SJIS glyph emitter is now fully native: one token walk owns exact guest packet/state writes and RQ_HUD output, with a 98-case whole-RAM/scratch/register interpreter differential at 0 mismatches (all 90 MAIN.EXE token-table entries plus both combining-mark variants). Its pre-existing host picture was already exercised by the five options-page 0/76800 pixel gates; this milestone removes substrate execution/duplicate decoding rather than restoring missing pixels. Track A LANDED: tile-grid layer owned (TileGridLayer, parity=verified f20820); backdropRender owns its picture. Remaining: gauge firing drive + USER eyeball of the whole 2D layer.
...
- **notes:** SW (non-GTE) 3-Euler RotMatrix; owned via tomba::native::declareOverride(0x800847F0). SBS-full 0-diff f0..f360 (billboard C5F8 fed it 8x, MAT_ROTZ byte-identical).
...
- **notes:** Was the busiest remaining typed runtime address dispatch target in the game: 127,275 hits per 6000 frames of replays/bugs/seesaw-weight.pad, 4x the runner-up. Frame contract from binary ABI evidence --contract (32-byte frame, s1/ra/s0 at +20/+24/+16, ra constants 0x8014649C/0x801464AC) rather than hand-derived. SBS 0-diff f1500 with ovhit proving 76378 executions on EACH leg. Leaf calls deliberately routed through the generated A00 overlay guest entries wrappers so the registry hit counters stay truthful — a direct call works but makes both leaves report NEVER HIT.
...
- **notes:** The cutscene interpreter's last two unowned links. Equivalence proven before use: full 2MB guest RAM at f2600 on replays/bugs/sequence-softlock-2.pad byte-identical to the pre-port run; SBS full green to f41280. loadNextEntry MUST install with a setter (guest 0x80040FA0 reaches it by a direct jal; typed runtime address dispatch never sees it). Owning these is what exposed kanban #60.
...
## render-panel-fill
- **status:** ported-unverified
- **notes:** Panel::fillQuad (FUN_8004FFB4): the 9-slice panel fill quad, hottest unowned render fn on the field path. binary evidence byte-faithful, port_check PASS, wired with setter, LIVE at 505 hits with the frame unchanged. READABILITY PASS PENDING - still in register form. Note game/ui/panel.cpp:185 calls gen_ directly so the existing tap is not intercepted.
