...
- **notes:** PORTED + SBS-gated 2026-07-29 (50/50 identical, ovhit 7397/7397, port_check PASS). READABILITY PASS IS HALF DONE: every field access is now NAMED via three per-record constant tables (actor/other/anchor keyed by base register r17/r22/r30 — they must stay separate, actor+0x30 is a 16.16 Y while anchor+0x30 is the anchor's Y). STILL OUTSTANDING: the register dataflow is untouched, so the body is still c->r[] arithmetic with L_8002xxxx labels. Remaining work = lift the register chains into named locals and the labels into named control flow, re-run port_check (PASSes today, so it is the equivalence gate for the rename). Field map + outcome codes in docs/re/collision-resolve-23d48.md. The stack hazard is HANDLED and must stay handled: callees go through generated func_XXXX wrappers, never the Trig methods.
