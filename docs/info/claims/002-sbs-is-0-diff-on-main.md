...
Operator-verified on the applied subpart_walk fix: SBS-full seesaw leg, PSXPORT_SBS_NOPAUSE=1, ZERO sbs-div lines, 'A/B identical' through f19140 (run ended on my 400s timeout, not a divergence). Was first-diverging at f169/0x801FE808. Cause: Render::subPartWalk held its walk state in C++ locals while its still-substrate callee guest 0x800803DC spills the guest s0/s1 into its own frame — so core A spilled stale registers. Fixed by mirroring s0..s4 (the LIVE-REGISTER LAW).
