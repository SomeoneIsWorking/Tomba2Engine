...
MEASURED 2026-08-06 in an isolated sibling clone of this repo (a `scratch-shockwave/T2` tree at the workspace root, SINCE REMOVED as orphaned debris — so this evidence is NOT re-runnable in place; re-measuring means making a fresh isolated clone), three separately-built binaries with distinct md5s, each build exit-0 checked, never the shared checkout: .off = the producer call site replaced by a shockwave-LEGOFF log line, .old = the shipped producer, .new = the fixed producer. In-band leg proof: off logs 152 LEGOFF marks and 0 producer calls, old and new log 152 producer calls and 0 marks, so all three reached the same scene.
