...
- **refs:** `external/psxport/runtime/psx/native_fmv.cpp`, `external/psxport/tests/test_fmv_watchdog.cpp`

## Native port of libcd directory cache — CdNewMedia + CdCacheFile (2026-07-04)
- **symptom:** SBS `MODE=gameplay` (pc_skip=true vs guest instruction path) f0 divergence 1273 B — 8 B at `0x800AC2D4..0x800AC2DC` (libcd cached-dir-idx + media cookie), ~500 B at `0x80102D68+` (path-table cache), ~500 B at `0x80102768+` (file-entry cache for last-cached dir), plus 2 KB of dir-sector scratch at `0x80104368+`.
