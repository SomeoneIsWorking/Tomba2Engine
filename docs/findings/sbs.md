...
- **fix:** added the three `GuestFrame<Size, Count>` declarations + spill tables, generated by
  `tools/abi_extract.py <addr> --scaffold --guestabi` (never hand-derived). `port_check` confirms the
...
`rec_dispatch` inside itself) is invisible to `port_check`: no `c->r[31]=…` precedes it at the call
...
helper's own body for `rec_dispatch`/`func_` before counting it — not pattern-match its signature.
...
- **fixed (file :: method — oracle frame/spills):**
  ...
  All spill tables generated by `abi_extract.py <addr> --scaffold --guestabi`, never hand-derived.
