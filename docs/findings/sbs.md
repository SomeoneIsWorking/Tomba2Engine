...
retired comparison path (substrate). Both cores get `pc_skip=false` (faithful branch of every fork).
...
- **Now:** ONE definition, derived from `GameConfig` — `external/psxport/runtime/psx/render_noise.h`
...
- **For the other ports:** spyro and spider1 leave the `packetPool*`/`otRegion*` fields 0, so their mask
  ...
  never a false clean.
- **refs:** psxport `10c37cf5`, `runtime/psx/render_noise.h`, spyro `docs/issues/0056`.
...
- **The class (3rd instance; treat as LAW when porting):** a native port that calls a still-substrate
  fn (typed runtime address dispatch/guest_fn) must reproduce EVERY register gen holds live at that jal — not just the
...
- **How to derive the live set:** read the guest-visible behavior at each jal — every register assigned since the
  last call that isn't dead by the jal is live; delay-slot literals count. `binary ABI evidence --contract`
...
- **refs:** game/input/pad_edge_fence.cpp (the annotated fix), runtime/psx/mem.cpp ov_guestMemset,
  commit 7db286a8.
...
- **Root cause**: `Engine::walkStart` (FUN_80054D14, game/core/engine.cpp) placed its
  ...
  compare: `guest 0x80054D14` descends sp and spills r16@+16/r17@+20/r18@+24/ra@+28 BEFORE the
  ...
  the voiceMixTick finding below: a native body that doesn't mirror the frame the guest-visible behavior pushes.
...
- **refs**: game/core/engine.cpp `Engine::walkStart`; oracle authenticated executable/overlay evidence
  `guest 0x80054D14` (L7290); scratch/logs/sbs_cut_prewatch.log (write-site capture).
...
- **RESOLVED (2026-07-14): the diverging 9 bytes are `FUN_80075824` (voiceMixTick)'s guest-frame
  ...
  music_coord.cpp). guest 0x80075824 descends 32 B from sp=0x801FE928 → 0x801FE908 and spills
  ...
  store — now `typed guest call (kFnSetFadeTarget, 0x800759B4, 0x47FF)`) + g2 smoother signed/unsigned read
...
- **RESOLVING (2026-07-11, MIRROR_VERIFY=all run headless):** the per-mirror register-output bugs
  ...
  - **Still open (MIRROR_VERIFY):** 0x80080F6C `Render::drawSync` (v0/v1 stale — dispatch target
    returns differ; subtler — the native relies on typed runtime address dispatch(tableSlot60) to set r2/r3 but the
...
- **PROGRESS (2026-07-11, same session, continued):** Fixed 7 more mirrors via MIRROR_VERIFY-driven
  ...
  - **Spawn::spawnTypedChild family** (spawn.cpp): rewrote all 4 trampolines to reproduce the
    substrate's EXACT dispatch path (typed runtime address dispatch(0x8007A980) with matching r4/r5/r6, r16/r17 swap,
...
- **Forcing caveat:** FORCES4C bypasses the NATURAL transition, which may itself set guest state (e.g.
  ...
  overlay code-residency gap: cross-area `warp` historical guest-entry misses at the destination overlay's object-init
...
- **MEASURED (2026-07-11, headless SBS-full, AUTONAV=1, the gate this finding was built to run):**
  ...
  3. **Diverges at f400 in r22/r23/r30** — Range 0x801FE91A..0x801FE923 = the spawn leaf's
     own-frame spills of r23 (@leaf+52) and r30 (@leaf+56). `guest 0x80075A80`'s callee-saved
...
- **Root cause:** SBS steps each core with `the retired in-process comparison mode=1`. `game/game_tomba2.cpp` per-frame tail
  RETURNS EARLY under the retired in-process comparison mode (before `gpu_present`). `GpuState::gpu_present_ex` is the ONLY
...
- **Lesson:** SBS suppresses per-core PRESENT (`the retired in-process comparison mode`) but the RENDER FINALIZE bookkeeping
...
- **Refs:** commit "sbs: run per-frame render finalize per core"; `runtime/psx/gpu_native.cpp`
  (`GpuState::frame_finalize`), `runtime/psx/sbs.cpp` (`grabPane`), `game/game_tomba2.cpp:94`.
...
- **The gate**: `PSXPORT_MIRROR_VERIFY=all|0xADDR[,..]` + `strict replay check(c, addr, xFaithful())` at
...
- **refs**: runtime/psx/sbs.cpp (recordDivergence, sbs_restore_sched, dbgCmd), skill
...
Sop::fieldModeFaithful (mirror of overlay guest 0x80109450) + Sop::areaLoadFaithful (mirror of
overlay guest 0x80109164, task-1 fiber body) landed; the ENTIRE SOP intro cutscene holds strict
...
conversion** — the walkable field: mirror gen overlay guest 0x801088D8 (7 states on sm[0x4c], its
...
guest-visible behavior first — 0x80109450 is the loaded MODE overlay's field-mode fn), with the established
...
- **Ported scheduler primitives** as PcScheduler methods with full guest-stack discipline (frame
  ...
  consults the table. Byte shapes from the generated guest instruction paths (original guest instructions in shard_2/3/5/6.c).
- **STAGE-0 native fiber body**: `Engine::startBinStageFaithful` is the complete overlay guest 0x8010649C
...
- **Task-1 FUN_80044F58 body**: `Asset::loadTexgroup` frame discipline + `unpackGroupFaithful`
  ...
  guest-frame offset table (guest 0x80044D8C sp+0..28).
...
**Proposed resolution (pending user decision):** make the existing "memory the still-guest side
...
- **refs**: `scratch/decomp/libgs_ctx.c` (Ghidra decomp of FUN_80097194 case 2 = the writer), `game/scene/engine_stage.cpp:startBinStageFaithful` (A's dispatch site), auto-diagnosis registry for `libgs.gfx_ctx` in `runtime/psx/sbs.cpp` (48ceae1).
...
- **refs**: `game/scene/engine_stage.cpp:sync_preload_spawn` (current stub port — task-slot writes only), `runtime/psx/scheduler.cpp:native_task_spawn` (base primitive port), scratch/decomp/task_spawn.c (Ghidra decomp of FUN_80044BD4 + FUN_80051F14 + FUN_80080860 syscall trampolines), `runtime/psx/sbs.cpp` (auto-diagnosis registry).
...
- **symptom**: Post-Slip-#4, SBS gameplay mode surfaces a byte-range divergence at `0x800EE6D4..0x800EE9D7` at f217. Auto-diagnosis CLASS = ASYMMETRIC at f248, but PREWATCH from boot pins the ORIGIN write to f192 with **CLASS = VALUE-MISMATCH** — both cores wrote to 0x800EE6D4 (u16 coord) via the SAME call path (`array8Dispatch → overlay guest 0x801158E0 → guest 0x8003116C`) with different values: A=0x2739 vs B=0x279A (delta ~100). Also 0x800EE6D6: A=0xF7BA vs B=0xF761. These are new-object position coords, copied from a stack-local `local_1e = (rnd & 0x7F) + 0x2731; local_1a = -0x831 - (rnd & 0x7F); local_16 = 0x17C3 - (rnd & 0x7F)` in FUN_801158E0 state 1 (RNG-driven decoration spawn).
- **root cause**: `FUN_8009A450` (linear-congruential RNG, state at `0x80105EE8`) is called **158× more on B than A**. Per-frame DISPWATCH count in the gameplay window: A ≈ 1 call every 14 frames, B = 7-8 calls per frame. `grep 'typed runtime address dispatch.*8009A450\|0x8009A450' game/ runtime/psx/` returns ZERO — no native code invokes the guest RNG. Every native handler that REPLACED a guest instruction path'd handler that would have called `FUN_8009A450` (physics jitter, decoration spawn, animation phase, sfx pitch, target-picker) skips the RNG advance. RNG state drifts almost immediately after gameplay starts; every downstream consumer sees uncorrelated values on A vs B. Same family as the known "Target #4" `FUN_80083F50` sin-arg divergence in this same file — that's a cadence-timing symptom of the same class (native writes to the aliased `rec+0x0C` fire at different ticks than guest instruction path).
- **f192 call chain, both cores** (via host-BT + Ghidra decomp):
  1. `Engine::fieldFrame` → `Array8Dispatch::tick` (native on A) / `guest 0x80026368` (guest instruction path on B).
  2. Dispatches obj-slot handler = `FUN_801158E0` (A00 overlay, dispatched via typed runtime address dispatch on both).
...
- **why this is the same class as Target #4**: both are cadence-drift symptoms downstream of native handlers not making the same guest-state advances as their guest instruction path equivalents. Target #4 = write-timing on `rec+0x0C` (aliased with `obj+0x42`). Slip #5 = RNG state advance count. Fixing individual native handlers to call the RNG (or write the aliased byte) at the same points as guest instruction path would close both families.
- **fix classes (documented, not landed)**:
  - **Full native port of FUN_801337E4** (the `beh_background_actor_tick` candidate — extensive RE already captured in this file above under "FUN_801337E4 RE'd semantic model"). Native port has to preserve the aliased `rec+0x0C = obj+0x42` writes at the same substate transitions as guest instruction path — that closes Target #4 directly. guest instruction path/native become tick-identical for that handler.
  - **RNG-cadence gating on !SBS for RNG-consuming native handlers**: audit `beh_*` native handlers for guest paths that would have called `FUN_8009A450` (grep the RE'd body for `func_0x8009a450` calls). Under SBS, invoke `typed runtime address dispatch(c, 0x8009A450u)` the same number of times the guest instruction path did. Faithful-mode-only — live PC gameplay stays skip-free. Same shape as Slip #3's `c->game->sbs` gate. This is the broad-coverage fix; the exact list of handlers to gate is large but bounded.
...
- **tooling landed this session that made this diagnosable in one go**:
  - Host C-stack backtrace at wwatch fire (runtime/psx/sbs.cpp) — turned "stale c->pc = 0x80071768" (misleading) into `Cull::coneCull2b278` (correct) in one shot for Slip #4; same mechanic named `overlay guest 0x801158E0 → guest 0x8003116C` for Slip #5.
...
- **workflow lesson**: the "faithful vs PC mode" split is a general pattern the port needs when the native reimplementation collapses a guest instruction path yield. Faithful mode = cadence-parity for SBS. PC mode = the runtime path. Gating on `c->game->sbs` is the mechanism.
...
## SBS crashes on runtime dispatch fault(0x80109F7C) during DEMO attract cursor=1 (A01)
- **symptom (all SBS modes render/gameplay/full):** `[historical guest-entry miss 0] no guest fn for 0x80109F7C  (caller ra=0x800587F8, a0=0x800E7E80)` inside `guest 0x80058648` at frame ~1814 (or wherever attract sm[0x6e] reaches 1). Diagnostic: `[miss-state] stage=0x801062E4 sm=0x801FE000 sm[48]=7 [4a]=1 [4c]=0 [4e]=0 [50]=0 [52]=0 1f80019b=1 areaidx(800bf870)=1`. Blocks divergence surfacing — SBS aborts before the harness can compare RAM.
- **status:** RE'd — root cause identified; fix not landed (needs game-content RE to know the correct handler for A01).
- **cause:** attract cursor table `@0x8010770C = {0, 1, 3}` cycles through 3 attract areas. Cursor 0 → A00 (works). Cursor 1 → A01 (**crashes**). Cursor 2 → A03. `guest 0x80058648` is an object-init routine that dispatches on `DAT_800BF870` (area byte) via a jump table at `0x801445B8` (base 0x80144000 + addiu 17848). That address is BEYOND A01's overlay end (0x8013A678, size 0x316DC). A00/A04/A06/A08 all extend past 0x80144000 (A00 to 0x8014E944 etc.) and DO cover the JT — but A01/A02/A03/A05/A07 and every A0[A-L] overlay are shorter, ending before the JT. So the JT is a **conditional-existence** table valid only when a "big" area overlay is resident.
- **real-PSX behavior (inferred):** the JT works on PSX because a previous big overlay (like A00 loaded in attract cursor 0) leaves its JT bytes in RAM past the freshly-loaded smaller overlay's end. RAM at `0x801445BC` still contains A00's `jt[1]` value even after A01 overwrites `0x80108F9C..0x8013A678`. The PC-guest instruction path fails-fast because when A01 is resident, the overlay-router doesn't know about a function at 0x80109F7C (A01 has no entry there; A0B / A0I do, and A00 has bytes there but not marked as fn).
- **fix path options (in priority order):**
  1. **DEEP RE (right):** disas guest 0x80058648 fully to determine whether the JT lookup is REALLY reachable with area==1, or if the code has an earlier guard that on real PSX diverts. If reachable-and-legit, the JT entry 0x80109F7C in A00's overlay is a REAL function entry — mark A00's function map to include `guest 0x80109F7C` and let guest instruction path emit it. If unreachable-should-be, find the missing guard (state check in a caller of 0x80058648).
...
- **refs:** #27 investigation trail (led here via fadetrace showing native paths work in boot); `runtime/psx/overlay_router.cpp` (route + slot logic); `game/scene/engine_demo.cpp:demo_frame_s7`; `authenticated executable/overlay evidence:guest 0x80058648`.
...
- **section B (0x80133838..0x80133864) — 5-way sub-state dispatch:** `if subState >= 5: return; else jump *(0x80109E58 + subState*4)`. Jump-table entries:
  ...
  - **[1] MAIN OSCILLATOR TICK (0x80133874):** the target-#4 hot path.
      ...
      - `else if subFlagX != 0:` — turn-trigger path:
          ...
          - `subState = 4;`
          - `typed runtime address dispatch(0x80077768)` — sub-behavior returning nonzero/zero to say TURN LEFT / TURN RIGHT
      ...
      - `else (counterA == 0 && subFlagX == 0):`
          ...
          - `else:` **THE OSCILLATOR TICK**:
              - `cos = Trig::rcos(oscPhase)`  (`typed runtime address dispatch 0x80083F50` — now `c->trig.rcos()`)
              - `renderRec[+0xC] = cos >> 5`
              - `r = c->rng.next()`  (`typed runtime address dispatch 0x8009A450` — now `class Rng` with seed at 0x80105EE8)
...
- **port readiness:** the RE'd surface is captured. Full native port of FUN_801337E4 as a method (candidate: `Actor::runBackgroundActorTick()` or a free helper `background_actor_tick(Actor&)` in a `game/ai/background_actor_tick.cpp`) is the NEXT arc — it will replace the parent handler's `typed runtime address dispatch(SUB_STATE1_TICK)` and, if the pilot-actor region 0x800E7E80 semantics get named at the same time (see below), turn the whole chain into named-field code.
- **still-un-RE'd sub-references** (all called via typed runtime address dispatch by FUN_801337E4 for now):
...
- **refs:** game/object/actor.h field docstrings; game/ai/beh_typed_table_seed_gate.cpp STILL OPAQUE list; overlay guest 0x801337E4 substrate body (authenticated executable/overlay evidence line 19260-19313).

## `PSXPORT_DISPWATCH=<addr>:ra=<caller_ra>` — caller-ra-scoped dispatch trace (2026-07-03, workflow-first tool extension)
- **finding:** Extended `PSXPORT_DISPWATCH` in runtime/psx/overlay_router.cpp to accept a `:ra=<caller_ra>` suffix that filters by the caller's `c->r[31]` at dispatch entry. Motivation: FUN_80083F50 has ~30+ typed runtime address dispatch call sites (grep `authenticated executable/overlay evidence` shows ~30 in that shard alone), so a plain `PSXPORT_DISPWATCH=0x80083F50` interleaves args from every caller and drowns the specific site we're chasing. The caller-ra filter isolates ONE call site so its per-frame per-core args are directly diffable.
...
- **applied to target #4:** `PSXPORT_DISPWATCH=0x80083F50:ra=0x80133900` (the exact site in `overlay guest 0x801337E4:19303-19304` where the state-1 helper dispatches cos of obj+0x42). At f118 both cores fire ~7 calls at that ra with `a0=0` (obj+0x42 reads 0 on both sides after the reset at line 19260) — SAME ARG on both cores. Since `guest 0x80083F50` is a pure cos LUT (`guest 0x80083F50` in the main-text substrate; no override registered in the overlay slot 1831), same arg → same return. Yet the eventual `mem_w16(rec+12, cos>>5)` writes 0x56 on A vs 0x57 on B (per the earlier UPPROBE log).
...
- **next-session probes (durable extensions that would end the chase):**
  ...
  - **Guest+native backtrace at the dispatch:** already exists in BYTETRACE for writes (via `capBt`), but not for dispatches. Extend `capBt` to work on the dispatch entry too so the FULL chain (native → typed runtime address dispatch → gen fn) is visible per call.
...
- **refs:** runtime/psx/overlay_router.cpp:157 (extended DISPWATCH block), $CLAUDE_JOB_DIR/tmp/dw_scoped.log (session capture: 7 hits/frame/core at ra=0x80133900 with a0=0 on both), the target-#4 finding block below (the concrete divergence the extension is meant to solve, still open).
...
- **finding:** After the wide-BYTETRACE sweep named 5 stride-0xC4 ONE-SIDED bytes at 0x800F00xx as a target-#4 candidate, applied the target-#1 primitive: cross-check native `game/ai/beh_typed_table_seed_gate.cpp` (0x80133C14) against guest instruction path `overlay guest 0x80133C14` (authenticated executable/overlay evidence). **No transcription defect found** — native state 0 (lines 65-83) and state 1 (lines 87-108) match the guest instruction path writes byte-for-byte:
  - state 0: same order, same constants (`obj+0x2a=0x22`, `obj+0x80=0x1e`, `obj+0x82=0x3c`, `obj+0x84=0x32`, `obj+4=1`, `obj+0=1`, `obj+0x29=0`, `obj+0x46=0`, `obj+0x86=0x64`, then `typed runtime address dispatch(0x8004766Cu)`, then `obj+0x42=0`, `obj+0x60=-0xc8`, `obj+0x32=tbl[n3*2]`).
  - state 1: same branches (`g<0x1c` → stash `obj+0x62`; `g==0x22` → `obj+1=st`, `typed runtime address dispatch(0x80077EBC)`, render tail; else → `typed runtime address dispatch(0x800778E4)`, if v0==0 stash `obj+0x62`, else render tail).
...
- **where the divergence actually lives:** the writes at rec+0x0C (which decodes to the 0x800F0036 / 0xFA / 0x1BE / 0x282 / 0x346 absolute addresses — records are stride-0xC4, laid out from 0x800F002A) come from line 19308 in `overlay guest 0x801337E4` (the state-1 entry helper, authenticated executable/overlay evidence):
  ```
  c->r[4] = (int16_t)mem_r16(obj+0x42);       // arg to FUN_80083F50
  typed runtime address dispatch(c, 0x80083F50u);                // returns a value in r[2]
  c->r[3] = mem_r32(obj + 0xC0);               // rec_ptr (render-cmd record)
  c->r[2] = c->r[2] >> 5;
  c->mem_w16(c->r[3] + 12, (uint16_t)c->r[2]);  // ← THE DIVERGENT WRITE (rec + 0x0C)
  ```
  Both A and B execute this same guest instruction path helper via the same dispatch path (native calls `typed runtime address dispatch(0x801337E4)`, guest instruction path calls `overlay guest 0x801337E4(c)` — both resolve to the same body). If both cores' obj[+0x42] and obj[+0x46] were equal at the moment of dispatch, they'd write the same value. **The 5 divergent live values prove obj[+0x42] (or the obj[+0x46] gate that decides whether FUN_80083F50 runs — line 19298-19300) differs upstream between the two cores.**
- **not a case-swap in this handler.** obj[+0x42] is initialized to 0 in state 0 by BOTH native and guest instruction path (matches). Something ELSE writes to obj[+0x42] over the object's lifetime and puts native and guest instruction path out of sync for these 5 objects. That "something else" could be another native handler with a transcription defect OR another target-#2-style architectural drift (native-render walk touching the field). Not narrowable without a wwatch on obj[+0x42] for one of the 5 divergent objects.
...
- **refs:** game/ai/beh_typed_table_seed_gate.cpp (native, verified byte-for-byte), authenticated executable/overlay evidence (guest instruction path gen_80133C14 body), authenticated executable/overlay evidence (guest instruction path gen_801337E4 helper — the actual divergent-write site at line 19308), game/world/graphics_bind.cpp:51-70 (obj_record_init native writing rec+0..0xc, +0x38/3a/3c, +0x40 — sets up records but doesn't touch rec+0x0C).
...
- **actionable cluster 2 — 5 stride-0xC4 ONE-SIDED bytes at offset 0x36 of records in 0x800F00xx (0x36 / 0xFA / 0x1BE / 0x282 / 0x346) is a NEW candidate defect:**
  ...
  - **RA signature is clean:**
    ...
    ```
    ra=0x80133910 is inside `overlay guest 0x80133774` state-1 entry helper (called from beh_typed_table_seed_gate 0x80133C14); ra=0x80133CEC is inside `overlay guest 0x80133C14` right after `overlay guest 0x801337E4(c)` (line 20577 authenticated executable/overlay evidence). A's DEAD0000×97 replaces those writes with native-inline (no r[31] update).
  - **native handler already exists:** `game/ai/beh_typed_table_seed_gate.cpp` (0x80133C14, registered in behavior_dispatch.cpp:85). Native calls `typed runtime address dispatch(0x801337E4)` for the state-1 entry helper. So both A and B execute the same guest instruction path helper via the same dispatch — the divergence has to be either (a) in what a1..a3 registers hold at the dispatch, or (b) in whether native takes the same state-1 branch (g<0x1c / g==0x22 / else) as guest instruction path.
...
- **refs:** $CLAUDE_JOB_DIR/tmp/bt_wide_out.txt (post-Slip-#1 sweep of 0x800EE000..0x800EEA00), $CLAUDE_JOB_DIR/tmp/bt_f000.txt (0x800F0000..0x800F1000 sweep with stride-0xC4 signature), game/ai/beh_typed_table_seed_gate.cpp (native handler, RA source for A's DEAD0000 writes), authenticated executable/overlay evidence (guest instruction path of FUN_80133c14 state 1, RA source for B's 0x80133CEC writes).
...
- **root cause:** Native `demo.frame()` at sm[0x48]==5 dispatches native_start_stage inline in one scheduler tick. Coro of 0x801062E4 substate 5 in the guest instruction path yields at least once through FUN_80051f80 before completing, so B's entry rewrite lands one tick LATER than A's. Same class as the closed Slip #2 (SOP fieldMode case 0 yield vs native inline).
- **fix (mirror sop_field_step pattern):** New `SchedulerState.demo_leave_step[3]`. In `runtime/psx/scheduler.cpp` DEMO native block, before calling `c->engine.demo.frame()`, read `sm[0x48]`. If it equals 5 and demo_leave_step==0, set step=1 and skip the dispatch (task saved as runnable). Next tick, actually call demo.frame() so native_start_stage runs and re-arms the step. Not applied on `demo_fresh` (prologue path) — fresh sm[0x48] is 0, never 5.
...
- **ramdiff @f218 unchanged at 1880 B** (~629 spans dominated by other pool ranges). Slip #1 owned only the specific spawn-node animation-timer bytes surfaced in BYTETRACE, not the whole ramdiff. The remaining REAL byte in the range (0x800EE104, val=0x18 A×4 B×2, A-ras=DEAD0000/native × 70 vs B-ras=8007A964/guest instruction path × 34) matches the target-#2 architectural signature (block below): same values, different write cadence between native and PSX render walks. Not a scheduling drift; not this target's shape.
...
- **refs:** runtime/psx/scheduler.cpp DEMO native block (demo_leave_step guard around `c->engine.demo.frame()`), runtime/psx/game.h SchedulerState.demo_leave_step[3], $CLAUDE_JOB_DIR/tmp/st_fixed.log (post-fix STAGETRACE), $CLAUDE_JOB_DIR/tmp/bt_after.log (post-fix BYTETRACE). Target #3 = CLOSED.
...
1. **CASE-SWAP** — a state-machine `switch/if (v == X)` in the native transcription routes value X to the wrong branch, or omits a case the guest instruction path handles. Signature: BYTETRACE ONE-SIDED byte writes on a state field (A writes some values but not others; B writes the full set). The file's own header comment tends to agree with the buggy code, not the decomp — trust the decomp. Fixed: `beh_anim_trigger_gates` (b1df8d7).
...
3. **LUI+ADDIU EFFECTIVE-ADDRESS DRIFT** — a `lui $hi, 0xHHHH` followed by `addiu $lo, $hi, IIII` was transcribed as the Ghidra-displayed hex literal (which shows the assembled pair but NOT the folded effective address). If `IIII` is negative (bit 15 set), the effective 32-bit value is `(0xHHHH0000) + sign_extend(IIII)` — the HIGH half becomes `0xHHHH − 1`, not `0xHHHH`. Off by exactly `0x10000`. Signature: a table/data pointer lands in unrelated memory; downstream side-effects look like arbitrary garbage. Fixed: `beh_prng_velocity_machine` (7f5e14c).
   - **Prevention rule:** never copy a hex literal from Ghidra's `lui/addiu` pair display. Compute `lui_hi_shifted + (int16_t)addiu_imm` from the generated guest instruction path — the recorded guest call graph contains `(uint32_t)HHHHu << 16` then `+ (uint32_t)-N`, which folds correctly. Cross-check every 0x8015XXXX / 0x800FXXXX / etc. constant in a transcribed function against the same address in `authenticated executable/overlay evidence` before landing.
   - **Static-scan candidate (future workflow-first tool):** grep native `beh_*.cpp` for `0x[0-9a-f]{4}[89ab][0-9a-f]{3}` constants and diff against the corresponding guest-visible behavior — the high nybble should match the folded value, not the raw `lui` immediate.
...
- **not a case-swap.** wwatch armed on 0x800EE18C, both cores hit the same word (write value=0x00000000). B's guest backtrace: `[sp+0x014]=0x8007A964 [sp+0x02C]=0x80108B68 [sp+0x044]=0x80106D08` — B is going through the guest instruction path of `FUN_80108B0C` (the field-frame update) which calls the guest instruction path of `FUN_80077A904` (the walker), whose per-node dispatch invokes handlers that write obj+0xC. A's stack is empty (native path, no guest stack maintained) — A goes through `Engine::fieldFrame() → objectList.walkAll()` NATIVELY, which also invokes the same handlers via `BehaviorDispatch::dispatchObj`.
- **root cause hypothesis:** In FULL mode `A native render, B PSX render` (sbs.cpp:342). On A, `Render::frame()` runs native render walks `rwalkAuxBcf4() / rwalkAuxBf00() / rwalkAuxEec0()` (engine_render_walk.cpp) which dispatch per-node RENDER handlers on top of the gameplay walk — each aux case invokes another `typed runtime address dispatch` or `perObjRender` per node. B's PSX render uses the guest instruction path which drives handlers differently (or not at all for these render-time hooks). Result: same handlers, dispatched at 2× rate on A vs B — a settled state that MATCHES (final value equal), but the transient trail differs.
...
- **decisive tool step:** Extended BYTETRACE dump to print top RAs per side. A-ras=0xDEAD0000×60 (native path w/ sentinel r[31]), B-ras=0x80117AC0×88 (guest instruction path gen_80117658 body, just after `typed runtime address dispatch(0x80077B5C)` at L_80117AB8). The RA nail-pointed the divergence to native beh_prng_velocity_machine vs guest instruction path FUN_80117658.
- **root cause:** Two `lui 0x8015; addiu $reg, -N` immediates were transcribed by copying the assembled register-load literals from Ghidra's view (0x8015C808, 0x80158574) instead of computing `0x80150000 + (int16_t)-N`. Off-by-0x10000. In native L76d8 (STATE 0, node[3]==0): `c->r[5] = 0x8015c808u` should be `0x8014C808` (guest instruction path: `lui 0x8015` = 0x80150000, `addiu -14328` = 0x8014C808 — the high half is 0x8014, not 0x8015). Same class of bug in L7c7c (STATE 2 exit): `0x80158574` should be `0x80148574` (`lui 0x8015 + addiu -31372`). Both are pointers to geometry/animation tables the game uses — pointing 0x10000 too high lands them in unrelated data, so obj_set_geom loads a garbage cnt into obj+0xe, and the anim-tick decrement (FUN_80077B5C) counts through 0 → −59 without ever triggering the anim-cmd-advance branch.
...
- **refs:** runtime/psx/sbs.cpp (mByteTrace + storeCb decomposition + `dumpByteTrace` + REPL `sbs bytetrace` + env `PSXPORT_SBS_BYTETRACE` / `_ALL` / `_STRICT`); `$CLAUDE_JOB_DIR/tmp/bt5.log` (session capture with the 4-class classifier).
...
- **finding (2026-07-03, root-cause named + fixed via per-ra bucketed ALLOCTRACE):** The +3 net-alloc residual at 0x800ED098 was caused by a **case-label swap** in `game/ai/beh_anim_trigger_gates.cpp:54` — the native STATE 0 dispatcher had `if (v1 == 2) leaf(c, obj, 0x8012982C)` but the decomp/guest instruction path ground truth dispatches FUN_8012982C on `v1 == 3`, NOT `v1 == 2`. FUN_8012982C is a 4-record-alloc init loop; with the swap, A's 2 objects with node[3]==2 wrongly went into the 4-alloc init (8 A-only allocs), while B's guest instruction path took the correct FUN_801296E0 branch for node[3]==2 (0 allocs). The scene has no node[3]==3 objects, so B never fired FUN_8012982C at all in this window. Net: A allocated 8 extra records; other symmetric-shape sites contributed a compensating −2, netting the +6 total delta.
...
- **decomp is the ground truth** (`scratch/decomp/collectables.c:1105 FUN_80129c00`, STATE 0 lines 1128-1141):
  ...
  ```
- **how the ALLOCRA tool got us here** (the workflow-first invariant paying off): after the flagbit misdiagnosis burned a session on ordinal-point-in-time comparison, the settled-state per-ra table (landed this session) made this bug visible as ONE A-only bucket (0x801298A4 = 8 allocs) in a mostly-timing-symmetric table. That pointed at the guest function containing 0x801298A4 (`overlay guest 0x8012982C`), then its callers (`beh_anim_trigger_gates` STATE 0 node[3]==2 branch), then the diff between the native and guest instruction path/decomp for that branch surfaced the case-label swap. The tool is meant to keep working: whenever a +N-alloc delta reopens, `PSXPORT_SBS_ALLOCTRACE=1` first, read ALLOCRA, and the A-only bucket names the caller.
...
- **refs:** game/ai/beh_anim_trigger_gates.cpp:5,55-60 (fix + updated header + inline citation), scratch/decomp/collectables.c:1105-1141 (FUN_80129c00 ground truth), authenticated executable/overlay evidence (guest instruction path confirming decomp), $CLAUDE_JOB_DIR/tmp/fix1.log (post-fix session capture — net=+0, ramdiff 2074).

## PRIOR ATTRIBUTION (kept for the trail) — Per-ra bucketed ALLOCTRACE landed as a durable settled-state compare — `PSXPORT_SBS_ALLOCTRACE=1` now dumps at exit; +6 net traced to native-record_gate lump + one A-only guest instruction path bucket ra=0x801298A4 (beh_anim_trigger_gates → FUN_8012982C init 4-alloc loop, 2 invocations)
- **finding (2026-07-03, workflow-first invariant landed):** Extended `PSXPORT_SBS_ALLOCTRACE=1` (runtime/psx/sbs.cpp) to bucket every 0x800ED098 store by guest `r[31]` per core. At end of run (atexit + SIGTERM/SIGINT handler so `timeout N …` also emits), prints a settled-state per-ra table sorted by `|A-B|`. New REPL command `sbs allocra` for live inspection; env `PSXPORT_SBS_ALLOCRA_ALL=1` to show symmetric rows (hidden by default so the delta rows stand out). This ENCODES the timing-shift compensation the manager called out — ordinal-point-in-time comparison lied on the prior turn (flagbit STATE 0 read as A-only when both cores fire it 2×, 36 ordinals apart); the fix is comparing per-caller COUNTS OVER THE WHOLE RUN.
- **evidence — 30 s SBS FULL AUTONAV run, total A=944 B=938 net=+6 (matches prior ALLOCTRACE cum + 0x800ED098 byte delta at f217):**
  ...
  0x801298A4        8       0      0     0         +8, +0   ← A-ONLY guest instruction path bucket ← real caller-side divergence
  0x80051BC4       14      96      0     0        -82, +0   ← symmetric-shape site: same code, different frame
  0x8013A3DC        0      26      0     0        -26, +0   ← B-only guest instruction path — corresponding site runs native on A (part of DEAD0000)
  ...
  ```
- **attribution math:** DEAD0000 +196 (A-native) + 0x801298A4 +8 (A-only guest instruction path) − 198 (sum of B-heavy non-native ra's) = **+6 net = the residual delta**. So the +6 is composed of two forces:
  1. A's native `record_gate` runs 196 allocs (untracked-by-ra because native code doesn't set r[31] for its C helper). Many of those correspond to B's non-DEAD0000 guest instruction path buckets (0x8013A3DC/0x8003AEA4/0x801350A0/…). Their sum is ~198, so A's native + B's guest instruction path are 2 apart — mostly timing-symmetric.
  2. **0x801298A4 = +8 A-only guest instruction path = the concrete asymmetric caller.** Anchored at ra=0x801298A4 which sits inside `overlay guest 0x8012982C` (overlay A00). FUN_8012982C is a 4-record-alloc init loop (authenticated executable/overlay evidence). A invokes it 2 times → 8 allocs. B invokes it 0 times in the 30 s window.
- **the caller chain**: `beh_anim_trigger_gates` (native, FUN_80129C00, game/ai/beh_anim_trigger_gates.cpp:54) STATE 0 with `node[3]==2` does `leaf(c, obj, 0x8012982Cu)` = `typed runtime address dispatch(c, 0x8012982C)`. On A, native BehaviorDispatch invokes `beh_anim_trigger_gates` which dynamically dispatches to 0x8012982C, allocating 4 records via the internal loop. **On B (the retired alternate-execution flag=1) the guest instruction path of FUN_80129C00 exists (as verified by the dispwatch pattern on 0x8013B2E4 — B does invoke A00 handlers)** — but the specific object slot that reaches STATE 0 node[3]==2 doesn't fire on B in this window, likely because B's overall scheduling is 30 s worth of gameplay behind (5329 vs 6391 STATE-1 tick counts for flagbit → B is running slower).
- **confidence gap I'm being explicit about:** the manager asked "name the real caller." The largest A-only asymmetric guest instruction path bucket is +8 (ra=0x801298A4 = beh_anim_trigger_gates STATE 0 → FUN_8012982C init). That accounts for 8 of the +6, but the actual +6 is a NET after B-heavy compensation ra's. Whether the +8 at 0x801298A4 is genuinely A-only OR a 30-s-window timing shift (like flagbit turned out to be — 2 fires each, 36 ordinals apart) is NOT PROVEN yet — the same category-error trap as before. To close it: run SBS for 90–120 s, watch whether B eventually fires FUN_80129C00 STATE 0 node[3]==2 twice too. If it does → same "timing shift, not caller divergence" pattern as flagbit → the +6 is not attributable to a single caller; it's a scheduling-cadence residual across ~10 boot-time behaviors. If B never does → real caller-side divergence, and the fix is same as manager's original (b): make B's overlay code path invoke this handler at the same time A's native path does.
...
- **refs:** runtime/psx/sbs.cpp:143-153 (RaBucket + mAllocRa), storeCb bucket increments, dumpAllocRa impl, run() atexit + SIGTERM/SIGINT hooks; game/ai/beh_anim_trigger_gates.cpp:54 (native typed runtime address dispatch to FUN_8012982C — the ra=0x801298A4 site); authenticated executable/overlay evidence (FUN_8012982C 4-record init loop, ra=0x801298A4 = JAL-return after typed runtime address dispatch to FUN_8007AAE8); `$CLAUDE_JOB_DIR/tmp/at2.log` (session capture).

## 3-slot 0x800ED098 delta caller **FALSIFIED** — flagbit_timer_machine STATE 0 fires 2× on BOTH cores (guest instruction path on B verified); +3 net source still unattributed
- **finding (2026-07-03, dispwatch probe on typed runtime address dispatch to 0x8013B2E4):** Landed a per-address per-core dispatch counter in `overlay_router.cpp:typed runtime address dispatch` (env `PSXPORT_DISPWATCH=0x…`, logs `[dispwatch] core=A|B addr=… node=… s0=… stage=…`). Runs in the router before slot-based routing, so it catches every typed runtime address dispatch to the watched address regardless of overlay residency.
- **result — B DOES reach FUN_8013B2E4 via guest instruction path, and hits STATE 0 the same 2 times as native A:**
  ```
...
- **the +3 net residual IS real** — end-of-30s-run: A cnt=76, B cnt=79, delta=-3 (A holds 3 more allocated). Total decrement counts: A=944, B=938, delta=+6. Both figures match the ALLOCTRACE cum=+6 and 0x800ED098 byte delta of 3 reported at f217. But the -82 delta at ra=80051BC4 vs +196 at DEAD0000 (native-only, no B counterpart) shows the +6 net is scattered across many callers, some going through native record_gate on A and their guest instruction path equivalents on B. **The +3 is not a single caller — it's a scheduling asymmetry between native+guest instruction path on A and historical guest-only on B, distributed across allocs.**
- **manager's structural fix ("B's guest instruction path dispatcher reaches overlay handler") is ALREADY SATISFIED:** overlay_router.cpp routes typed runtime address dispatch(0x8013B2E4) via A00's resident switch, `overlay guest 0x8013B2E4` runs `overlay guest 0x8013B2E4`, STATE 0 hits 2 times as expected. There is no missing dispatch path to close on this handler.
- **corrected next probe (NOT landed):** the +3 residual is not attributable to one caller. To close it we'd need per-frame per-ra delta over the whole run (extend ALLOCTRACE to bucket by-ra) — that surfaces WHICH ra's contribute the residual +6 alloc after the timing shifts settle. Only then can we point at a specific native vs guest instruction path asymmetry. My earlier close-out on flagbit_timer_machine was premature.
- **workflow lesson (record dead ends too):** the initial `[flagbit] A=2 B=0` result was measured with a native-only probe (behavior_dispatch.cpp:106 entry), which structurally could never fire on B (the retired alternate-execution flag=1). Concluding "B misses this behavior" from that measurement was a category error. The correct instrumentation for "does B run this behavior?" is at the guest instruction path-router entry (`overlay_router.cpp:typed runtime address dispatch`), not the NATIVE-dispatcher entry. When probing whether behavior X fires on the substrate side, hook the substrate router, not the native handoff.
- **refs:** runtime/psx/overlay_router.cpp:157-179 (typed runtime address dispatch + PSXPORT_DISPWATCH), game/ai/beh_flagbit_timer_machine.cpp:51-55 (PSXPORT_FLAGBIT_ENTRY), `$CLAUDE_JOB_DIR/tmp/dw2.log` + `fe.log` (session captures — both-sides symmetric STATE 0 hit count).
...
- **finding (2026-07-03, probe 1 landed — per-store ra logging + core-attribution):** Added the two probe hooks the handoff called for and the previous "indirect via Sop::fieldMode" hypothesis is FALSIFIED in favor of a specific native-vs-PSX behavior-dispatch asymmetry:
  1. `mem.cpp:wwatch_check` now prints `[wwatch] core=%p store […]=… by pc=… ra=… stage=…` — pc alone can't attribute the caller (guest instruction path always emits pc at basic-block entry of the target fn), but ra IS the caller's return address inside the guest instruction path substrate.
...
- **why B misses it:** `FUN_8013B2E4` is OVERLAY code (per beh_flagbit_timer_machine.cpp:1 header — "overlay handler, disassembled from scratch/ram/field_seaside.bin"). The native BehaviorDispatch table (game/object/behavior_dispatch.cpp:106) has it registered so A's native object-list walk (`c->engine.behaviors.dispatchObj`) invokes native `beh_flagbit_timer_machine`. B runs with `the retired alternate-execution flag=1` — B does NOT enter the native scheduler's object-walk path, so it never consults `BehaviorDispatch`. B's PSX-side scheduler is the guest instruction path resident dispatcher; if that dispatcher's handler-jump for 0x8013B2E4 lands on address whose source generation isn't in B's set (overlay), B silently does nothing at that handler. Result: A dispatches the behavior + allocates 6 records; B never dispatches + never allocates → +6 alloc lead, +3 net after releases also diverge.
...
- **corrected fix direction:** two paths, pick per the port's architecture:
  - **(a) match the FULL-PSX B path.** Make the SBS harness's B path invoke `BehaviorDispatch::dispatchNative` first (like A), and drop the `the retired alternate-execution flag` gate for BehaviorDispatch. This aligns the two cores on native behavior ownership but partially defeats the "full PSX comparison" purpose. Cheap.
  - **(b) match the NATIVE A path from B via guest instruction path** — the intended long-term shape. Ensure FUN_8013B2E4 is EITHER in B's guest instruction path set OR make B's resident overlay-loader populate the same behavior handler. Structural, matches the port's "PSX substrate is behavioral ground truth" invariant.
  - Either way, the "correct" behavior is that BOTH cores run the flagbit behavior at boot; only the mechanism differs.
- **refs:** runtime/psx/mem.cpp:75-79 (wwatch print with ra), runtime/psx/sbs.cpp:636 (core-map print), game/world/graphics_bind.cpp:71-80 (GraphicsBind::recordAlloc `__builtin_return_address(0)` under PSXPORT_RECALLOC_TRACE), game/ai/beh_flagbit_timer_machine.cpp:63-66 (PSXPORT_FLAGBIT_TRACE), game/object/behavior_dispatch.cpp:106 (native registry entry for 0x8013B2E4), scratch/decomp/ram_f1000_all.c L31632-31683 (FUN_800519E0 multi-record loop decomp = the 3-alloc site), `$CLAUDE_JOB_DIR/tmp/ww2A.log`+`ww2B.log`+`flag.log` (session captures).
...
- **decomp of both sides shows spawn.dispatch itself is NOT the direct writer of 0x800ED098** (spawn.cpp:180-223 + authenticated executable/overlay evidence guest 0x8007A980):
  ...
  * guest instruction path `guest 0x8007A980` for cls=3 dispatches to `guest 0x8007A12C` (shard_5.c:11296). Per the spawn.cpp:198-200 comment, native `pool_spawn(POOL_VAR3)` is byte-perfect reimpl of 0x8007A12C (verified in commit af27fd8).
...
- **refs:** runtime/psx/sbs.cpp `mAllocTraceOn`/`storeCb` + arm block, game/scene/sop.cpp:252-261 (the 3-iteration spawn loop — the exact-count source), authenticated executable/overlay evidence L_801094F8 (guest instruction path equivalent), scratch/decomp/functions.csv (find FUN_8007A980 + FUN_8007AAE8 bodies for the next decomp), $CLAUDE_JOB_DIR/tmp/alloctrace4.log (session capture).

## FULL-mode 2-frame A→B lead ROOT-CAUSED: native DEMO/GAME dispatcher runs substates ~1 frame ahead of the coro/guest instruction path — introduced twice (+1 at DEMO leave, +1 at GAME cutscene entry); durable `PSXPORT_SBS_STAGETRACE=1` diagnostic landed [PARTIALLY CLOSED — Slip #2 fixed 31fa879; Slip #1 residual remains, tracked below]
- **finding (2026-07-03, attack (a) — stagetrace of sm[0x48..0x4e] + task-entry per core per lockstep frame):** In `PSXPORT_SBS_MODE=full` with the new `PSXPORT_SBS_STAGETRACE=1` diagnostic (runtime/psx/sbs.cpp), the 2-frame A→B lead that produces the `0x800ED098` off-by-3 divergence has TWO distinct +1-frame slips, both at NATIVE-vs-guest instruction path stage-body cadence boundaries. Not a specific opcode bug — a structural pacing gap.
...
- **native path advantage — where it comes from:** two scheduler branches in `runtime/psx/scheduler.cpp` that run stage BODIES synchronously on A (native_content=true) while B runs them as pure guest coros that yield mid-body:
  1. **DEMO native (scheduler.cpp:127-169):** A's DEMO fresh entry runs `c->engine.demo.stageMain()` (full prologue + s0 in one C call) THEN `c->engine.demo.frame()` (one substate dispatch) in the SAME scheduler tick. B's coro path runs 0x801062E4 guest instruction path — its own prologue equivalent yields ONCE mid-way (via `FUN_80051f80` per-frame yield), so the prologue costs one extra scheduler tick.
  2. **GAME native (scheduler.cpp:210-273):** A's GAME fresh entry runs `c->engine.stagePrologue()` then `c->engine.frame()` (which dispatches sm[0x48], reaches Engine::fieldRun, and its sm[0x4e]=0 case in one C call runs Pool::init + resetControlBlock + seedAreaObjects + placeAreaObjects + reset75240 + setupViewScroll + finalViewInit ALL in one C call, then sets sm[0x4e]=1). B's coro of 0x8010637C yields mid-way through the SAME sequence — its guest instruction path of 0x80106b98 sm4e=0 case does 8 typed runtime address dispatch calls whose deep leaves yield to the scheduler, spreading the work across multiple ticks.
- **why "cadence match" is the real fix, not the SBS-side hold band-aid:** the SBS harness EXISTS to name real gameplay divergences. A 2-frame lead in the boot pool-alloc phase is REAL scene-time desync that gameplay reads see (docs/findings above: 0x800ED098 gates spawn thresholds). Suppressing it inside SBS (option b) would tell every future divergence session "no problem here" while the port itself would still ship a native path that reaches gameplay state 2 frames ahead of the substrate baseline — divergences from that lead would masquerade as genuine bugs. Correct fix is to make native DEMO/GAME dispatcher's per-tick work match what the guest instruction path substrate does per tick.
- **fix direction (NOT landed — needs the next session):** either (a) split the native DEMO/GAME fresh-entry work so the C call yields after each substate the guest instruction path would have yielded after (mirroring `FUN_80051f80`'s frame boundaries — likely requires each Engine::frame() step to return to scheduler if it has done its "one substate's worth"), OR (b) make the coro path skip its `FUN_80051f80` yield when the substate is one that native would have run inline (extends `typed runtime address dispatch` with a per-fn hint). Option (a) is cleaner but touches every native stage dispatcher; option (b) is one-place but pollutes the substrate. Preferred (a) — the guest instruction path yield IS the ground truth for stage-body pacing, so native should match it.
- **first-attempt fix TRIED & REVERTED (this session):** removed the `c->engine.demo.frame()` call after `demo.stageMain()` on DEMO fresh entry in scheduler.cpp (line 148), on the hypothesis that native fresh entry was running TWO DEMO substates (s0 via stageMain + s1 via frame) vs coro's one. Change worked in isolation — trace shows f2 A sm4a=0 instead of 1 — but did NOT reduce the overall lead: A still reached GAME_ENTRY at f26 and B at f27 as before. Post-fix ramdiff at f240 stayed at 4794 bytes (same as pre-fix). This proves the DEMO substate cadence isn't the actual bottleneck. What IS the bottleneck (verified from the reverted-fix trace):
  * f1: A enters DEMO (task-0 entry rewritten from 8010649C→801062E4). B stays at 8010649C until f8. That's a **7-frame START.BIN lead** (native `startBinStage` in scheduler.cpp:400 runs the file-table build synchronously in one tick; B's coro of 0x8010649C takes 7 ticks to progress the guest instruction path).
...
- **per-tick verbose stagetrace landed (`PSXPORT_SBS_STAGETRACE=2`):** logs EVERY tick in f22..f36 with the full sm state (sm[0x48/0x4a/0x4c/0x4e/0x50] + task-0 base state + init48 selector + cut flag). Confirmed the slip windows tick-by-tick:
  * **Slip #1 = DEMO sub-machine 0x80106AC4 (main-menu, dispatched from `demo_frame_s3`).** f24 both cores sm48=3/4a=1. f25 A sm48=5, B still 3. Iter 25 A: `demo_frame_s3` → `typed runtime address dispatch(0x80106ac4)` returns v0=2 with sm[0x68]==2 → advances sm48=5 in one tick. Iter 25 B: SAME typed runtime address dispatch — but returns a value that doesn't advance sm48. B needs an extra tick (f26 catches up). The sub-machine is called with `typed runtime address dispatch` (SYNC advertised) on both sides, so the divergence is INSIDE the sub-machine body — likely a per-frame input/timer read that native saw as "advance now" and coro saw as "one more tick". Concrete symptom: v0 differs on the SAME frame between the two cores. This is the specific writer of the +1 in Slip #1.
...
- **fix path narrowed:** for Slip #1, either (a) instrument the 0x80106AC4 guest instruction path to see WHICH iteration returns v0=2 on B vs A (grep decomp for it — likely a menu input debouncer), OR (b) hold native's demo_frame_s3 for 1 extra tick when the sub-machine indicates "just advanced to LEAVE-DEMO" (matches coro's tick count without touching guest instruction path). For Slip #2, either (a) find the mid-body yield in SOP's guest instruction path fieldMode and either drive it sync in the substrate or defer native by 1 tick, OR (b) same hold-one-tick approach in native Sop::fieldMode. Attempts NOT landed this session — needs the decomp analysis of 0x80106AC4 first (the actual "does one iteration of a menu-input debouncer take a different count on native vs coro") before a real fix; a blind 1-tick hold is a stopgap, not a root cause.
- **durable tool landed:** `PSXPORT_SBS_STAGETRACE=1` (runtime/psx/sbs.cpp) — on every SBS lockstep frame during f0..f250, prints A vs B TASK0_ENTRY + sm[0x48/0x4a/0x4c/0x4e] + CUT_FLAG whenever any of them change on either core. Re-usable for any future "when did the two cores diverge in stage progression?" hunt. Zero cost when off.
- **refs:** runtime/psx/sbs.cpp `stagetrace` block (~line 675 in `Sbs::Impl::run`), runtime/psx/scheduler.cpp:127-169 (DEMO native), :210-273 (GAME native), :283-331 (coro fallback for B — the guest instruction path-substrate path), game/scene/engine_stage.cpp:571-584 (Engine::fieldRun case 0 — the 7-native-call cluster that runs in one C call on A), game/scene/engine_demo.cpp:660-661 (Demo::frame's pool.init call — never reached on B), scratch/decomp of 0x80108784/0x80106b98 (guest instruction paths whose per-frame yields set the substrate cadence), `$CLAUDE_JOB_DIR/tmp/stagetrace.log` (session capture).
...
- **refs:** runtime/psx/sbs.cpp — `isObjectPoolNoise`, filter improvement in `storeCb` divergent-check, `ramdiff` cmd using `isDiffNoise`.
...
- **status of the first-byte finding (0x800BF878):** RE'd — it's a per-frame counter incremented once per `Engine::fieldFrame` call. `engine_stage.cpp:392-393` does `mem_w32(0x800bf878, mem_r32(0x800bf878) + 1)` unconditionally. Both cores hit this via different PCs — `PSXPORT_WWATCH=0x800BF878,0x800BF87C` shows:
  * Core A (native gp) writes with `pc=0x8008E0C0` (stale from last typed runtime address dispatch — FUN_8008E0C0, a small helper unrelated to the counter — native code doesn't update pc, so this is just the last-known guest PC).
...
- **status:** ROOT CAUSE CONFIRMED. Not a `mem_w8` writer at all — a `memcpy(c->ram, snapshot_ram, 0x200000)` in `DualviewSnapshot::restorePre` (runtime/psx/dualview_snapshot.cpp:18) called from `Engine::fieldFrame` (game/scene/engine_stage.cpp:428). The flow inside fieldFrame:
...
- **new SBS tooling in this session (runtime/psx/sbs.cpp):** `PSXPORT_SBS_PREWATCH=<hex>` arms the write-watch at boot (before f217 pause); pause only on DIVERGENT stores (not same-value shared writes); per-store `[sbs-ww]` log with A/B attribution + pc + stage; `PSXPORT_SBS_FORCE_PSX_RENDER=1` forces both cores through PSX render (the culprit-bisecting knob). These land as the durable workflow-first output of the hunt.
- **refs:** game/scene/engine_stage.cpp:391–431 (Engine::fieldFrame full flow — capturePre / render / submit / restorePre), runtime/psx/dualview_snapshot.cpp:13–18 (capturePre/restorePre memcpy), game/game_tomba2.cpp:108 (Engine::frameUpdate the retired in-process comparison mode early return — WHY frameUpdate isn't the location), scratch/decomp/ram_f1000_all.c:44026 (downstream reader that makes the byte gameplay-relevant), scratch/decomp/ram_f1000_all.c:37494/37567 (the `=1` writer FUN_80056C00). Previous incorrect hypothesis kept for history below.
...
- **evidence at f1672 gameplay via `sbs ramdiff`:** 2085 RAM spans / 5595 B diverge between A (native gp) and B (PSX gp) in the widened 0x80010000..0x80200000 region + 12 B in scratchpad. Real, large gameplay divergence — same class of native-vs-guest instruction path divergence that produces wrong SFX under native. Not a harness artifact.
...
- **known separate issue (not this):** standalone `PSXPORT_RENDER_PSX=1` renders black at f400 even with `PSXPORT_AUTO_SKIP=1` reaching free-roam. Plumbing likely stale; low priority because SBS's B pane shows the full scene via the retired alternate-execution flag, which is the actually-used PSX render route.
- **refs:** runtime/psx/sbs.cpp:227 (apply_mode gameplay = PSX render on both), runtime/psx/scheduler.cpp:114 (native_content gate), runtime/psx/projprim_state.h + gte_beetle.cpp (per-Core depth cache, e395095), scratch/screenshots/sbs_gameplay_pp.png (reproduction).

## SBS DEMO attract-path crash — hits an unavailable to the then-current guest executor overlay function 0x80109F7C
- **symptom:** with SBS running headless and no autonav (`PSXPORT_SBS=1 PSXPORT_SBS_MODE=<any>`, no `PSXPORT_SBS_AUTONAV=1`), letting DEMO idle to its attract timeout aborts core A with `rec-MISS 0x80109F7C` at `demo_frame_s7` → `overlay guest 0x80106E28` → `guest 0x80059D28` → `guest 0x8005950C` → `guest 0x80058648` → `runtime dispatch fault`. Standalone at 1500 frames of DEMO idle never enters the attract path (`sm[0x48]` stays at 2), so this is SBS-scoped.
...
- **cause hypothesis:** SBS's `step_core` sets `0x1f80019d=1` (the Start-skip flag) every frame the DEMO SM is in intro-FMV sub-state 1, cycling DEMO substates faster than standalone → reaches `demo_frame_s7` phase-1 → `typed runtime address dispatch(0x80106e28u)` → chains into overlay code (a0b/a0i/etc) that is NOT guest into the resident set. Interpreter is gone → fail-fast miss BY DESIGN. Fix would be seeding those attract-path functions into the guest instruction path set OR having Demo::frame's s7 stay native through phase-1 without crossing into unavailable to the then-current guest executor overlay.
- **refs:** game/scene/engine_demo.cpp:635 (demo_frame_s7), runtime/psx/sbs.cpp:295 (intro-FMV skip flag).
...
- **code reading (what SHOULD be different between A and B in gameplay mode):**
  - `sbs.cpp:504-505` sets `g_a->the retired alternate-execution flag=0, g_b->the retired alternate-execution flag=1`.
  - `apply_mode(M_GAMEPLAY)` forces `g_render_psx=1` on both (SAME renderer BY DESIGN — that's the point of isolating gameplay).
  - `scheduler.cpp:114` reads `native_content = !the retired alternate-execution flag` → on B skips the native DEMO/GAME/SOP dispatchers and routes stage bodies through guest instruction path coroutines.
  - `Engine::frameUpdate` (`game_tomba2.cpp:113`) runs on BOTH cores; in `the retired in-process comparison mode` it just calls `typed runtime address dispatch(0x800788AC)` (per-frame state-update leaf, same on both) + `spu_audio_frame_logic()`, then returns before host output. So the frame-body is guest instruction path on both; only the SCHEDULED STAGE BODIES differ (A native, B guest instruction path).
  - Per memory [[byte-perfect-ab-is-baseline-not-truth]] and countless "0-diff verified" per-native notes, every promoted native was gated on a bit-exact A/B vs `original guest-body call`. If ALL covered natives are bit-exact, RAM at frame boundaries IS identical between A and B — meaning "same panes / no divergence" is the CORRECT current output, not a harness bug. This would mean SBS's failure to surface the wrong-SFX perturbation isn't a bug in the retired alternate-execution flag gating — it's that the perturbation lives outside what SBS diffs.
- **what SBS does NOT diff:** SPU register state, VRAM, per-frame transient scratchpad within a frame (only frame-boundary snapshot is compared). If a native subsystem calls a libsnd/SPU leaf via `typed runtime address dispatch` with wrong args, the SPU registers diverge but guest RAM does not — SBS can't see it.
...
- **refs:** wrong-SFX investigation 2026-07-02, `runtime/psx/sbs.cpp:504-505` (the retired alternate-execution flag per core), `runtime/psx/scheduler.cpp:114` (native_content gate), `game/game_tomba2.cpp:113` (Engine::frameUpdate the retired in-process comparison mode branch), `runtime/psx/spu_beetle.c` (per-instance SPU — candidate for the extra diff).

## SBS both cores freeze after Start — the retired in-process comparison mode skips the SPU/XA advance → intro-cutscene XA clip wait never clears
...
- **status:** PARTIAL — XA clip wait FIXED (later-271); a DEEPER guest-fiber register-corruption freeze remains (next block).
- **cause:** in SBS both cores set `the retired in-process comparison mode=1`, and `ov_frame_update` (game_tomba2.cpp) early-returned on `the retired in-process comparison mode` BEFORE calling `spu_audio_frame()`. So the per-core XA stream never advanced → `xa_stream_voice_busy()` stayed true → task-2 (`0x801fe0e0`) never cleared → the intro-area cutscene `while(*(0x801fe0e0)!=0)` wait never cleared → freeze. The later-270 fix (advance XA when active even headless) was entirely bypassed in SBS because the retired in-process comparison mode returns first. This is the SAME audio-gated wait as later-270, surfaced via the SBS/the retired in-process comparison mode path.
- **fix (later-271):** added `spu_audio_frame_logic()` = `spu_audio_frame_ex(0)` (spu_audio.c): advances spu_update + drains spu_render (driving `CDC_GetCDAudioSample` → XA decode) but suppresses ALL output (no SDL device feed, no WAV, no native-music mix). `ov_frame_update`'s the retired in-process comparison mode branch now calls it `quota`× before returning. Per-core SPU/XA bind makes it safe; the two cores share the one output device so neither feeds it (no double audio). Verified: core B task-2 `0x801fe0e0` now clears 2→0 (clip completes).
- **refs:** runtime/psx/spu_audio.c (spu_audio_frame_ex/spu_audio_frame_logic), engine/game_tomba2.cpp (ov_frame_update the retired in-process comparison mode branch), [[Headless audio-gated game LOGIC hangs]], later-271

## SBS core-B (full-PSX) field FREEZES deeper — recorded control flow omitted FUN_8003c048's in-function jump-table `jr` (skips epilogue → corrupts the GAME-loop's callee-saved s0/SP)
...
- **the corruption mechanism (SHARP, btyield SP evidence):** the corruption is a one-time event at GAME-loop yield #36 (the first frame the field reaches its full sub-state-2 path). Comparing the last-good yield #35 to #36:
  ...
  - The C backtrace at #36 is SHALLOW (thread_main→overlay guest 0x801063F4→yield). So the field chain RETURNED in C (C stack unwound to the loop), but left the GUEST SP (r29) and callee-saved s0–s3 at the deep callee's values — i.e. a guest function in the field chain returned WITHOUT restoring guest r29/s0–s3.
...
- **leads for the fix (next session):** (1) find the guest function in the chain (FUN_80052078 / 0x801087CC / 0x80108784) whose nested-task-dispatch path returns without restoring the caller's guest r29/s0–s3 — likely the guest-fiber handling of a NESTED FUN_80052078 dispatch needs to save+restore the full register context (r29 + s0–s7) around the nested task, the way the top-level scheduler does via task_ctx. (2) The native GAME-coop path is immune because it re-seeds r16/r17/r18 AND re-enters at the loop top each frame (native_boot.cpp ~400); a STOPGAP (not a real fix) would be to do the same for the coro path, but the real bug is the nested-dispatch context save/restore. Catch the exact failing epilogue by single-frame-stepping yield #35→#36 with btyield.
- **refs:** runtime/psx/native_boot.cpp (ov_switch btyield C-backtrace, GAME-coop re-seed ~400, coro path), authenticated executable/overlay evidence (overlay guest 0x801063F4 loop: bump *(r16+0x198) + typed runtime address dispatch 0x80051F80), engine/engine_stage.cpp (overlay guest 0x80108784 field dispatcher), tools/disas.py + capstone-on-dump for the 0x80109xxx field overlay
...
- **status:** fixed (render_queue.cpp flush the retired in-process comparison mode gate)
- **cause:** `psxport_settings.ini` has `fps60=1`, so `g_fps60_on` is set. `RenderQueue::flush` (render_queue.cpp) diverted the sorted queue to `fps60.rq_capture()` and RETURNED without the inline `gpu_emit_rq_item` loop — on the assumption that `fps60_present_vk` would emit+present both interpolated halves later. But under SBS both cores set `the retired in-process comparison mode=1`, and `ov_frame_update` (game_tomba2.cpp:129) early-returns on `the retired in-process comparison mode` BEFORE `fps60_frame_commit`/present — so the captured queue was NEVER drawn. Net: world quads queued, fps60 captured them, nothing emitted them to the batch, `gpu_gpu_render_readback` rendered an empty batch = black pane. (Matches the trace exactly: worldquads>0, tex=0.)
- **fix:** gate the fps60 capture on `!core->game->the retired in-process comparison mode` (render_queue.cpp flush). In the retired in-process comparison mode the per-core present is suppressed and the SBS composite reads the geometry batch directly via `gpu_gpu_render_readback`, so the flush must do the inline emit. Verified: both panes now full (native region-nonzero=76800/76800 tex=11382; PSX 76796/76800 tex=4497); scratch/screenshots/sbs.png.
- **refs:** engine/render_queue.cpp flush (g_fps60_on && !the retired in-process comparison mode); engine/game_tomba2.cpp:129 (the retired in-process comparison mode early-return); runtime/psx/sbs.cpp grab_pane; scratch/screenshots/sbs.png

## Full-PSX SBS modes (gameplay/both) fail-fast at a scheduler yield-return (post-interpreter)
- **symptom:** `PSXPORT_SBS_MODE=both` (or `=gameplay`) aborts at frame 0 with `[historical guest-entry miss 0] no guest fn for 0x80051FA4 (caller ra=0x80051FA4, a0=0xFF000000)`. `mode=render` is fine (runs indefinitely).
- **status:** fixed (later-264) — full-PSX runs guest-instruction-only via thread-fiber coroutines + load-time overlay-identity routing; both cores boot START→DEMO→GAME with 0 historical guest-entry miss, no crash/hang.
- **cause:** `mode=both`/`mode=gameplay` run core B as FULL PSX (`the retired alternate-execution flag`): the cooperative tasks run under the guest execution via the generic path `rec_coro_run(c, resume_pc)`. Since the interpreter removal (later-254) `rec_coro_run` is just `typed runtime address dispatch` (dispatch.cpp:29) — it can only ENTER a guest function at its TOP. A yielded task resumes at its saved `r31` which is a MID-FUNCTION PC (0x80051FA4 = the `lw ra,16(sp)` right after the yield in the task-switch primitive FUN_80051F80, which stores the scheduler state at 0x1F800138). No guest entry exists there → fail-fast.
- **fix (later-264, USER directive "guest-instruction-only; condvars with pause-resume"):** each full-PSX cooperative task runs on its OWN fiber thread (`Coro`, runtime/psx/coro.{h,cpp}) that BLOCKS on a condvar at a yield — the whole nested C call stack is preserved, so resume CONTINUES exactly there, guest-instruction-only, no interpreter. Strict ping-pong so the shared `Core::r[]` is save/restored around the handoff like the old longjmp scheduler. `ov_switch` coro-yields (or `Coro::exit_now` on task-end) when `cur_is_coro`, else longjmps. `Coro::cancel` unwinds a fiber blocked mid-yield before destruction (else `~Coro` destroys a condvar with a live waiter → hang/abort). Native path UNTOUCHED (coros fire only in the retired alternate-execution flag). Also fixed two secondary blockers surfaced by the full-PSX miss-loop: (a) stage/task entries owned natively (DEMO 0x801062E4, GAME 0x8010637C, START 0x8010649C, sub-task 0x8004514C) were unseeded → seeded in the removed CPU-source emitter; (b) the overlay router identified the resident overlay by a content signature = the raw .BIN's first bytes, but the game MUTATES its header pointer table at runtime (GAME +0x08 swapped 0x80106510→0x80106470) so the live bytes no longer matched → 0x801063F4 mis-routed → MISS. Fixed by recording the resident overlay at LOAD time (`overlay_note_load`, when the fresh image still matches its signature) into a per-core `SchedulerState::resident_ov[slot]`; the router routes by that IDENTITY, robust to later mutation. See [[overlay-router-resident-by-load-identity]].
- **refs:** runtime/psx/coro.{h,cpp} + test_coro.cpp; runtime/psx/native_boot.cpp (ov_switch, the coro path in native_scheduler_step); runtime/psx/overlay_router.cpp (overlay_note_load + resident_overlay); runtime/psx/cd_override.cpp (loaders call overlay_note_load); the removed offline emitter EXTRA_SEEDS/OVERLAY_EXTRA_SEEDS; journal later-264
...
- **refs:** runtime/psx/spu_audio.c (spu_audio_frame consumer gate + xa_stream_is_active decl), runtime/psx/xa_stream.c (CDC_GetCDAudioSample drives decode), later-270

## Full-PSX field-mode does NOT fully run under coroutines past the intro cutscene (deeper limit)
- **symptom:** after the prologue-split + SPU-advance fixes, the full-PSX (the retired alternate-execution flag / SBS core B) field reaches free-roam, runs its loop, plays + completes the intro XA clip — but then the GAME outer loop counter *(0x1F800198) stops advancing (stuck ~34); the coro keeps yielding but the field-mode (sm[0x48]=2, sm[0x4a]=0 submode0, sm[0x4e]=1 → overlay 0x80109450) doesn't progress.
...
- **cause:** UNRESOLVED — the guest field-mode runs deep cooperative-wait/yield chains under the coro; a yield observed mid-callee shows s0(r16)=1 (a callee local, not the 0x1F800000 base), i.e. the field-mode is parked deep in a sub-wait, not at its outer loop. Whether this is a missing native HLE the guest instruction path field expects, a real guest-fiber register/stack fidelity bug, or simply more audio/resource-gated waits is NOT yet determined. This is the diagnostic full-PSX path; do NOT block the shipping native game on it.
...
- **refs:** runtime/psx/selftest.cpp, engine/engine_stage.cpp (ov_game_submode0 / 0x80109450 field-mode), runtime/psx/native_boot.cpp (coro path), later-270

## Full-PSX (SBS core-B / the retired alternate-execution flag) FREEZES after Start — recorded binary evidence SPLITS the GAME stage fn (prologue doesn't flow into its loop)
...
- **status:** FIXED (later-269) — recorded guest call graph contains a fall-through tail-call at deliberate re-entry seeds.
- **cause:** the GAME stage fn 0x8010637C is a cooperative task: a PROLOGUE (0x8010637C..0x801063F0, sets sm from *(0x1F800134), clears fields) that FALLS THROUGH into its infinite LOOP at 0x801063F4 (dispatch sm[0x48] handler → bump *(0x1F800198) → `jal 0x80051f80` yield → `j 0x801063f4`). Both 0x8010637C AND 0x801063F4 are seeded as separate functions (OVERLAY_EXTRA_SEEDS["GAME"]) because the native game_coop path re-enters the loop top per frame. But the removed CPU-source emitter emitted a bare `return;` at every function's end (emit_func) — so guest 0x8010637C (cut at the 0x801063F4 boundary) RETURNED after the prologue instead of flowing into the loop. On the full-PSX coro, rec_coro_run(0x8010637C) ran the prologue, returned (done=1), and the scheduler reaped the task as finished (native_boot.cpp coro path line ~366) → the GAME stage never ran its loop → field frozen at sm[0x48]=0. (Native path didn't hit this: it runs ov_game_frame / re-enters 0x801063F4 directly, never rec_coro_run'ing 0x8010637C as a blocking coro.)
- **fix:** emit_func now emits a TAIL-CALL to `hi` (the next fn) instead of `return` when the body FALLS THROUGH (last insn not an unconditional `j`/`jr`) AND `hi` is a deliberate re-entry seed (`reentry` = OVERLAY_EXTRA_SEEDS for that overlay). So guest 0x8010637C ends `overlay guest 0x801063F4(c); return;` → the prologue flows into the loop; the loop blocks at ov_switch (true mid-fn coro resume) and never returns. SCOPED to re-entry seeds on purpose: a global fall-through tail-call regressed the native field (executed previously-dead fall-through regions in area overlays that `jal` undiscovered fns → historical guest-entry miss 0x80124448); the discovery scan doesn't seed those, so only the documented stage-split seeds get the tail-call.
- **TDD gate:** `PSXPORT_SELFTEST=startgame` (runtime/psx/selftest.cpp) boots one the retired alternate-execution flag core, mashes Start, asserts it reaches GAME free-roam (sm[0x48]==2) AND the GAME loop runs (*(0x1F800198) advances). RED before the fix (stuck sm=0), GREEN after. NOTE: it does NOT assert the loop keeps advancing forever — entering the first area starts a long one-shot voice clip and the field's `while(*(0x801fe0e0)!=0)` wait legitimately pauses the loop until the clip ENDS, and clip progress is driven by REAL-TIME audio consumption (CDC_GetCDAudioSample), which headless logic-frames outrun. That audio-gated cutscene is NOT a freeze (with audio it resolves: clip→end_lba→s_active=0→task2 state 0→wait clears). A headless fast-forward XA pump would let the test run the whole cutscene deterministically — future workflow improvement.
- **refs:** the removed offline emitter (emit_func body_falls_through + reentry param; OVERLAY_EXTRA_SEEDS), authenticated executable/overlay evidence (overlay guest 0x8010637C → overlay guest 0x801063F4), runtime/psx/native_boot.cpp (coro path ~328-373, ov_switch ~116), runtime/psx/selftest.cpp, runtime/psx/xa_stream.c (CDC_GetCDAudioSample drives the clip), later-269

## SBS core-B (full-PSX guest instruction path) FREEZES at the title — dead libetc VSync counter (timebase)
...
- **the actual trigger (live backtrace, corrects the 0x8010A0E0 prediction below):** the abort backtrace was `ov_draw_otag → ov_scene_native → guest 0x8003C048 → guest 0x8003CCA4 → guest 0x8003CDD8 → guest 0x8003F698 → MISS`. So it was reached from the NATIVE `ov_scene_native` calling the NATIVE `ov_render_walk` — but `submit_render_walk` (engine_render_walk.cpp) FELL BACK to `original guest-body call(0x8003C048)` because the narration scene has a render-list node of TYPE 16 (jump-table tgt 0x8003C148), which the native walk did not own. The whole-body super-call then ran the guest per-object chain (8003CCA4→8003CDD8→per-area dispatch 8003F698→A00's 0x80146478, not resident) → miss. The native per-object path (`submit_perobj_flush → native_gt3gt4`) bypasses 8003F698 entirely and would never miss; the only thing forcing PSX was the one unowned node type.
- **cause (corrected — supersedes the earlier "quick-path skipped the A00 load" theory):** A00 is legitimately NOT loaded during the intro narration, and that is CORRECT — the SOP intro's area-load body (real LAB_80109164, faithfully = native_sop_area_load) loads area DATA only (to 0x8018A000), never the A00 CODE overlay; the code overlay loads LATER, in the field sub-mode (sm[0x4a]==1, ov_game_submode1 → native_transition_area_load → ov_80045080). During the intro (sm[0x4a]==0, SOP signature 0x3C021F80 present, so the NATIVE ov_sop_field_update DOES run), ov_sop_field_update dynamically dispatches the still-guest SOP ENTITY UPDATE 0x8010A0E0, whose render chain (MAIN 0x800ACB94 → 0x8003D07C → 0x8003CD08) calls the guest render walk guest 0x8003C048, which dispatches a node to the per-AREA render dispatcher guest 0x8003F698 (0x8003F698; switches on the area index 0x800BF870, =0 = A00) → FIXED `jal 0x80146478` = A00's GT3/GT4 submitter (`ov_gt3gt4_caller`). A00 code isn't resident (SOP is) → historical guest-entry miss. The NATIVE render walk `ov_render_walk` handles these GT3/GT4 submitters IN C (no overlay dispatch) and would NOT miss — the bug is that this render goes through the guest walk (reached from the un-ported entity update 0x8010A0E0), not the native one. Verified state at miss: sm[0x48]=2 sm[0x4a]=0 sm[0x50]=2, sopsig=0x3C021F80, areaidx(0x800BF870)=0, 1f800234=0; A00 (LBA 374) loaded 0 times.
- **fix (later-274):** own the remaining simple master-walk node cases in the NATIVE walk so it never falls back to the guest instruction path (`submit_render_walk`, engine_render_walk.cpp). The walk now handles, besides the existing PEROBJ/DEFAULT: the render-nothing skip cases (types 9-14,21-31 → 0x8003C2AC) and the special-effect leaf cases (types 16-19 → leaves 0x8003C2D4/464/5F8/788). The four leaves only call resident-MAIN library fns (matrix/GTE/libgpu) — never 0x8003F698, never an overlay address — so dynamically dispatching them (content stays PSX) can't miss; their prim span is depth-tagged like the other owned cases. type-4 (per later-239 occlusion note) and type-20 (its leaf conditionally jal's the field overlay 0x8011be5c) deliberately KEEP falling back. This makes the field/cutscene render stay PC-native (the user directive: reduce recorded binary evidence dependency) and removes the only path that reached the unloaded area submitter. Verified: 0x80146478 miss gone; AUTO_SKIP free-roam 0-miss; emit 14/14; SELFTEST=startgame PASS. The earlier prediction (port 0x8010A0E0 / the 800ACB94→8003CD08 chain) was for a DIFFERENT entry that AUTO_SKIP exercises; the live shipping-path trigger was the walk fallback, fixed here without porting that chain. Did NOT bandaid (no force-load of A00, no miss-swallow).
- **repro (headless, ~90s, dies at the miss) — NATIVE single core (shipping path):** `printf 'newgame\nrun 2500\nquit\n' | PSXPORT_REPL=1 PSXPORT_VK_HEADLESS=1 PSXPORT_NOAUDIO=1 ./scratch/bin/tomba2_port scratch/bin/tomba2/MAIN.EXE` — letting the narration PLAY (no Start) hits `[historical guest-entry miss] 0x80146478` + `[miss-state] sm[4a]=0 ...`. (SBS repro also works: `PSXPORT_SBS_MODE=both` + cross-then-start `PSXPORT_SBS_KEYS`.)
- **refs:** engine/sop.cpp ov_sop_field_update (typed runtime address dispatch 0x8010A0E0 entity update + native ov_render_walk), engine/engine_submit.cpp ov_render_walk (native walk, handles GT3/GT4 natively) + ov_gt3gt4_caller (0x80146478 orphan), engine/engine_stage.cpp ov_game_frame/ov_game_submode0, MAIN guest 0x8003F698 (0x8003F698 per-area render dispatch, table 0x80015268) + render chain 0x8003CD08/0x8003D07C, runtime/psx/hle.cpp (resident-overlay + miss-state diagnostics), runtime/psx/overlay_router.cpp (`ovload` channel), later-273
...
- **refs:** later-278, runtime/psx/selftest.cpp od_is_cd_cache, scratch/decomp/ram_f1000_all.c FUN_8008b8f0/FUN_8009ae60, docs/oracle.md
...
- **refs:** later-282, runtime/psx/selftest.cpp (run_oracle free-roam capture, run_oraclediff free-roam checkpoint + od_advance_to_ovsig + diff_band), docs/oracle.md
...
- **fix:** runtime/psx/selftest.cpp run_oraclediff — PAD_RIGHT define, diff_band returns range count + count-only mode (label==nullptr), interactive-play walk block + summary_div. docs/port-progress.md CURRENT FRONTIER later-283 block.
- **refs:** later-283, runtime/psx/selftest.cpp (run_oraclediff interactive-play scan), engine/engine_render_walk.cpp (ov_render_walk terrain routing), engine/sop.cpp:218, engine/native_terrain.cpp, docs/port-progress.md
...
- **status:** RESOLVED. AUTONAV now reaches real player control; a full interactive walk/jump run is
  BYTE-IDENTICAL A/B (pc_faithful vs historical guest-execution reference) for 20,000+ lockstep frames, ZERO divergences.
...
- **nav extension:** runtime/psx/sbs.cpp navStep — new Phase AWAIT_CONTROL between SKIP_CUT and DONE:
...
- **KEY RESULT:** `PSXPORT_SBS_MODE=full PSXPORT_SBS_AUTONAV=1 PSXPORT_SBS_POSTDRIVE=1` headless —
  ...
  "A/B identical", NO [sbs] DIVERGENCE / WRITE-SITE / abort. So pc_faithful is byte-exact to the guest instruction path
...
- **refs:** runtime/psx/sbs.cpp (navStep AWAIT_CONTROL phase, sbsPostdriveOn, run() dispatch fix),
...
- **goal:** `PSXPORT_SBS_MODE=skip` compares core A (`pc_skip=true`, the real `./run.sh` shortcut
  config) against core B (the pure historical guest-execution reference, `the retired alternate-execution flag=1`). A's collapsed-multi-step forks
...
- **mechanism (landed):** `Sbs::skipCompareMode()` / `Sbs::skipRendezvousReached(c, addr, minVal,
  label)` (runtime/psx/sbs.h, impl in sbs.cpp). A fork's shortcut leg calls
...
- **fork inventory — what's actually a "collapsed multi-step init" (rendezvous candidate) vs a
  ...
  (`game/`, `runtime/psx/`) turns up ~25 hits, but most are STEADY-STATE forks — two
  ...
  - **genuine load-collapse forks (rendezvous candidates), in rough boot order:**
    1. **`Engine::startBinStage` (startBinStageSkip vs startBinStageFaithful)** — the START.BIN
       ...
       **MEASURED RESULT (2026-07-10, `MODE=skip`, `PSXPORT_SBS_EXIT_FRAME=150`):
       ...
       own `CdSearchFile` loops (in `startBinStageFaithful`'s mirror of the guest-visible behavior) also run with
  ...
  - A subsequent, unrelated crash (`runtime dispatch fault 0x80111A20`, an un-guest A02-overlay
...
- **gates:** standard `SBS_MODE=full AUTONAV=combat` 95s gate — **0-diff**, unaffected (this
  ...
  cutoff); the run's eventual failure is the pre-existing, unrelated guest instruction path-miss above.
- **refs:** runtime/psx/sbs.h (skipCompareMode/skipRendezvousReached API doc), runtime/psx/
...
- **refs:** runtime/psx/mods.{h,cpp}, game.h (Mods member + oracle/setOracle),
...
- **refs:** runtime/psx/sbs.cpp (stepCore poke removal + PSXPORT_SBS_SHOT), scratch/sbscmp/
...
- **fix:** core A keeps the user's mods (settings ini loads in the Game ctor); ONLY core B is
  ...
  margin). fps60 presentation is inert under the retired in-process comparison mode (panes sample real frames), so pane A shows
...
- Classification: TIMING (phase-skew), NOT value — both cores converge to 0x02 by f3000 (BYTETRACE
  settled = CLEAN). Writer core A: Sop::transitionAreaLoad -> guest 0x8007566C -> 800753D4 -> mem_w16
  at f464.
...
## Three ActorZonedAttacker overrides never mirrored their guest stack frame
- **symptom:** would surface as an SBS guest-stack divergence (the "diverges at 0x801FE9xx" class), not
  as a crash — so nothing in a normal run flags it.
- **status:** FOUND + FIXED 2026-07-21.
- **cause:** `ActorZonedAttacker::pickAttackByRange` / `defaultSubStateMachine` / `idleTick` had NO
  frame at all — no `GuestFrame`, no `c->r[29]` arithmetic — while their guest bodies descend sp and
  spill callee-saved registers:
  | method | guest addr | oracle frame | oracle spills | native had |
  |---|---|---|---|---|
  | pickAttackByRange     | 0x801409C0 | 40 | 5 | nothing |
  | defaultSubStateMachine| 0x80143A00 | 40 | 5 | nothing |
  | idleTick              | 0x80144B50 | 48 | 4 | nothing |
  The file's other three methods (gateCheck / typeInit / approachAndFace) DID use `GuestFrame`, which
  is what makes this a plausible oversight rather than a deliberate choice. All six are installed as
  live overrides, so all six run natively.
- **fix:** added the three `GuestFrame<Size, Count>` declarations + spill tables, generated by
  `tools/binary ABI evidence <addr> --scaffold --guestabi` (never hand-derived). `port_check` confirms the
  frame axis now matches on all six.
- **how it was found — the generalisable bit:** the file had NO `// ORACLE:` markers, so `port_check`
  had never looked at it. Adding markers to an un-gated faithful port is cheap and immediately
  reported the missing frames. **Any `game/` file full of `c->mem_*` faithful bodies with no ORACLE
  marker is unverified by construction** — worth sweeping for.
- **do-not-re-derive:** `port_check` reporting `frame OPEN size mismatch: oracle=[40] native=[]` is
  NOT a tool gap — it already understands `GuestFrame` RAII. That message means the frame really is
  missing.

### Known port_check limitation found alongside (NOT a port defect)
A file-local dispatch helper (`call1(c, node, FN_8004766C);`, which sets the ra constant and issues
`typed runtime address dispatch` inside itself) is invisible to `port_check`: no `c->r[31]=…` precedes it at the call
site, so the opaque-call fallback never fires and the native call count reads low. That is why
`approachAndFace` / `defaultSubStateMachine` / `idleTick` still report FAIL on the CALL axis — the
calls are there; the tool cannot see them.
**Failed fix, do not repeat as-is:** counting every bare `name(c, …);` statement as an opaque call
regressed FOUR previously-PASSing methods elsewhere (49 PASS -> 45), because plenty of ordinary native
helpers also take `Core*` first. A correct fix must PROVE the helper dispatches — e.g. scan the
helper's own body for `typed runtime address dispatch`/`func_` before counting it — not pattern-match its signature.

## Sweep: 15 more native overrides were missing their guest stack frame
- **status:** FOUND + FIXED 2026-07-21 (follow-up to the ActorZonedAttacker finding above).
- **how:** the previous entry's lesson — "any faithful body with no ORACLE marker is unverified by
  construction" — was applied across the tree. Marking every installed override and running
  `port_check --all` raised coverage from 67 to ~370 methods and surfaced a batch of bodies that
  descend no frame while their guest bodies spill callee-saved registers.
- **fixed (file :: method — oracle frame/spills):**
  `beh_toy_spawn_family` type5 32/3 · type4 32/3 · type2 40/5 ·
  `ReleaseTriggerMotion` hoverBobCycle 24/2 · leaderFollowSync 32/? · driftReposition 40/? ·
  arcSwoopMotion 48/? · doubleArcMotion 32/? · circleOrbitMotion 48/? ·
  `ActorReward` smBlinkB 32/3 · smWindowScroll 32/? · smTallyTick 24/? · smEventDispatch 32/? ·
  `CubeTextLedger` deactivateSlot 32/? · spawnPopup 32/4.
  All spill tables generated by `binary ABI evidence <addr> --scaffold --guestabi`, never hand-derived.

### A near-miss worth keeping: one proposed fix was WRONG
A 16th candidate — `SceneEvents::delayedTrigger` — was ALSO reported as missing a 32-byte frame, and
a frame was applied to it. It is wrong: that method is the port of **0x80042258**, a leaf whose own
comment says "Leaf, no frame". The suggested oracle (0x80040B48) belongs to a different function.
Adding a frame to a genuinely frameless leaf *creates* the divergence it claims to fix.
- **what caught it:** `port_check` reported the INVERSE mismatch — `oracle=[] native=[32]`. That
  direction (native has a frame, oracle does not) is the signature of a frame applied to the wrong
  address. Reverted.
...
- **audit method (reproducible):** take every `{0xADDR, beh_symbol, "name"}` row of the dispatch
  ...
  `binary ABI evidence 0xADDR --contract` reports a non-zero frame while the body contains neither
...
- **fix per entry:** `GuestFrame<size, n> frame(c, kSpills_<ADDR>);` at the body's entry, spill table
  from `binary ABI evidence <addr> --scaffold --guestabi`. Confirm the address against the dispatch-table
...
- native `typed runtime address dispatch` callers that do NOT set r31: beh_cull_substate_leaves.cpp:303-304,
...
(cutscene_camera.cpp:935-1023) never reach the native siblings (they typed runtime address dispatch the substrate).
...
- **cause:** hand-transliteration. Each draft reproduces `c->r[29] -= N` and `+= N` but stashes the
  ...
  `binary ABI evidence <addr> --contract`: 0x8012E8A8 frame 48 / 8 spills, 0x8012F494 frame 24 / 2,
...
- **fix:** regenerate each body with `direct executable disassembly` at wiring time. It emits the prologue
...
- **DO NOT REUSE THE GREP THAT FOUND IT — it does not generalise.** "descends sp but has no
  ...
  `runtime/psx/guest_abi.h`, neither of which the pattern sees; and (2) more fundamentally,
...
- **refs:** game/ai/beh_substate_edge_leaves.cpp banner, direct executable disassembly, docs/port-framework.md
...
- **symptom (as reported):** `beh_substate_edge_orchestrator` (native, game/ai/) calls
  `typed runtime address dispatch(c, 0x80131134u)` and `typed runtime address dispatch(c, 0x801316CCu)` without setting r31, while the
  gen orchestrator sets `r31 = 0x8012EC7C` / `0x8012EC84` first (authenticated executable/overlay evidence,
...
- **status:** NOT A BUG. Every mechanical leg of it is true — `typed runtime address dispatch` never assigns r31
  (external/psxport/runtime/psx/overlay_router.cpp), the gen sets it, the native does not, and the
...
- **why it does not follow:** the beh_* table is not on the byte-exact path. `dispatchNative` gates on
  `substrateOnly = the retired alternate-execution flag || verify.inSubstrateLeg || !pc_skip`
  (game/object/behavior_dispatch.cpp:197), and SBS full constructs BOTH cores with `pc_skip = false`
  (external/psxport/runtime/psx/sbs.cpp:1984-1985 — core A is `(mMode == M_SKIP)`, which is false in
...
- **provenance:** raised as the HEADLINE hazard by an RE agent, which had verified the gen/native
  asymmetry but explicitly marked "typed runtime address dispatch does not set r31" as unverified. It was right about
...
- **refs:** game/object/behavior_dispatch.cpp:190-212, external/psxport/runtime/psx/sbs.cpp:1984
...
- **the defect:** `dynamic differential evidence` takes FILE PATHS, but every neighbouring tool in the pipeline
  (`binary ABI evidence`, `direct executable disassembly`, `codemap.py --addr`) takes an ADDRESS. So `dynamic differential evidence 800834A0`
...
- **WHAT THE WORKING GATE ACTUALLY REPORTS — `dynamic differential evidence --all` on this tree:**
...
- **refs:** external/psxport/tools/dynamic differential evidence (main); instruments I029, I030; claim C026
