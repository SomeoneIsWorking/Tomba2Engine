...
- **cause:** `gpu_vk.cpp:602` `if (!s_headless) overlay_glue_init(...)`. The window is an output SINK, not a mode (coord/PROTOCOL.md) — whether the UI EXISTS must not depend on it. Compounding it, the overlay could only be OPENED by an SDL ESC key event, i.e. only through a window.
