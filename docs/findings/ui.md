...
## RmlUi overlay: RML IS NOT HTML, and DATA MUST NOT BE HANDED TO A MARKUP PARSER (2026-08-06)
- **symptom:** the debug overlay printed the literal text `&middot;` where a `·` separator was intended — `render 1398x720 &middot; window 1536x790 &middot; internal 3x` (USER screenshot).
...
- **refs:** kanban #76; spyro issue #52; coord/claims/rmlui-overlay/claim.md

## The overlay's EXISTENCE used to depend on the window — that is why it was undiagnosable (2026-08-06)
- **symptom:** "the RmlUi overlay does not work" (spyro, USER) could not be investigated at all, because agents may not run windowed and no headless instrument could see the overlay.
- **status:** PARTLY FIXED — the EXISTENCE half is fixed and measured; the VISIBILITY half (headless rasterisation) is NOT done, see below. Framework patch `coord/patches/rmlui-overlay.diff`, claim `coord/claims/rmlui-overlay/`; not yet landed on psxport origin/main. Spyro itself was NOT rebuilt or re-measured.
- **cause:** `gpu_vk.cpp:602` `if (!s_headless) overlay_glue_init(...)`. The window is an output SINK, not a mode (external/psxport/docs/workspace/PROTOCOL.md) — whether the UI EXISTS must not depend on it. Compounding it, the overlay could only be OPENED by an SDL ESC key event, i.e. only through a window.
- **fix:** init runs in BOTH legs, taking `sink_size()` and the sink's colour format; `s_win` may be NULL and is used only for input translation. NEW REPL/debug-server command `menu [on|off|toggle]`. `RmlOverlay::newFrame()` now tracks the SINK (pixels) instead of `SDL_GetWindowSize` (LOGICAL POINTS) — see below.
- **verified:** a headless run now prints `[rmlui] overlay up (headless sink 960x720, 3/3 fonts, ...)`, which had never happened before this change.
STILL OPEN — the visibility half, named so nobody reads the above as "the overlay works headless":
          `overlay_glue_record` is called from the two SWAPCHAIN passes only, so headless brings the
          overlay UP and updates its DOM but never rasterises it. A headless capture therefore cannot
          show the menu, and the evidence above is text, not pixels. The proper fix is to composite
          the overlay in a stage that runs in both legs while keeping `shot` free of the menu — i.e.
          a sink image distinct from `s_present_img` (the game picture), not an `if (s_headless)`
          branch. Sized, not started.
- **refs:** spyro issue #52 defects 1/3/4 (1, 3 and 4 fixed here; 2, 5, 6, 7, 8 still open)
